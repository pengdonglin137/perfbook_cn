...
本节将概述前面列出的性能障碍的实际\IXpl{overhead}。
不过在此之前，需要读者对硬件体系结构有一个粗略的认识，下一节将对此进行阐述。
...
是一个粗略的八核计算机系统概要图。
每个芯片上有两个CPU核心，每个核心都有自己的cache，
芯片内还有一个互连模块，使芯片内的两个核心可以互相通信。
系统互连模块则让四个芯片之间以及与主存之间可以进行通信。

数据以``\IXpl{cache line}''为单位在该系统中传输，
cache line对应于内存中一个2的幂次方大小的字节块，
通常在32到256字节之间。
当CPU从内存中读取一个变量到其寄存器时，
必须首先将包含该变量的cache line读取到CPU的cache中。
类似地，当CPU将寄存器中的一个值存储到内存时，
不仅必须将包含该值的cache line读到CPU的cache中，
还必须确保没有其他CPU拥有该cache line的副本。
...
\QuickQuizB{
	...
	复杂~\cite{Hennessy2026,DavidECuller1999,MiloMKMartin2012scale,DanielJSorin2011MemModel}。
%
}\QuickQuizEndB
%
\QuickQuizE{
...
学科的开端~\cite{Hennessy2026,DavidECuller1999,MiloMKMartin2012scale,DanielJSorin2011MemModel}，
...
一些在并行程序中很重要的常见操作开销如
\cref{tab:cpu:CPU 0 View of Synchronization Mechanisms on 8-Socket System With Intel Xeon Platinum 8176 CPUs at 2.10GHz}所示。
该系统的时钟周期约为0.5\,ns。
虽然现代微处理器在每时钟周期执行多条指令并非罕见，
但在表格的第三列中，操作的开销被归一化到了整个时钟周期，称作``Ratio''。
关于这张表，首先需要注意的是各个比率的值都很大。
...
这里``同CPU''意味着当前对某个变量执行CAS操作的CPU正好是最后一个
操作该变量的CPU，因此对应的cache line已经在该CPU的cache中了。
类似地，同CPU的lock操作（一个包括获取锁和随后释放锁的``往返''对）
消耗超过15纳秒，即超过30个时钟周期。
lock操作比CAS更昂贵，是因为锁操作的数据结构中需要两次原子操作，
一次用于获取，另一次用于释放。
...
即近100个时钟周期。
测量cache miss所使用的代码将cache line在一对CPU中来回传递，
这样可以确保cache miss不是来自内存，而是来自其他CPU的cache。
非blind CAS操作（如前所述，需要查看变量的旧值并存储新值）
消耗超过100纳秒，即超过200个时钟周期。
想想这意味着什么：在执行\emph{一次}CAS操作的时间里，
CPU本可以执行超过\emph{两百}条普通指令。
这不仅体现了细粒度锁的局限性，
也体现了所有依赖细粒度全局一致性的同步机制的局限性。
...
简而言之，硬件工程师和软件工程师其实都在同一个战壕里战斗，
让计算机在物理定律允许的范围内跑得更快，
如\cref{fig:cpu:Hardware and Software: On Same Side}
中异想天开地描绘的那样，其中我们的数据流正竭尽全力超越光速，
还受到非零原子大小的进一步阻碍。
