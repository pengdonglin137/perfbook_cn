...
中比率的数值至关重要，因为它们限制了给定并行应用程序的效率。
为了理解这一点，假设并行应用程序使用\IXacr{cas}操作在线程之间通信。
这些CAS操作通常会涉及cache miss，
即假设线程主要是在彼此之间通信而非与自身通信。
进一步假设每次CAS通信操作对应的工作单元需要300\,ns，
这足以计算几个浮点超越函数。
那么大约一半的执行时间将被CAS通信操作消耗！
这反过来意味着，运行这样一个并行程序的双CPU系统
不会比在单个CPU上运行的顺序实现更快。

在分布式系统的情况下，情况更加糟糕，
因为单次通信操作的延迟可能与数千甚至数百万次
浮点运算所需的时间一样长。
这说明了通信操作极度稀少并能启用大量处理是多么重要。
...
教训应该相当清楚：
并行算法必须在牢记这些硬件特性的前提下进行显式设计。
一种方法是运行几乎独立的线程。
线程之间的通信越不频繁——无论是通过\IX{atomic}操作、lock还是显式消息——
应用程序的性能和可扩展性就越好。
...
简而言之，实现出色的并行性能和可扩展性意味着
要努力追求令人尴尬的并行算法和实现，
无论是通过精心选择数据结构和算法、利用现有的并行应用和环境，
还是将问题转化为令人尴尬的并行形式。
