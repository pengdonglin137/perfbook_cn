...
在商用计算机中，多核系统已经越来越常见，
本章将描述如何利用编程惯用法（design patterns）~\cite{Alexander79,GOF95,SchmidtStalRohnertBuschmann2000v2Textbook}，
来设计能更好利用多核优势的软件，
以便权衡性能、可扩展性和响应时间。
正确的分区会带来简单、可扩展和高性能的解决方案，
而不恰当的分区则会导致缓慢而复杂的解决方案。
本章将帮助你将分区设计融入代码中，
同时也会讨论批处理和弱化。
"设计"这个词非常重要：
你应该先分区，再批处理，三弱化，四编码。
颠倒这个顺序往往导致糟糕的性能和可扩展性，
伴随着极大的挫折感。\footnote{
	另一个绕过物理定律的巧妙方法——只读复制，
...
请注意，这并\emph{不}意味着你应该在每个程序中都使用多线程方式编程。
我再一次说明，如果程序在单个处理器上运行得足够快，
那么就可以避免SMP同步原语的开销和复杂性。
\cref{lst:SMPdesign:Sequential-Program Hash Table Search}中
哈希表（hash table）查找代码的简单之美强调了这一点。\footnote{
	本节中的示例取自Hart等人的
...
\IXh{Code}{locking}是最简单的设计，因为它只使用全局锁。\footnote{
	如果你的程序在数据结构中有锁，
	或者在Java的情况下使用了同步实例的类，
	那么你使用的是"数据锁"，
	在\cref{sec:SMPdesign:Data Locking}中描述。}
在已有的程序上使用代码锁，可以很容易地让程序在多处理器上运行。
如果程序只有一个共享资源，代码锁
甚至可以提供最佳性能。
然而，许多较大且复杂的程序
会在\IXpl{critical section}中执行许多次，
这就让代码锁的可扩展性大大受限。

因此，代码锁最适合用于临界区执行时间占比很小、
或者对可扩展性要求不高的程序。
...
然而，请注意，与\cref{lst:SMPdesign:Sequential-Program Hash Table Search}相比，
因为在返回前需要释放锁，\co{hash_search()}中简单的比较返回
从一条语句变成了三条语句。
...
代码锁尤其容易引起"\IX{lock contention}"，
即多个CPU需要同时获取该锁。
如果你照看过一群小孩子（或者一群像小孩子一样行事的大人），
你肯定能马上意识到只有一个玩具的危险，
...
许多数据结构都可以分区，
数据结构的每个部分带有自己的锁。
这样数据结构每个部分的\IXpl{critical section}
可以并行执行，
尽管给定部分在给定时刻只能有一个临界区实例在执行。
如果此时同步带来的开销不是主要瓶颈，
你应该使用数据锁来降低竞争程度。
数据锁通过将过大的临界区分散到各个小的临界区
来减少竞争，
例如，维护哈希表中每个哈希桶的临界区，
如\cref{lst:SMPdesign:Data-Locking Hash Table Search}所示。
不过这种可扩展性的增强带来的是复杂性的少量增加，
增加了一个额外的数据结构，即\co{struct bucket}。
...
和\cref{fig:SMPdesign:Lock Contention}中所示的紧张局面不同，
数据锁带来了和谐，如
\cref{fig:SMPdesign:Data Locking}所示——在并行程序中，
这\emph{几乎}总是意味着性能和可扩展性的提升。
...
\QuickQuiz{
	...
	\item	使用\emph{\IXpl{hazard pointer}}~\cite{MagedMichael04a}，
		可以将其视为一种由内而外的引用计数（reference counting）。
...
数据所有权方法按照线程或CPU的个数分区数据结构，
使得每个线程/CPU在不需要任何同步开销的情况下
访问属于它的数据子集。
...
会带来显著的复杂性和开销。
不仅如此，如果使用最频繁的数据恰好属于
单个CPU，该CPU就成为了"\IX{hot spot}"，
有时就会导致类似于\cref{fig:SMPdesign:Data and Skew}中所示的情况。
不过，在不需要共享的情况下，数据所有权
可以达到理想的性能，而且代码可以像
\cref{lst:SMPdesign:Sequential-Program Hash Table Search}中
所示的顺序程序情况一样简单。
最坏情况通常被称为"\IX{embarrassingly
parallel}"（令人尴尬的并行），而最好的情况，
则类似于先前在\cref{fig:SMPdesign:Data Locking}中展示的情况。
...
本节从同步效率的数学角度来考察锁粒度和性能。
对数学不感兴趣的读者可以选择跳过本节。

方法是用一种关于操作单个共享全局变量的同步机制\IX{efficiency}的
粗略排队模型，基于M/M/1队列。
M/M/1排队模型基于服从指数分布的
"到达间隔率"$\lambda$和服从指数分布的
"服务率"$\mu$。
到达间隔率$\lambda$可以被认为是系统在无需同步时每秒可以处理的
平均同步操作数，
换句话说，$\lambda$是对每个非同步工作单元开销的反向度量。
例如，如果每个工作单元是一个事务，
每个事务处理需要一毫秒（不包括同步开销），
那么$\lambda$将是每秒1,000个事务。

服务率$\mu$的定义与之类似，如果每个事务的开销为零，
并且忽略CPU必须彼此等待以完成同步操作的事实，
服务率等于系统每秒可处理的平均同步操作数，
换句话说，$\mu$可以被粗略地认为是
在没有竞争时的同步开销。
...
这又对应于几百（也许几千）条指令。
因为每条曲线都随着CPU或线程数量的增加而急剧下降，
我们可以得出结论：如果在当前商用硬件上大量使用，
使用原子操作（atomic operation）对单个全局共享变量
进行同步的机制扩展性不佳。
这就是\cref{chp:Counting}中讨论的
并行计数算法原因的数学描绘。
你的实际情况可能有所不同。

尽管如此，即使在几乎没有或
完全没有正式同步的情况下，效率的概念也是有用的。
例如，考虑矩阵乘法，其中一个矩阵的列
与另一个矩阵的行相乘（称为"点积"），
产生第三个矩阵中的一个值。
因为这些操作相互之间不冲突，所以可以将第一个矩阵的列
分配给一组线程，每个线程计算结果矩阵的相应列。
因此线程之间可以完全独立操作，
完全没有同步开销，如\path{matmul.c}中所做的那样。
这样人们可能期望并行矩阵乘法具有1.0的完美效率。
...
说明了一个不同的故事，特别是对于64乘64的矩阵乘法，
即使在单线程运行时，其效率也从未超过约0.3。\footnote{
...
在几十个线程时也明显偏离完美。
尽管如此，此图清楚地展示了批处理在性能和
可扩展性上的好处：
如果你必须承受同步开销，那就让它物有所值，
...
并行内存分配器所面临的基本问题是：在大多数情况下
快速分配和释放内存的需求，与在特殊的分配和释放模式下
高效利用内存的需求之间的矛盾。

要理解这种矛盾，考虑将数据所有权直接应用于
这个问题——简单地将内存按照CPU数划分，这样每个CPU都有专属自己的一份内存。
例如，假设一个有12个CPU的系统有64千兆字节的内存，
例如我现在正在使用的笔记本电脑。
我们可以简单地给每个CPU分配一个5千兆字节的内存区域，
允许每个CPU从自己的区域分配，不需要
锁及其复杂性和开销。
但这种简单的方案有个问题：当CPU~0只分配内存而CPU~1只释放内存时，
方案就失败了，正如简单的生产者-消费者工作负载中发生的那样。
...
\cref{fig:SMPdesign:Allocator Pool Schematic}说明了
池数据结构的操作。六个方框表示组成\co{pool}字段的指针数组，
前面的数字表示\co{cur}字段。
阴影方框表示非\co{NULL}指针，
空方框表示\co{NULL}指针。
...
如果为空，\clnrefrange{ack}{rel}尝试在自旋锁（spinlock）保护下
从全局池重新填充，该锁在\clnref{ack}处获取、在\clnref{rel}处释放。
