...
因此，你最好在那些只有一小段执行时间花在临界区（critical section）中
或者对可扩展性要求不高的程序上使用代码锁。
...
并且，代码锁尤其容易引起"\IX{lock contention}"，
即多个CPU需要同时获取该锁。
...
基于原子操作（atomic operation）单个全局共享变量
的同步机制扩展性不佳。
...
允许每个CPU从自己的区域分配，不需要
锁及其复杂性和开销。
可是这种简单的方案有个问题：当CPU~0只分配内存而CPU~1只释放内存时，
...
并在\clnref{rel}释放的自旋锁（spinlock）下从全局池中重新填充它。
