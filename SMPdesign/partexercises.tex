...
五位哲学家整天无所事事，要么思考，要么吃一种
需要两把叉子才能吃的"非常难缠的意大利面"。\footnote{
	你也可以用筷子来类比思考这个问题。}
每位哲学家只能使用紧邻自己左右两侧的叉子，
一旦拿起了叉子，不吃到心满意足是不会放下的。
...
目标是构造一个算法来——字面意义上的——防止饥饿（\IX{starvation}）。
一种饥饿场景是所有哲学家同时拿起了左手边的叉子。
因为在吃饱之前没有人会放下自己的叉子，而且他们还需要第二把叉子才能开始进餐，
所以所有哲学家都会挨饿。
...
在假设通信延迟可以忽略不计的情况下非常完美，
但这一假设在1980年代末或1990年代初就已失效了。\footnote{
...
每位哲学家先拿起自己盘子旁边编号较小的叉子，然后再拿编号较大的叉子。
因此，图中最上方位置的哲学家先拿左手边的叉子，再拿右手边的叉子，
而其余哲学家则先拿右手边的叉子。
由于有两位哲学家试图先拿起叉子~1，而且只有一位能成功，
所以只有四位哲学家抢五把叉子。
这四位中至少有一位肯定能拿到两把叉子，从而能够进餐。

这种对资源编号并按编号顺序获取的通用技术经常用在
死锁预防上。
但是很容易就能想象出一种事件序列来产生这种结果：虽然所有人都在挨饿，
但是一次只有一位哲学家在进餐：
...
双端队列是一种数据结构，其中的元素可以从两端插入或
删除~\cite{Knuth73}。
据说实现一种基于锁的、允许在双端队列两端并发操作的方法
是困难的~\cite{DanGrossman2007TMGCAnalogy}。
本节展示了分区设计策略如何产生
一个相当简单的解决方案，在以下小节中考察三种
通用方法。
...
但是至少有五种特殊情况这一事实应该引起高度警惕，
尤其是考虑到列表另一端的并发活动可以随时
将队列从一种特殊情况转换到另一种。
...
这意味着元素有时必须从一个双端队列转移到另一个，
这种情况下必须同时持有两个锁。
可以使用简单的锁层次来避免\IX{deadlock}，例如，
始终在获取右手锁之前先获取左手锁。
这将比对同一个双端队列使用两个锁简单得多，
因为我们可以无条件地将左端入列元素进入左手队列，
右端入列元素进入右手队列。
主要的复杂性出现在从空队列出列时，
这种情况下需要：
...
确定性地分区数据结构最简单、最有效的方法之一就是哈希。
可以根据元素在列表中的位置为其分配序列号来对双端队列进行哈希，
使得第一个从左边进入空队列的元素编号为零，
第一个从右边进入空队列的元素编号为一。
一系列从左边进入空队列的元素将被分配递减的编号（$-1$, $-2$, $-3$, \ldots），
而一系列从右边进入空队列的元素将被分配递增的编号（2, 3, 4, \ldots）。
关键是，实际上不需要真正为元素编号，
因为元素的序号由它在队列中的位置隐含。
...
采用这种方法，我们用一个锁保护左手索引，
用另一个锁保护右手索引，再各用一个锁保护对应的哈希链。
\Cref{fig:SMPdesign:Hashed Double-Ended Queue}展示了
给定四个哈希链的结果数据结构。
注意锁的范围没有重叠，并且为了避免死锁，
只在获取链锁之前获取索引锁。
每种类型的锁（索引或链），一次获取从不超过一个。
...
\co{pdeq_pop_r()}的实现如图中\clnrefrange{b}{e}所示。
...
\QuickQuiz{
	...
	但其他声称具有这一属性的算法也是如此。
	例如，在使用基于哈希锁数组的软件事务内存（transactional memory）
	机制的解法中，最左端和最右端元素的地址
	有时恰好会哈希到同一个锁。
	这些哈希冲突也会阻止并发访问。
...
当然，更智能的重新平衡方案可以任意复杂，
但这里展示的简单方案与软件替代方案相比
性能相当不错~\cite{LukeDalessandro:2011:ASPLOS:HybridNOrecSTM:deque}，
甚至与使用硬件辅助的算法相比也是
如此~\cite{DavidDice:2010:SCA:HTM:deque}。
不过，从这种方案中我们最多只能期望2倍的可扩展性，
...
算法~\cite{DBLP:conf/europar/Michael03}。\footnote{
	这篇论文很有趣，因为它表明不需要特殊的
	double-compare-and-swap（DCAS）指令来实现双端队列的无锁（lock-free）实现。
...
这表明对于并发程序来说，严格的FIFO属性并没有那么有价值，
事实上Kirsch等人已经证明不提供严格FIFO保证的队列
在性能和可扩展性上更好~\cite{ChristophMKirsch2012FIFOisntTR}。\footnote{
	Nir Shavit出于大致相同的原因
	提出了宽松的栈~\cite{Shavit:2011:DSM:1897852.1897873}。
	这种情况使一些人认为线性化点对理论家比对开发者更有用，
	也使另一些人怀疑这些数据结构和算法的设计者
	在多大程度上考虑了用户的需求。}
总之，如果你打算把并发程序使用的所有数据都通过
一个单一队列推送，那真的该重新思考一下整体设计。
...
相比之下，双端队列实现是"垂直并行"或
"pipelining"的例子，因为数据从一个线程转移到另一个线程。
流水线所需的更紧密协调反过来需要
更大的工作单元来获得某种程度上的\IX{efficiency}。
