...
并行算法难于编写，甚至更难调试。
测试虽然必不可少，但往往还不充分，因为致命的\IXpl{race condition}
可能具有极低的发生概率。
正确性证明是有意义的，但它与原始算法一样容易出现人为错误。
此外，不能指望正确性证明发现你假设中的错误、需求中的缺陷、
对底层软件或硬件原语的误解，
或者你没有想到要为之构造证明的那些错误。
这意味着形式化方法永远不能取代测试。
然而，形式化方法可以成为你验证工具箱中有价值的补充。

拥有一个工具能以某种方式定位所有的竞态条件（race condition），那将非常有帮助。
已经存在许多这样的工具，例如，
\cref{sec:formal:State-Space Search}介绍了
通用状态空间搜索工具 Promela 和 Spin，
...
本章描述的形式化验证技术是验证小型并行算法的
非常强大的工具，但它们不应该是你工具箱中的唯一工具。
尽管数十年来一直关注形式化验证，测试仍然是大型并行软件
...
但这两个步骤只发现拼写错误和误报的情况并不少见。
无论如何，大部分相关的 bug——即那些在生产中可能实际遇到的 bug——
通常是通过测试发现的。

当测试由预期或实际的用例驱动时，
最后一个相关 bug 通过测试找到的情况并不罕见。
这种情况可能会让人完全否定形式化验证的价值，
然而，不相关的 bug 有一个恼人的特点：它们会在最不方便的时刻
突然变得相关——这要拜黑帽攻击所赐。
对于安全关键的软件（它在总量中的占比似乎持续增长），
找到并修复最后一个 bug 因此有着强烈的动机。
测试显然无法找到最后一个 bug，因此形式化验证
有一席之地——当然前提是形式化验证能够
证明自己有能力胜任这个角色。
正如本章所示，当前的形式化验证系统
极为有限。
...
请注意，形式化验证通常比测试难用得多。
这在一定程度上是一个文化观念问题，有理由希望
随着熟悉度的增加，形式化验证会被认为更容易。
话虽如此，非常简单的测试框架就能在任意大的软件系统中
找到重大 bug。
相比之下，应用形式化验证所需的工作量
似乎随着系统规模的增加而急剧增长。

尽管如此，我在近30年中偶尔使用形式化验证，
利用形式化验证的优势，
...
形式化验证工具，你应该仔细重新考虑你的设计，
特别是如果你的形式化验证工具要求你将代码
手动翻译到专用语言的话。
...
决定投资于验证往往依赖于经验丰富且性格强势的人来推动。
但这并不能保证成功，因为其他利益相关者可能
同样性格强势。
更糟糕的是，这些利益相关者可能会举出这样的例子：
昂贵的验证工作最终还是让令人尴尬的 bug 逃逸到了
最终用户手中。
...
Linux 内核 v6.19 添加了几种 SRCU 风格，同时还向 \co{refscale}
测试套件添加了许多原语。
Linux 内核 v7.0 移除了 RCU Tasks Trace，使用新的 SRCU-fast
RCU 风格实现其 API。
