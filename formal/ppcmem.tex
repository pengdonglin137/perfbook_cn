...
虽然 Promela 和 Spin 几乎允许你验证任何（较小的）算法，
但有时候它们的通用性反而成为一种负担。
例如，Promela 不理解 \IX{memory model} 或任何类型的
重排序语义。
因此，本节描述了一些理解实际系统中使用的 memory model 的
状态空间搜索工具，大大简化了对弱序代码的验证。

例如，
\cref{sec:formal:Promela Example: QRCU}
展示了如何让 Promela 识别弱内存排序。
虽然这种方法可以正常运行，但它要求开发者
彻底理解系统的 memory model。
不幸的是，很少有（如果有的话）开发者能彻底理解
...
\QuickQuiz{
	...
	重排序的，这样所有线程（thread）都同意它们可见的所有操作的顺序。
...
\QuickQuizE{
	...
	本回答的其余部分假设
	\cref{lst:formal:PPCMEM Litmus Test}中
	\co{P0} 的汇编语言旨在实现一个返回值的原子操作（atomic operation）。
...
这些工具有望对从事 \ARM\ 和 Power 平台上底层并行原语
开发的人员提供巨大帮助。
这些工具确实有一些固有的局限性：
...
值得再次强调的是，形式化方法和工具不能替代测试。
事实是，构建大型可靠的并发软件工件（例如 Linux 内核）
相当困难。
因此，开发者必须准备好运用手头的一切工具
来实现这一目标。
本章介绍的工具能够定位那些通过测试极难暴露
（更不用说追踪）的 bug。
另一方面，测试可以应用于比本章介绍的工具
所能处理的规模大得多的软件。
一如既往，使用合适的工具做合适的工作！

当然，最好是通过将并行代码设计为易于分区，
