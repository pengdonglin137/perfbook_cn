...
在 PPCMEM 出现之前，解决这个问题需要翻阅大量参考手册、
尝试证明、与专家讨论，
然后在所有这些之后，仍然对最终答案无法确定。
...
然而，考虑到 litmus test 的简单性，所需的时间和空间
还是有些出人意料——该测试只有两个线程分别向两个不同的变量存储，
另外两个线程以相反的顺序从这两个变量加载。
如果两个加载线程对两个存储操作的顺序不能达成一致，
则 assertion 触发。
即使按照 memory-order litmus test 的标准，这个测试也相当简单。

PPCMEM 消耗大量时间和空间的原因在于它执行的是
基于跟踪的完整状态空间搜索，这意味着它必须生成
并评估架构级别上所有可能的事件和动作顺序与组合。
由于加载和存储操作对应于大量的事件和动作序列，
状态空间极为庞大，必须被完整搜索，
进而导致大量的内存和 CPU 消耗。
...
提出的公理化方法正是这样做的：它创建一组公理来表示 \IX{memory model}，
然后将 litmus test 转换为可以在这组公理上
证明或反证的定理。
由此产生的工具称为 \qco{herd}，它方便地接受与 PPCMEM
...
以上各节展示了公理化方法如何能够成功地
在 C 语言 litmus test 中对锁和 RCU 等同步原语建模。
从更长远来看，公理化方法有望对更高级别的
软件工件建模，从而实现指数级的验证加速。
这可能使得对更大的软件系统进行公理化验证成为可能，
也许还能结合分离逻辑中的空间同步
