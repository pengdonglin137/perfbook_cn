...
还是有点令人惊讶——该测试有两个线程向两个不同的变量进行存储，
另外两个线程以相反的顺序从这两个变量加载。
如果两个加载线程对两个存储操作的顺序不能达成一致，
则 assertion 触发。
这个 litmus test 是简单的，即使按照 memory-order litmus test 的标准也是如此。

PPCMEM 消耗大量时间和空间的一个原因是它进行了
基于跟踪的完整 state-space search，这意味着它必须生成
和评估在架构级别上事件和动作的所有可能顺序和组合。
对应于大量事件和动作序列的存储和加载，
导致必须被完整搜索的非常大的 state space，
进而导致大量的内存和 CPU 消耗。

当然，许多跟踪事件彼此之间是类似的，这表明
将类似跟踪视为一个进行处理的方法可能会
提高性能。
