...
这种情况类似于 \cref{sec:together:Correlated Fields} 中的问题：
存在一个 hash table，我们需要两个或更多数据元素的关联视图。
这些数据元素被同时更新，
我们不希望看到第一个元素的旧版本，
而看到其他元素的新版本。
例如，Schr\"odinger 决定将其大家庭连同所有动物
添加到其内存数据库中。
Schr\"odinger 也是一个传统主义者，
他认为结婚和离婚这样的事情不会随时发生。
因此，他绝不希望在数据库中出现这样的情况：
新娘已经结婚了，而新郎却没有，反之亦然。
另外，如果你觉得 Schr\"odinger 是传统主义者，
试试和他的一些家庭成员交谈！
换句话说，Schr\"odinger 希望在数据库中
要保持婚姻状态一致。

一种方法是使用 sequence lock
（参见 \cref{sec:defer:Sequence Locks}），
这样婚姻相关的更新将在
\co{write_seqlock()} 的保护下进行，
而要求婚姻状况一致性的读请求将在
\co{read_seqbegin()} / \co{read_seqretry()} 循环体之中进行。
请注意，sequence lock 并不是 RCU 保护机制的替代品：
sequence lock 是保护并发的修改操作，
而 RCU 仍然是需要的，它保护并发的删除。

当关联元素数量少、读取这些元素的时间短
且更新速率低时，这种方法可以运行得很好。
否则，更新可能会频繁发生，以至于读者总是不能完成。
虽然 Schr\"odinger 不期望即使他最不理智的亲戚
也会快速到足以使结婚和离婚成为问题，
但他确实意识到这个问题很有可能在其他情况下出现。
避免读者饥饿（starvation）问题的一种方法是
让读者在重试次数过多时使用更新侧原语，
但这会同时降低性能和可扩展性。
...
此外，如果更新侧原语使用得太频繁，
由于锁竞争（lock contention）的原因将带来性能和可扩展性的问题。
避免此问题的一种方法是维护每元素 sequence lock，
并在更新它们的婚姻状态时应当持有双方配偶的锁。
读者可以在任一配偶的锁之下进行重试循环，
以获得对涉及该对成员的婚姻状态变化的稳定视图。
这避免了由于高频率的结婚和离婚所带来的竞争，
但复杂性在于：在单次扫描数据库期间
需要获取所有婚姻状态的稳定视图。
