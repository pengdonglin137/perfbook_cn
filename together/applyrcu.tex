...
设计目的是使用 RCU 而非 \co{final_mutex} 来保护
\co{read_count()} 中的线程遍历，
以获得良好的性能和可扩展性，而不仅仅是保护
\co{inc_count()}。
但是，我们并不希望放弃求和计算的精确性。
特别是，当给定线程退出时，
我们绝不能丢失退出线程的计数，
也不能重复对它进行计数。
这样的错误将导致不精确的结果等于完全精确的结果，
换句话说，这样的错误将使得结果完全无用。
并且事实上，\co{final_mutex} 的一个目的是，
确保线程不会在 \co{read_count()} 运行过程中进入并退出。

因此，如果我们不用 \co{final_mutex}，就必须拿出其他
确保一致性的方法。
其中一种方法是将所有已经退出线程的计数和，
以及指向每线程计数器的指针放到一个单一数据结构中。
这样的数据结构一旦对 \co{read_count()} 可用，
就保持不变，以确保 \co{read_count()} 看到一致性的数据。
...
\co{->total} 字段用于对之前已经退出线程的计数，
以及一个 \co{counterp[]} 指针数组指向每个当前正在
运行的每线程 \co{counter}。
此结构允许 \co{read_count()} 的给定执行过程
看到一致的计数总和以及运行线程的集合。
...
以及 \co{final_mutex} 自旋锁（spinlock）。
...
至少在 \clnref{rru} 处当前 RCU 读侧临界区（critical section）结束之前
不会被释放。
\Clnref{init} 将 \co{sum} 初始化为 \co{cap->total}，
即之前已经退出的线程的计数值之和。
\Clnrefrange{add:b}{add:e} 将当前正在运行的线程对应的
每线程计数值添加到 \co{sum} 中，
最后，\clnref{ret} 返回 \co{sum}。
\end{fcvref}
...
这样任何可能正在 \co{read_count()} 中并发执行、
并且可能持有对旧 \co{countarray} 结构引用的线程，
都能退出其 RCU 读侧临界区，从而放弃对该结构的引用。
...
对 RCU 的使用，使得正在退出的线程能够等待，
直到其他线程保证已经结束了对退出线程的
\co{__thread} 变量的使用。
这允许 \co{read_count()} 函数免于使用锁，
从而为 \co{inc_count()} 和 \co{read_count()} 函数
提供了优良的性能和可扩展性。
但是，这些性能和可扩展性来自于代码复杂性的增加。
希望编译器和库的编写者使用用户级
...
虚构代码片段。
由于需要获取读写锁，这些代码片段在快速路径
（启动 I/O）上招致了过高的负载。

本节展示如何使用 RCU 来避免此开销。

执行 I/O 的代码与原来的版本非常相似，
它使用 RCU 读侧临界区替代了原来版本中的
读写锁读侧临界区：
...
由于 \co{synchronize_rcu()} 的缘故，
一旦到达 \clnref{nextofsync}，
我们就能够知道所有剩余的 I/O 已经被识别到了。

当然，\co{synchronize_rcu()} 的开销可能比较大，
但鉴于移除设备这种情况比较少，这通常是一个不错的权衡。
\end{fcvref}
...
数组 \co{->a[]} 的长度可以动态变化，
在任意时刻，其长度由字段 \co{->length} 表示。
当然，这带来了以下竞争条件（\IX{race condition}）：
...
一种方法是小心使用内存屏障（memory barrier），
内存屏障的内容在 \cref{chp:Advanced Synchronization: Memory Ordering} 中介绍。
这个方法可行，但是带来了读侧开销，
也许更糟糕的是需要显式使用内存屏障。
...
一个更好的方法是将值和数组放入同一结构中，
如 \cref{lst:together:Improved RCU-Protected Variable-Length Array}~\cite{Arcangeli03}
所示。
分配一个新数组（\co{foo_a} 结构），就自动为数组长度
提供了一个新的存储空间。
这意味着如果某个 CPU 获取了 \co{->fa} 的引用，
就能够确保 \co{->length} 与 \co{->a[]} 的长度相匹配。
...
\item	CPU~0 从 \co{->a[]} 获取新指针，
	并在第 12 个元素中存储新值。
	但因为 CPU~0 仍然引用旧的 \co{foo_a} 结构，
	该结构包含 16 字节的数组，一切正常。
\end{enumerate}

当然，在所有情况下，CPU~1 都必须在释放旧数组之前
等待一个宽限期。
...
读者从单次完整更新的角度看到这三个值，
这一点至关重要：
如果读者看到 \co{meas_1} 的旧值，
而看到 \co{meas_2} 和 \co{meas_3} 的新值，
读者将会变得非常迷惑。
我们怎么才能确保读者看到这三个值的协调集合？\footnote{
	这种情况类似于
...
然后通过更新指针的方式，用新结构替换旧结构。
这确实保证所有读者看到测量值的一致集合，
但由于 \co{->photo[]} 字段的原因，需要复制一个大型结构。
这样的复制可能带来不可接受的高开销。
...
将测量值填充到该结构中，
并使用 \co{rcu_assign_pointer()}
更新 \co{animal} 结构的 \co{->mp} 字段使其指向这个新的
\co{measurement} 结构。
在一个宽限期过去后，旧的 \co{measurement} 结构可以被释放。
...
由于在统计扫描进行期间，动物被添加到
hash table 或从中删除而引起的轻微误差。
Schr\"odinger 应当怎么来控制并发性？

一种方法是将统计扫描置于 RCU 读侧临界区之内。
这允许更新并发地进行，而不影响扫描过程。
特别是，扫描过程不会阻塞更新，反之亦然，
这允许即使面对高频率的更新，
也能对包含大量数据元素的 hash table 进行优雅的扫描。
...
\begin{enumerate*}[(1)]
\item	使用 SRCU 代替 RCU，
\item	在退出 RCU 读者之前获取引用（可能是每 CPU 的），或者
\item	在退出 RCU 读者之前获取 hazard pointer。
\end{enumerate*}

\QuickQuiz{
	但如果无论如何都要使用 hazard pointer，为什么不直接
	到处使用它们而完全放弃 RCU 呢？
}\QuickQuizAnswer{
	在许多情况下，完全切换到 hazard pointer 是非常合理的。

	然而，请记住 hazard pointer 不能在所有情况下无条件地
	遍历受 hazard 保护的指针。
	在这种情况下，完全放弃 RCU 可能会大大增加
	遍历需要长期保护的数据元素的链式数据结构的复杂性。
	最佳策略是使用 RCU 遍历链式数据结构，
	并使用 hazard pointer 保护该遍历末端的数据元素。

	与其他工作~\cite{JeehoonKang2020MarryHazptrRCU}不同，
	这种将 hazard pointer 和 RCU 结合的方法利用了
	两种机制的优势。
	只是要非常小心地在退出 RCU 读侧临界区 \emph{之前}
	获取 hazard pointer！
}\QuickQuizEnd
...
\subsubsection{获取 Hazard-Pointer 引用}
\label{sec:together:Acquire a Hazard-Pointer Reference}

不过，\co{refcount_inc_not_zero()} 等原子操作有时会导致
过多的内存竞争。
在这种情况下，另一种修复方法是使用 hazard pointer
而非显式引用计数。
...
因为 Linux 内核内存模型（memory model）不支持动态分配，
此代码通过存储值零来模拟释放。
\end{fcvref}
...
在大约十年的时间里，Linux 内核的 RCU 逐渐增加了轮询
grace period 的 API。
本节概述这些 API 的使用场景，重点介绍 RCU\@。
本节内容同样适用于 SRCU 的
\apik{get_state_synchronize_srcu()}、\apik{start_poll_synchronize_srcu()}
和 \apik{poll_state_synchronize_srcu()} API。

\subsubsection{检测中断的读者}
\label{sec:together:Detecting Broken Readers}

任何基于作用域的同步机制（无论是加锁、中断禁止还是 RCU）面临的
挑战之一是，被调用的函数可能临时退出临界区。
这种做法被认为是不好的编程习惯，但有时为了
避免死锁、RCU CPU stall 警告，或者因为
在临界区停留太久而引发其他后果，不得不这样做。
只不过这种临时退出对于调用者来说可能是致命的意外。
...
\Clnref{rrl} 进入 RCU 读侧临界区，\clnref{rrul}
退出临界区。
\Clnref{rd} 将全局指针 \co{gp} 加载到局部变量 \co{p} 中，
准备对其解引用。
\Clnref{sf} 调用一个可能（错误地）
临时退出临界区的函数。
\Clnref{dsw} 将 \co{p} 传递给 \co{do_something_with()}，该函数可能
解引用此指针，如果 \co{suspicious_function()}
已经退出了临界区（哪怕只是短暂退出），这就会失败~\cite{LorenzoStoakes2025ReaderLeak,VlastimilBabka2025PollReaderLeak}。
...
轮询 RCU grace period 至少提供了检测此类
致命意外的可能性，如
\cref{lst:together:Polling For Broken RCU Readers} 所示。\footnote{
	感谢 \ppl{Lorenzo}{Stoakes} 和 \ppl{Vlastimil}{Babka}
	提出这一方法。}
\apik{get_state_synchronize_rcu()} 和 \apik{poll_state_synchronize_rcu()}
函数可用于检测临界区的临时退出，
如 \clnref{gssr,pssr} 所示。\footnote{
	感谢 Vlastimil Babka 发明此技术。}
\Clnref{gssr} 调用 \co{get_state_synchronize_rcu()}，将其
返回值保存在 \co{oldstate} 中。
\Clnref{pssr} 将 \co{oldstate} 传递给
\co{poll_state_synchronize_rcu()}，该函数仅在
自调用 \co{get_state_synchronize_rcu()} 以来已经过了
一个完整的 RCU grace period 时才返回 \co{true}。
假设 RCU 实现正常，返回 \co{true} 只可能是
\co{suspicious_function()} 退出了临界区，
因此使用了 \apik{WARN_ON_ONCE()}。
\end{fcvref}

不幸的是，RCU grace period 持续时间长达数毫秒，
而 \co{suspicious_function()} 可能仅退出临界区
几微秒。
这意味着 \co{WARN_ON_ONCE()} 触发的概率
非常小。
可以通过使用 \co{CONFIG_RCU_STRICT_GRACE_PERIOD=y} 编译 Linux 内核来
提高这一概率，该选项加速了 RCU grace period，
但将 CPU 数量限制为四个。

还可以通过以 \co{rcupdate.rcu_expedited=1} 启动
Linux 内核来加速同步 RCU grace period。
此外，内核地址消毒器（KASAN）可以帮助定位
use-after-free bug，包括由临时退出
RCU 读侧临界区引发的那些。

\subsubsection{抑制回收}
\label{sec:together:Restraining Reclamation}

RCU 实现在自己选择的时间调用回调\footnote{
	来自 Linux 内核的 \co{call_rcu()} 或 C++26 的 \co{rcu_retire()}}，
这个时间未必方便当前工作负载。
这对于实时工作负载尤其烦人，因为在这些场景中，
在高优先级线程唤醒后立即恢复它
通常比释放少量内存紧急得多。

在 Linux 内核中，这种情况通常可以通过回调卸载来处理，
使用 \co{CONFIG_RCU_NOCB_CPU=y} 编译内核并在
\co{rcu_nocbs} 内核引导参数中设置需要卸载的 CPU 的位。
系统管理员可以为生成的 \co{rcuo} 内核线程设置优先级和 CPU 绑定，
从而防止这些线程干扰更高优先级的线程或运行在其他 CPU 上的线程。\footnote{
	更多信息参见 \url{https://lwn.net/Articles/988641/}}。
这可行，但需要全局管理 CPU 和优先级，
随着越来越多的工作负载整合到单个系统上，这变得越来越困难。
此外，每个 RCU 回调在禁用 softirq（也称为"下半部"）的情况下调用，
这意味着即使最高优先级的线程也无法抢占
给定的回调，无论该线程运行在内核空间还是用户空间。

另一种方法是内核用户使用
\apik{get_state_synchronize_rcu()} 或 \apik{start_poll_synchronize_rcu()} 的
返回值标记每个数据元素（或它们的组），
然后在合适的时间定期调用 \apik{poll_state_synchronize_rcu()} 来释放内存。
或者，可以使用这三个 API 的 \co{_free()} 对应版本，
它们消耗更多内存，但能更准确地处理
普通和加速的 RCU grace period。

\subsubsection{主动回收}
\label{sec:together:Proactive Reclamation}

传统的 RCU grace period 等待 API 之间存在尖锐的权衡。
一方面，\apik{synchronize_rcu()} 的存储开销为零，
但另一方面它给调用者带来数毫秒的延迟。
虽然 \apik{synchronize_rcu_expedited()} 给调用者带来的延迟
比 \co{synchronize_rcu()} 小，但在小型系统上仍有
数十微秒，在大型系统上则超过一毫秒。
至于 \apik{call_rcu()} 和 \apik{kfree_rcu()}，它们避免了这种
延迟，但代价是给每个 RCU 保护的数据结构
增加一个 \co{rcu_head} 指针。
最后，\apik{kfree_rcu_mightsleep()} 避免了 \co{rcu_head} 的存储
开销，也避免了数毫秒的延迟，但前提是
有足够的空闲内存。
当没有空闲内存时，\co{kfree_rcu_mightsleep()} 给调用者带来的延迟
与 \co{synchronize_rcu()} 一样大。

想要两全其美的人当然会主张一种
既不产生延迟、又不需要给 RCU 保护的数据元素
添加烦人的 \co{rcu_head} 结构的方案。

通过构造一个单向链表来存放最近从 RCU 保护的链式数据结构中
移除的数据元素，然后定期将整个链表传递给 \co{call_rcu()}，
确实可以实现最小延迟的同时将每个数据元素的开销
（几乎）降低到一个指针。
然而，这意味着单个 RCU 回调将遍历并
释放该链表的每个元素，从而进一步增加
调用回调对工作负载造成的干扰。
可以通过使用 \apik{queue_rcu_work()} 来降低干扰的优先级（但不减少其持续时间），
它在一个启用了抢占的 workqueue 处理函数中调用回调。
不过这可能导致每次回调都发生上下文切换，而且
即使忽略这一点，回调也是在 RCU 和
workqueue 子系统选择的时间被调用的。
...
这种情况看起来不太乐观，但在某些使用场景中，
RCU 保护的数据元素被链接到一个搜索结构中，
该结构缓存了此类结构的一部分。
这由 \cref{fig:together:Polled RCU Cache} 中最左边的"可被读者访问"框表示。

一段时间未使用的数据元素可能会从读者可访问的缓存中
移除并转移到一个暂存区域（参见标记为"A"的箭头），
该区域由图中的"旧读者访问"框表示。
这些元素可以用轮询 grace period API 成员之一的
返回值来标记，例如
\co{get_state_synchronize_rcu()}。
由于能够表示待处理 RCU grace period 的轮询值数量有限，
这个暂存区可以包含一个小数组，
每个元素保存一个 grace period 的 \co{oldstate} 值。
元素数量由 \apik{NUM_ACTIVE_RCU_POLL_OLDSTATE} 和
\apik{NUM_ACTIVE_RCU_POLL_OLDSTATE_FULL} 宏指定。
该数组的每个条目都作为链表头，链接那些已从
最左边的"可被读者访问"列表中移除、但仍可能被旧读者引用的元素。
如果读者需要已被降级到"旧读者访问"区域的数据元素，
它可以使该元素再次对读者可访问，
如图顶部的箭头所示。

当某个轮询 API（例如 \co{poll_state_synchronize_rcu()}）
指示对应的 RCU grace period 已经过期时，就\emph{不再有}
读者访问这些数据。
然后这些数据元素被移动到"无读者访问"区域，
如标记为"B"的箭头所示。
它们仍然可以被重新设为读者可访问，
同样如图顶部的箭头所示。
...
这种方法几乎不需要额外的每数据元素存储开销，
对调用函数几乎没有延迟，并且完全控制回收的"何时"和"如何"。

\subsubsection{培育非阻塞同步}
\label{sec:together:Nurturing Non-Blocking Synchronization}

如上一节所述，\apik{synchronize_rcu()} 和
\apik{synchronize_rcu_expedited()} 都会给调用者
带来显著的延迟。
虽然 \apik{call_rcu()} 和 \apik{kfree_rcu()} 在常见情况下
几乎不产生延迟，但在 Linux 内核中 \co{call_rcu()} 可能
产生唤醒延迟，\co{kfree_rcu()} 可能产生
内存分配延迟，两者都涉及获取内核锁。
对于低延迟应用来说，这些延迟中的任何一个都可能是不可接受的，
而这些应用通常选择使用非阻塞同步。

这些应用的内核部分可以使用本节前面提到的轮询 API，
其中 \apik{get_state_synchronize_rcu()}、
\apik{get_state_synchronize_rcu_full()}、
\apik{poll_state_synchronize_rcu()} 和
\apik{poll_state_synchronize_rcu_full()}
是无等待的。
这些 API 都不会实际启动 grace period，
但是，可以在某个对延迟不敏感的时间或地点
调用传统的 grace period 等待 API，以及
\apik{start_poll_synchronize_rcu()} 和
\apik{start_poll_synchronize_rcu_full()}，
来启动所需的 grace period。
