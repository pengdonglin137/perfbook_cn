...
访问数据的效率是如此重要，以至于对算法的讨论也包括相关数据结构的
时间复杂度~\cite{ThomasHCorman2001Algorithms}。
然而，对于并行程序，时间复杂度的度量还必须包括并发效应。
如\cref{chp:Hardware and its Habits}所示，这些效应可能是压倒性的因素，
这意味着对并发数据结构的设计，必须要像关注串行数据结构的时间复杂度那样关注并发复杂度。
...
Schr\"odinger 拥有一家动物园，里面有大量的动物，他想使用内存数据库
来记录它们。动物园中的每只动物在数据库中都有一个条目。
每只动物都有一个唯一的名字作为 key，同时还有与每只动物有关的各种数据。

出生、捕获和购买将导致数据插入，而死亡、释放和出售导致数据删除。
因为 Schr\"odinger 的动物园包含大量短命的动物，包括老鼠和昆虫，
所以数据库必须能够支持高频更新请求。
对 Schr\"odinger 的动物感兴趣的人可以查询它们，但是，
Schr\"odinger 已经注意到他的猫拥有非常高的查询率，
多到以至于他怀疑他的老鼠们可能正在使用数据库来检查它们的天敌。
这意味着 Schr\"odinger 的应用必须支持对单个数据条目的高频查询请求。

请记住这个应用，因为接下来将涉及各种数据结构。
...
如今的计算机世界使用了各种各样的数据结构，市面上讲数据结构的教科书多如牛毛。
本节专注于一种数据结构，即哈希表。
聚焦于哈希表使我们能够更深入地研究并发与数据结构的交互方式，
同时让我们熟悉这种在实践中被大量使用的数据结构。
...
另外，每个 bucket 都有自己的锁，因此哈希表中不同 bucket 的元素可以
完全独立地进行插入、删除和查找。
因此，包含大量元素的哈希表提供了极好的可扩展性。

这种方法解决了\cref{sec:intro:Existing Sequential Designs}中指出的元素计数问题——
不让 \co{hashtab_add()} 或 \co{hashtab_del()} 返回哈希表中的元素数量。
它还通过允许重复元素以及将指向待删除元素的指针传递给 \co{hashtab_del()} 来进一步
简化 API。
这两项更改消除了返回错误的可能性，
\co{hashtab_add()} 和 \co{hashtab_del()} 的返回类型因此可以是 \co{void}。
...
(\path{hash_bkt.c})
展示了一组用于简单固定大小哈希表的数据结构，
该哈希表使用链接法和每 hash bucket 加锁的方式，
...
该宏使用简单的取模运算：
如果需要更好的 hash 函数，调用者需要自行实现从 key 到 hash 值的映射。
...
如果该元素不存在则返回 \co{NULL}。
该函数同时接受 hash 值和指向 key 的指针，这使得调用者可以使用任意 key 和
任意 hash 函数。
...
\end{fcvref}
\co{hashtab_del()} 函数简单地从 hash chain 中移除指定的元素，
因为是双向链表所以这很容易。
...
随着 CPU 数量增加，情况只会变得更糟，如
\cref{fig:datastruct:Read-Only Hash-Table Performance For Schroedinger's Zoo; 448 CPUs}所示。
我们甚至不需要额外的线来表示理想性能：
29 个 CPU 及以上的性能非常糟糕。
这清楚地警示了从中等数量的 CPU 外推性能的危险。

当然，性能大幅下降的一个可能原因是 hash bucket 数量不足。
这一点很容易通过增加 hash bucket 数量来验证。
...
所示，虽然增加 bucket 数量后性能确实有所提高，可扩展性仍然很差。
特别是，我们仍然看到 29 个 CPU 及以上时性能的急剧下降，
这清楚地证明了
\cpageref{sec:datastruct:NUMA-induced failure to scale}上复杂情况列表中
\cref{sec:datastruct:NUMA-induced failure to scale}提出的问题。
显然还有别的东西在捣鬼。

其实这是多 socket 系统惹的祸，CPU~0--27
和~224--251 会映射到第一个 socket，如
\cref{fig:datastruct:NUMA Topology of System Under Test}所示。
因此测试程序只用前 28 个 CPU 时性能相当好，
但测试涉及 socket~0 的 CPU~0--27 以及
socket~1 的 CPU~28 时，会产生跨 socket 边界传输数据的开销。
正如\cref{sec:cpu:Hardware System Architecture}中所讨论的，
这可能会严重降低性能。
总之，对于大型多 socket 系统来说，除了数据结构完全分区之外，还需要良好的引用局部性。
本章剩余部分将讨论在哈希表本身内部提供良好引用局部性的方法，
但请注意，另一种提供良好引用局部性的方式是在哈希表元素中放置大型数据。
例如，Schr\"odinger 可以通过在每个哈希表元素中放入动物的照片甚至视频来获得出色的 cache 局部性。
但如果你的哈希表需要包含小数据元素，请继续阅读！
...
展示了 RCU 保护的每 bucket 加锁哈希表的 \co{hashtab_lookup()}。
...
非 RCU 哈希表的对应函数非常相似。
\co{hashtab_add()} 使用 \apiur{cds_list_add_rcu()} 而非
\apiur{cds_list_add()}，以确保在向哈希表添加元素的同时
有人查找时的正确排序。
\co{hashtab_del()} 使用 \apiur{cds_list_del_rcu()} 而非
\apiur{cds_list_del_init()}，以处理元素在被删除之前刚好被查找的情况。
与 \co{cds_list_del_init()} 不同，\co{cds_list_del_rcu()} 保留前向指针不变，
这样 \co{hashtab_lookup()} 就能遍历到新删除元素的后继者。
...
但具有无锁（lock-free）RCU 保护查找的哈希表确实需要尽早对验证给予特别关注。
...
展示了 RCU 保护和 hazard pointer 保护的哈希表与上一节的每 bucket 加锁实现的
只读性能对比。
如你所见，RCU 和 hazard pointer 的性能和可扩展性都远优于每 bucket 加锁，
因为只读复制避免了 NUMA 效应。
差异随线程数量增加而增大。
还展示了全局加锁实现的结果，不出所料，
其表现甚至比每 bucket 加锁实现更差。
RCU 略优于 hazard pointer。
...
但为什么 RCU 的性能比理想值低五倍？
一个可能的原因是 \co{rcu_read_lock()} 和 \co{rcu_read_unlock()} 的每线程计数器
拖慢了速度。
...
添加了完全无同步的结果——因为这是只读基准测试，没有需要同步的内容，所以这是可行的。
...
在 RCU 和 QSBR 运行中每个哈希表元素（\co{struct zoo_he}）占用 72 字节。
...
通过检查无同步代码可以找到更多内存系统瓶颈的证据。
该代码不需要锁，因此每个 hash bucket 只占 16 字节，
而 RCU 和 QSBR 需要 56 字节。
类似地，每个哈希表元素只占 56 字节，
而 RCU 和 QSBR 需要 72 字节。
因此，单 CPU 无同步运行的性能比 QSBR 或 RCU 快约一半并不令人意外。
...
\end{figure}
	...
	中的双对数图，它展示了 448 个 CPU 下性能随哈希表大小
	（即 bucket 数量和最大元素数量）的变化。
	大小为 1,024 的哈希表有 1,024 个 bucket，
	最多包含 1,024 个元素，平均占用为 512 个元素。
	因为这是只读基准测试，实际占用始终等于平均占用。
...
这并不令人惊讶，因为如果所有 CPU 只做查找猫的操作，
猫所在 bucket 对应的锁实际上就是一把全局锁。
...
在所有 448 个 CPU 都在更新时降至零。
Hazard pointer 和 RCU 优于每 bucket 加锁，
因为它们的读取者不会增加更新侧的锁竞争。
随着更新者数量增加，RCU 相对于 hazard pointer 表现更好，
这是因为后者的读侧 memory barrier 产生了更大的开销，
尤其是在有更新且涉及多个 socket 时。
...
Hazard pointer 和 RCU 一开始具有显著优势——与 bucket 加锁不同，
读取者不会排斥更新者。
然而，随着更新 CPU 数量增加，更新侧的延迟执行开销开始显现，
先是 RCU，然后是 hazard pointer。
当然，这三种实现都远优于全局加锁，差距超过一个数量级。
...
虽然这种不安全的 hazard pointer 实现对基准测试通常够用，
但绝对不建议在生产环境中使用。
...
\QuickQuiz{
	...
	理论上不会更安全，在更大的系统上运行这些程序将是一个有益的练习。
...
一对并发读取者可能对猫的状态产生分歧。
例如，一个读取者可能在猫的数据结构被移除之前获取了指向它的指针，
而另一个读取者可能在之后才获取这个指针。
前者会认为猫还活着，后者则会认为猫已经死了。
...
简而言之，内部一致性并非所有问题领域的必要组成部分，
追求它往往需要在性能、可扩展性、与外部世界的一致性
...
固定大小的哈希表是完全可分区的，但可调整大小的哈希表在增长或缩小时
会带来分区挑战，如\cref{fig:datastruct:Partitioning Problems}所形象地描绘的那样。
然而，事实证明可以构建高性能可扩展的 RCU 保护哈希表，
如以下各节所述。
...
与 2000 年代初的情况形成鲜明对比的是，现在有几种不同类型的
可扩展 RCU 保护哈希表。
...
第一种哈希表实现背后的关键洞察是，
每个数据元素可以有两组链表指针，一组当前正被 RCU 读取者使用
（以及非 RCU 更新者），另一组用于构建新的调整大小后的哈希表。
...
分配一个四 bucket 数组，使用一索引链接将元素串入这四个新 bucket。
...
在此状态下，所有读取者都在使用新的四 bucket 数组，
旧的两 bucket 数组因此可以被释放，产生状态~(d)，
...
如果我们使用传统的基于锁或原子操作（atomic operation）的实现，
...
除非正在进行调整大小操作，
此时它表示其元素正在被插入新哈希表的 bucket 索引，
新哈希表由 \clnref{ht:new} 上的 \co{->ht_new} 字段引用。
...
一旦所有旧读取者完成，旧哈希表的 \co{ht} 结构就可以被释放。

\clnref{ht:idx} 上的 \co{->ht_idx} 字段表示哈希表的这个实例
正在使用两组链表指针中的哪一组，
...
\co{ht_bucket} 结构，所以需要持有两把锁，分别对应两个 bucket。
...
\co{hashtab_unlock_mod()} 函数释放 \co{hashtab_lock_mod()} 获取的锁。
\Clnref{relbkt0} 释放旧 \co{ht_bucket} 结构上的锁。
如果 \clnref{ifbkt1} 确定正在进行调整大小操作（这不太可能），
\clnref{relbkt1} 释放新 \co{ht_bucket} 结构上的锁。
无论哪种情况，\clnref{rcu_unlock} 退出 RCU 读侧临界区。
\end{fcvref}

\QuickQuiz{
	假设一个线程在调整大小操作期间正在向哈希表插入元素。
	是什么防止这个插入因后续的调整大小操作在插入完成之前完成而丢失？
}\QuickQuizAnswer{
	第二个调整大小操作将无法越过插入正在进行的 bucket，
	因为插入操作持有哈希表中一个或两个 hash bucket 上的锁。
...
\QuickQuiz{
	...
	这不意味着读取者可能会遗漏在调整大小操作期间添加的元素吗？
}\QuickQuizAnswer{
	不会。
	我们很快会看到，
	\co{hashtab_add()} 和 \co{hashtab_del()} 函数
	在调整大小操作进行期间保持旧哈希表是最新的。
}\QuickQuizEnd
...
如果 \clnref{ifnew} 确定此 bucket 已被分发到新版本的哈希表，
...
否则，\clnref{get_curtbl} 获取当前哈希表的引用，
\clnrefrange{alloc:b}{alloc:e} 分配所需大小的新哈希表。
...
\QuickQuiz{
	...
	确保在我们在 \clnref{set_newtbl} 上安装新哈希表引用
...
然后 \clnref{rel_master} 释放调整大小序列化锁，
\clnref{free} 释放旧哈希表，
...
将可调整大小的哈希表与其固定大小的对应物进行了比较，
分别在哈希表中使用 262,144 和 2,097,152 个元素。
该图为每个元素数量显示了三条曲线，一条用于固定大小的 262,144 bucket 哈希表，
另一条用于固定大小的 524,288 bucket 哈希表，
第三条用于在 262,144 和 524,288 bucket 之间来回调整的可调整大小哈希表，
每次调整大小操作之间有一毫秒的暂停。

最上面的三条曲线是 262,144 元素哈希表的。\footnote{
	你只看到两条曲线？
	虚线那条实际上由两条差异很小的曲线组成，
	因此虚线图案看起来不规则。}
虚线曲线对应两个固定大小哈希表，
...
下面三条曲线是 2,097,152 元素哈希表的。
上面的虚线曲线对应 262,144 bucket 的固定大小哈希表，
中间的实线曲线（低 CPU 数量时在中间，高 CPU 数量时在底部）
对应可调整大小哈希表，
另一条曲线对应 524,288 bucket 的固定大小哈希表。
现在每个 bucket 平均有八个元素的事实只能预期会导致性能急剧下降，
实际上图中也显示了这一点。
更糟糕的是，哈希表元素占用 128\,MB，
...
\QuickQuiz{
	\cref{fig:datastruct:Overhead of Resizing Hash Tables Between 262;144 and 524;288 Buckets vs. Total Number of Elements}
	中大小哈希表之间的性能差异，有多少是由于长 hash chain，
	有多少是由于内存系统瓶颈？
}\QuickQuizAnswer{
...
\end{figure}
	...
	2,097,152 bucket 的哈希表无法放入 L3 cache。
	这条新曲线刚过 28 个 CPU 时上升，这也是可以预期的。
	这一上升是因为第 29 个 CPU 在另一个 socket 上，
	带来了额外的 39\,MB cache 以及额外的内存带宽。

	但大型哈希表相对于 524,288 bucket（但仍有 2,097,152 个元素）
	的哈希表的优势随着 CPU 增加而减少，
	这与瓶颈位于内存系统一致。
	在约 400 个 CPU 以上，2,097,152 bucket 的哈希表实际上
	略微不如 524,288 bucket 的哈希表。
	这不应令人惊讶，因为内存系统是瓶颈，
...
每核一个 CPU，这解释了可调整大小哈希表在 28 个 CPU 之后
性能急剧下降的原因。
尽管这种下降很急剧，请记住它是由于不断来回调整大小造成的。
显然一次性调整到 524,288 个 bucket 会更好，
或者更好的是，做一次八倍的调整到 2,097,152 个元素，
从而将每个 bucket 的平均元素数降低到上面三条曲线的运行所享有的水平。

从这些数据得出的关键点是，RCU 保护的可调整大小哈希表
...
鉴于这种可调整大小哈希表的性能和可扩展性
非常接近 RCU 保护的固定大小哈希表，
我们必须得出结论，这种方法相当成功。

最后，需要注意的是插入、删除和查找可以与调整大小操作并发进行。
这种并发性在调整大型哈希表大小时至关重要，
...
本节前面描述的可调整大小哈希表的一个缺点是内存消耗。
每个数据元素有两对 linked list 指针而不是一对。
是否有可能创建一个只需一对指针的 RCU 保护可调整大小哈希表？

事实证明答案是肯定的。
Josh Triplett 等人~\cite{Triplett:2011:RPHash}
开发了一种\emph{相对论哈希表}，它增量地分割和合并对应的 hash chain，
...
在这种情况下是将一个双 bucket 哈希表缩小为单 bucket 哈希表，
也称为线性链表。
这个过程通过将旧的较大哈希表中的成对 bucket 合并为
新的较小哈希表中的单个 bucket 来工作。
为使这个过程正确工作，我们显然需要约束两个表的 hash 函数。
一种约束是对两个表使用相同的底层 hash 函数，
但在从大缩小到小时丢弃低位 bit。
例如，旧的双 bucket 哈希表将使用值的前两个 bit，
而新的单 bucket 哈希表可以使用值的最高位 bit。
这样，旧的大哈希表中相邻的偶数和奇数 bucket 对
就可以合并为新的小哈希表中的单个 bucket，
...
我们从分配新的大型双 bucket 哈希表开始，产生状态~(b)。
...
简而言之，相对论哈希表以调整大小期间产生的额外 grace period 为代价，
减少了每个元素的链表指针数量。
这些额外的 grace period 通常不是问题，
因为插入、删除和查找可以与调整大小操作并发进行。

然而，当哈希表快速增长时，大量的 grace period 可能会
延迟下一次调整大小操作的开始。
...
哈希表中的数据元素被排列成一个单一的有序 linked list，
...
本章主要聚焦于哈希表，包括不完全可分区的可调整大小哈希表。
\Cref{sec:datastruct:Other Data Structures}简要概述了几种非哈希表数据结构。
尽管如此，这些对哈希表的阐述是了解围绕高性能可扩展数据访问的
许多问题的优秀入门，包括：
