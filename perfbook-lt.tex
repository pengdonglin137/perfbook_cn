...
% For arxiv.org (pdflatex only):
\ifdefined\pdfoutput\pdfoutput=1\fi

% To accomodate change in Ghostscript 9.26 (pdflatex only):
...
% XeLaTeX font setup with Chinese support
\usepackage{fontspec}
\usepackage{xeCJK}
\usepackage[table,svgnames]{xcolor}

% Set Western fonts (matching original newtxtext/tgheros/lmodern choices)
\setmainfont{TeX Gyre Termes}[Ligatures=TeX]
\setsansfont{TeX Gyre Heros}[Scale=0.9,Ligatures=TeX]
\setmonofont{Latin Modern Mono}[Scale=MatchLowercase]

% Set CJK fonts (Noto CJK Simplified Chinese)
\setCJKmainfont{Noto Serif CJK SC}
\setCJKsansfont{Noto Sans CJK SC}
\setCJKmonofont{Noto Sans Mono CJK SC}

% Chinese typesetting support (structural names, line breaking, etc.)
\usepackage[fontset=none,scheme=chinese,zihao=false,heading=false]{ctex}

% Improves the text layout (limited in XeLaTeX)
...
\newindex[API 索引]{api} % index for API
\newindex[人名索引]{ppl} % index for People Name
...
\usepackage[slantedGreek]{newtxmath} % math package
}
% Linux Biolinum for epigraphs (OpenType via fontspec)
\newfontfamily\biolinum{Linux Biolinum O}[Ligatures=TeX]
...
\IfCleverefTF{
  ...
  \crefname{sublisting}{代码清单}{代码清单}
  \crefname{sequencei}{步骤}{步骤}
  \Crefname{sequencei}{步骤}{步骤}
  \crefname{enumi}{项}{项}
  \Crefname{enumi}{项}{项}
  \crefname{page}{页}{页}
  ...
  % Define \crefthro{} for "第m.n至m.p节"
  \newcommand{\crefthro}[2]{%
    \namecrefs{#1}~\ref{#1}至~\ref{#2}%
  }
  \newcommand{\Crefthro}[2]{%
    \nameCrefs{#1}~\ref{#1}至~\ref{#2}%
  }
  ...
  \zcsetup{cap=true, nameinlink=false, lastsep={、}, nameinlink=false}
  ...
  \zcRefTypeSetup{listing}{
    Name-sg=代码清单, name-sg=代码清单,
    Name-pl=代码清单, name-pl=代码清单,
  }
  \zcRefTypeSetup{sublisting}{
    Name-sg=代码清单, name-sg=代码清单,
    Name-pl=代码清单, name-pl=代码清单,
  }
  \zcRefTypeSetup{subsubsubappendix}{
    Name-sg=附录, name-sg=附录,
    Name-pl=附录, name-pl=附录,
  }
  \zcRefTypeSetup{sequencei}{
    Name-sg=步骤, name-sg=步骤,
    Name-pl=步骤, name-pl=步骤,
  }
  ...
  \newcommand\crefthro[2]{\zcref[range,rangesep={至}]{#1,#2}}
  \newcommand\Crefthro[2]{\zcref[S,range,rangesep={至}]{#1,#2}}
  \newcommand\namecrefs[1]{\zcref[noref]{#1}}
}
...
\newcommand{\clnrefp}[2]{%
  ...
  \def\nextitem{\def\nextitem{、}}% Chinese comma separator
  \ifnum\value{lblcount}=1 第\lnref{#1}行%
  \else 第%
  \renewcommand*{\do}[1]{%
    \addtocounter{lblcount}{-1}%
    \ifnum\value{lblcount}=0 和\else\nextitem\fi\lnref{##1}}%
  \docsvlist{#1}行%
  \fi%
}
\newcommand{\clnrefpraw}[2]{%
  ...
  \docsvlist{#1}% Process list and count labels
  \def\nextitem{\def\nextitem{、}}% Chinese comma separator
  \ifnum\value{lblcount}=1 第\lnrefraw{#1}行%
  \else 第%
  \renewcommand*{\do}[1]{%
    \addtocounter{lblcount}{-1}%
    \ifnum\value{lblcount}=0 和\else\nextitem\fi\lnrefraw{##1}}%
  \docsvlist{#1}行%
  \fi%
}
\newcommand{\clnref}[1]{\clnrefp{#1}{}}
\newcommand{\Clnref}[1]{\clnrefp{#1}{}}
\newcommand{\clnrefr}[1]{\clnrefpraw{#1}{}}
\newcommand{\Clnrefr}[1]{\clnrefpraw{#1}{}}
\newcommand{\clnrefrange}[2]{第\lnref{#1}\==\lnref{#2}行}
\newcommand{\Clnrefrange}[2]{第\lnref{#1}\==\lnref{#2}行}
\newcommand{\clnrefthro}[2]{第\lnref{#1}至\lnref{#2}行}
\newcommand{\Clnrefthro}[2]{第\lnref{#1}至\lnref{#2}行}
\newcommand{\pararef}[1]{第~\pageref{#1}~页``\nameref{#1}''段落}
\newcommand{\Pararef}[1]{第~\pageref{#1}~页``\nameref{#1}''段落}
