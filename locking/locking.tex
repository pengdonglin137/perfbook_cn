...
\IX{unfairness}不公平、\IXpl{data race}数据竞争，以及各种其他并发"罪过"。
有趣的是，在生产级共享内存并行软件中
真正承担重任的角色同样是加锁。
...
这种杰基尔-海德（Jekyll-and-Hyde）式的二元对立源于以下几个原因：
...
\item	所有美好的故事都需要一个反派，而加锁在研究论文中
	充当出气筒（whipping boy）的历史可谓悠久而"光荣"。
\end{enumerate}
...
往往只是一个很小的代价。
更重要的是，这种潜在的死锁最好能在设计阶段就被发现，而不是在代码已经生成之后！
...
只要用户代码在处理每个列表元素时，不去获取已被其他
\co{list_start()}或\co{list_next()}调用持有的锁，
锁的细节就可以在用户代码中保持隐藏。
获取重复的锁会导致死锁。
