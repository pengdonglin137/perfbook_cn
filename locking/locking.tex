...
\Epigraph{加锁是最差的通用同步机制，
	  除了所有其他曾经被尝试过的机制。}{向Winston Churchill的记忆
	  以及他所引用的那个人致歉}

在近来对并行编程的研究中，加锁总是扮演着反派角色。
\IX{加锁}背负着诸多指控，包括引发死锁、护送效应（convoy）、\IX{starvation}饥饿、
\IX{unfairness}不公平、\IXpl{data race}数据竞争以及其他各种并发"罪过"。
有趣的是，在生产级共享内存并行软件中
真正承担重任的角色同样是加锁。
...
\item	加锁在某些软件制品上效果极佳，
	在另一些上又极差。
	那些在加锁效果良好的制品上工作的开发者，
	对加锁的态度往往比那些在效果不佳的制品上工作的开发者
	更加正面，
	这一点将在\cref{sec:locking:Locking: Hero or Villain?}中讨论。
\item	所有好的故事都需要一个反派，而加锁在研究论文中
	充当出气筒（whipping boy）的历史确实悠久而"光荣"。
\end{enumerate}
...
当一组线程中的每个线程都持有至少一个锁，同时又在等待
该组中另一个成员持有的另一个锁时，就会发生\IXB{Deadlock}。
即使在只包含单个线程的组中，当该线程试图获取它已经持有的
非递归锁时，这种情况也会发生。
因此，即使只有一个线程和一个锁，也可能发生死锁！

如果没有外界干预，死锁将永远持续。
没有线程能获取到它正在等待的锁，因为该锁被另一个线程持有，
而持有该锁的线程又在等待获取另一个锁，
在等待的线程释放那个锁之前，它无法释放自己持有的锁。
...
\QuickQuiz{
	但基于锁的死锁的定义只是说每个线程都持有
	至少一个锁并等待另一个被某个线程持有的锁。
	你怎么知道其中一定存在循环？
}\QuickQuizAnswer{
	假设图中没有循环。
	那么我们将得到一个有向无环图（DAG），它至少有一个叶节点。

	如果这个叶节点是一个锁，那么就意味着有一个线程在等待
	一个没有被任何线程持有的锁，这与定义矛盾。
	在这种情况下，线程将立即获取该锁。

	另一方面，如果这个叶节点是一个线程，那么就意味着
	有一个线程没有在等待任何锁，同样与定义矛盾。
	在这种情况下，线程要么正在运行，
	要么被阻塞在非锁的某物上。
	在第一种情况下，如果没有无限循环bug，
	线程最终会释放锁。
	在第二种情况下，如果没有唤醒失败的bug，
	线程最终会被唤醒并释放锁。\footnote{
		当然，一种唤醒失败的bug是涉及不仅是锁，
		还有非锁资源的死锁。
		但问题确实说的是"基于锁的死锁"！}
...
虽然有一些软件环境（如数据库系统）可以从已有的死锁中恢复，
但这种方法要么需要杀掉其中一个线程，要么强制从某个线程中夺取锁。
杀线程和强制夺取对于事务来说是可以的，
但是对内核和应用程序层面的加锁来说问题多多：
处理部分更新的数据结构极其复杂、危险且容易出错。

因此，内核和应用程序应该避免死锁，而非从死锁中恢复。
避免死锁的策略包括锁层次结构
...
虽然没有任何一种避免死锁的策略可以适用于所有情况，
但有很多工具可供选择。

\subsubsection{锁层次结构}
\label{sec:locking:Locking Hierarchies}

锁层次结构为锁逐个编号，禁止不按顺序获取锁。
在\cref{fig:locking:Deadlock Cycle}中，
我们可以用数字为锁编号，这样如果线程已经持有编号相同或者更高的锁，
就不允许再获取编号相同或者更低的锁。
线程~B违反了这个层次结构，因为它在持有Lock~4时
又试图获取Lock~3，因此导致了死锁的发生。

再次强调，应用锁层次结构时要为锁编号，严禁不按顺序获取锁。
...
然而，锁层次结构的全局性使得它很难应用于库函数。
毕竟，当使用某个库函数的应用程序还没有编写时，
可怜的库函数实现者又怎么才能遵循一个尚不存在的锁层次结构呢？

一种特殊情况——幸运的是这也是常见情况——是库函数并不调用任何调用者的代码。
这时，库函数持有的锁永远不会与调用者的锁同时被持有，
因此不可能出现同时包含库和调用者的锁的死锁循环。

\QuickQuiz{
	这条规则有没有例外情况？即使库代码从不调用
	...
	\item	如果库函数的参数之一是指向锁的指针，
		...
		那么就可能出现涉及调用者和库锁的
		死锁循环。
	\item	如果某个库函数返回指向锁的指针，
		调用者获取该锁，并且调用者在持有库的锁时
		获取其自身的锁，同样可能出现涉及
		调用者和库锁的死锁循环。
	\item	如果某个库函数获取了一个锁然后在仍然持有
		该锁的情况下返回，而调用者获取其自身的锁，
		又多了一种创建涉及调用者和库锁的
		死锁循环的方式。
...
传递给\co{qsort()}，而这个简单函数不获取任何锁。

现在，假如\co{qsort()}在持有Lock~C时调用\co{cmp()}，
这违背了之前提到的黄金规则"释放全部的锁"，如
\cref{fig:locking:Without qsort() Local Locking Hierarchy}所示，
那么死锁将会发生。
具体来说，假设一个线程调用\co{foo()}，
另一个线程同时调用了\co{bar()}。
第一个线程将获取Lock~A，第二个线程将获取Lock~B\@。
如果第一个线程调用\co{qsort()}时获取了Lock~C，那么它在调用
\co{cmp()}时将无法获取Lock~B。
但第一个线程持有Lock~C，所以第二个线程调用\co{qsort()}时
将无法获取Lock~C，因此也无法释放Lock~B，导致死锁。
...
如果每个模块都在调用未知代码之前释放全部的锁，
那么每个模块自身都避免了死锁，
整个系统也就避免了死锁。
因此，这条规则极大地简化了死锁分析，增强了代码的模块化。

然而，这条黄金规则也附带一个警告。
当你释放那些锁时，它们所保护的任何状态都可能发生任意变化，
而函数的调用者太容易忽略这些变化，导致微妙且难以重现的bug。
因为\co{qsort()}的比较函数很少获取锁，
让我们换一个不同的例子。
...
\end{enumerate*}
如果一个线程调用\co{tree_add()}而另一个线程释放树的锁
来运行回调函数，就可能遇到其中一些危险。
...
一个危险确实是子节点的数量可能变了也可能没变。
虽然这与\co{tree_add()}在迭代器启动之前或之后被调用
是一致的，但最好不要将其留给编译器的随意处理。
一个更严重的问题是\co{realloc()}可能无法原地扩展数组，
导致堆释放迭代器正在使用的那个数组并替换为另一块内存。
如果\co{children}指针没有被重新读取，那么迭代线程
将访问无效内存（要么已释放，要么已被回收）。
}\QuickQuizEnd

一种策略是确保即使锁被释放，状态仍然被保留，
例如，通过获取节点的引用来防止其被释放。
另一种策略是在回调函数返回后重新获取锁时重新初始化状态。
...
不幸的是，有时保留状态不可行，
重新初始化也不可行，
这就排除了在调用未知代码之前释放所有锁来构建局部锁层次结构的方案。
不过我们可以代之以构建一个分层锁层次结构，如
...
请注意，让\co{cmp()}使用新的Lock~D并不是简单的机械修改。
恰恰相反，这种改动通常需要在设计层面进行深入的修改。
然而，为了避免死锁而付出这些修改的努力
通常只是一个很小的代价。
更重要的是，这种潜在的死锁最好能在设计阶段就被发现，而不是在代码已经生成之后！

还有一个例子，也是无法在调用未知代码之前释放所有锁的情况。
请设想一个链表的迭代器，如
\cref{lst:locking:Concurrent List Iterator}（\path{locked_list.c}）所示。
\co{list_start()}函数获取链表的锁并返回第一个元素（假如链表不为空），
\co{list_next()}要么返回指向下一个元素的指针，
要么在到达链表末尾时释放锁并返回\co{NULL}。
...
\Clnref{start}对列表加锁并获取指向第一个元素的指针，
\clnref{entry}提供指向外层\co{list_ints}结构的指针，
\clnref{print}打印相应的整数，
\clnref{next}移动到下一个元素。
这个例子十分简单，并且隐藏了所有加锁的细节。
\end{fcvref}

只要处理每个列表元素的代码不去获取已被其他
\co{list_start()}或\co{list_next()}调用持有的锁，
加锁的细节就可以在用户代码中保持隐藏。
重复获取锁会导致死锁。
通过对列表迭代器的锁使用分层锁层次结构，就可以避免死锁。

这种分层方法可以扩展到任意多的层次，
但是每增加一层，加锁设计的复杂度也随之增加。
这种复杂度的增长对于某些类型的面向对象设计来说极不方便，
因为控制会在一大堆对象之间毫无章法地来回传递。\footnote{
	这种现象有个名字，叫做"面向对象的意大利面式代码"。}
面向对象设计的习惯与避免死锁的需求之间的这种不匹配，
...
避免死锁的一种方法是推迟获取冲突锁中的一个。
Linux内核的RCU就使用了这种方法，其\apik{call_rcu()}
函数由Linux内核调度器在持有其锁时调用。
这意味着\co{call_rcu()}并不总是能安全地调用调度器
来执行唤醒操作，例如，为了唤醒一个RCU内核线程以启动
由\co{call_rcu()}排队的回调所需的新宽限期。

\QuickQuiz{
	你说"并不总是能安全地调用调度器"是什么意思？
	...
	就不会持有任何调度器锁，\co{call_rcu()}
	可以安全地调用调度器。
	否则，如果中断被禁用，某个调度器锁\emph{可能}已被持有，
...
调用者的锁必须被持有直到移交完成，
但必须在函数返回之前释放。
POSIX的\apipx{pthread_cond_wait()}函数就是一个例子，
传递一个指向\apipx{pthread_mutex_t}的指针
可以防止因丢失唤醒而导致的挂起。
...
\QuickQuiz{
	...
	然后阻塞在\co{mutex_b}上，程序就会死锁。
}\QuickQuizEnd

简而言之，如果你发现自己设计的API需要将指向锁的指针作为参数或返回值，
请重新审视你的API设计。
这可能是正确的做法，但经验表明这种可能性很小。
...
数据包流是双向的。
当数据包从一层传往另一层时，有可能需要在两层中同时持有锁。
因为数据包可以从协议栈上层传往下层，也可能相反，
这简直是死锁的天然温床，
...
与无条件获取第1层的锁不同，\clnref{trylock}
使用\apik{spin_trylock()}有条件地获取锁。
该原语在锁可用时立即获取并返回非零值，
在锁不可用时不获取锁而返回零。
...
函数必须做出路由决策。
这个决策可能随时改变，特别是在移动系统中。\footnote{
	而且，与20世纪不同，移动性已成为常态。}
因此，\clnref{recheck}必须重新检查决策，如果决策已改变，
必须释放锁并重新开始。
\end{fcvref}
...
条件加锁有一个重要的特例：在执行任何处理之前，
先获取所有需要的锁，这些锁可以通过对涉及的数据结构地址
进行哈希来确定。
在这种情况下，处理不需要是幂等的：
如果发现在不释放某个已持有锁的情况下无法获取另一个锁，
那就释放所有锁重新来过。
只有在持有所有需要的锁之后才开始处理。
...
已经使用了很长时间，正是这种策略的一个例子。
两阶段加锁事务的第一阶段，只获取锁但不释放锁。
一旦所有需要的锁全部获取，事务进入第二阶段，
只释放锁但不获取锁。
这种加锁方法使得数据库可以对事务提供可串行化保证，
换句话说，保证所有事务看到和产生的数据与某个全局排序一致。
很多数据库系统都依赖中止事务的能力，不过通过在获取所有需要的锁之前
避免修改共享数据，也可以简化这一过程。
虽然使用两阶段加锁仍然会出现活锁和死锁，
但现有的大量数据库教科书中已经有了很多实用的解决办法。

\subsubsection{一次一个锁的设计}
\label{sec:locking:Single-Lock-at-a-Time Designs}

在某些情况下，可以避免嵌套加锁，从而避免死锁。
比如，如果问题可以完美分割，可以为每个分区分配一个锁。
处理任何特定分区的线程只需获取对应的那把锁。
因为没有任何线程在同一时刻持有一把以上的锁，死锁就不可能发生。

但必须要有某种机制来保证在没有持有锁的情况下
所需的数据结构仍然存在。
一种这样的机制在
\cref{sec:locking:Lock-Based Existence Guarantees}中讨论，
不过一个重要的限制条件是被处理的问题必须能够被分割。
...
然而，精心构造一种\emph{可以}在信号处理程序中使用的
加锁原语是可能的（虽然通常不太明智）。
此外，几乎所有操作系统内核都允许在中断处理程序中获取锁，
中断处理程序可以说是内核中信号处理程序的类似物。

其中的诀窍是：在任何可能在信号（或中断）处理程序中获取锁的时候，
阻塞信号（或禁用中断）。
不仅如此，如果已经持有这样的锁，那么在不阻塞信号的情况下
尝试获取任何可能在信号处理程序之外被持有的锁，
都是非法的。
...
\QuickQuiz{
	...
	因为这会导致死锁。
	鉴于Lock~A有时在信号处理程序之外、不阻塞信号的情况下被持有，
	在持有该锁时就可能处理一个信号。
	相应的信号处理程序可能随后获取Lock~B，
	这样Lock~B就是在持有Lock~A的情况下被获取的。
	因此，如果我们在持有Lock~B时也获取Lock~A，
	就会形成一个死锁循环。
	注意，即使在持有Lock~B时信号被阻塞，
	这个问题仍然存在。

	这是在中断或信号处理程序中获取锁时要非常小心的
	又一个原因。
	但Linux内核的锁依赖检查器能识别这种情况
	以及许多其他情况，所以请充分利用它！
}\QuickQuizEnd

如果一个锁被多个信号的处理程序获取，
那么每次获取该锁时都必须阻塞所有这些信号，
即使锁是在信号处理程序内部获取的也不例外。
...
不幸的是，在一些操作系统中阻塞和解除阻塞信号代价高昂，
Linux就是如此。所以出于性能考虑，
信号处理程序中获取的锁通常只在信号处理程序中获取，
应用程序代码和信号处理程序之间的通信通常使用无锁同步机制。

又或者，除非处理致命错误，否则完全避免使用信号处理程序。
...
\QuickQuiz{
	...
	在许多应用程序环境中，在信号处理程序中获取锁
	是不被推荐的~\cite{OpenGroup1997pthreads}。
...
对于基于共享内存的并行程序员来说，有大量避免死锁的策略可用，
但有些顺序程序并不适合其中任何一种策略。
这也是专家级程序员的工具箱里总有不止一样工具的原因之一：
加锁是处理并发的强大工具，但有些工作更适合用其他工具来完成。

\QuickQuiz{
	给定一个面向对象的应用程序，它在一组对象之间自由传递控制，
	使得不存在直接的锁层次结构，\footnote{
		也被称为"面向对象的意大利面式代码"。}
	无论是分层的还是其他形式的，这个应用程序如何并行化？
}\QuickQuizAnswer{
	有多种方法：
	\begin{enumerate}
	\item	在通过模拟进行参数搜索的情况下，
		即需要运行大量模拟以收敛到（例如）机械或电气设备的
...
\QuickQuiz{
	...
	所有需要的锁。
	这既避免了死锁又避免了活锁，并且在全局锁
	不需要被太频繁获取的情况下具有合理的可扩展性。
}\QuickQuizEnd

活锁可以被看成是饥饿的一种极端形式，
此时不是一个线程，而是一组线程都在挨饿。\footnote{
	不要过于纠结于活锁、饥饿和不公平
	等术语的精确定义。
	任何导致一组线程无法取得足够前进进度的情况
	都是需要修复的bug，争论名称并不能修复bug。}
...
活锁和饥饿在软件事务内存实现中都是严重问题，
因此引入了\emph{竞争管理器}的概念来封装这些问题。
以加锁为例，简单的指数退避通常就能解决活锁和饥饿。
指数退避是指在每次重试之前引入按指数增长的延迟，
...
不过，为了获得更好的效果，退避应该有个上限，
而且通过排队加锁~\cite{Anderson90}甚至可以在高竞争时取得更好的效果，
\cref{sec:locking:Other Exclusive-Locking Implementations}讨论了这个方法。
当然，最好的方法还是通过良好的并行设计来维持较低的锁竞争。
...
\IXB{Unfairness}可以被看成是饥饿的一种不太严重的表现形式，
当一组线程争抢同一把锁时，其中一部分线程获得了绝大多数的获取机会。
这在带有共享缓存或\IXacr{numa}特性的机器上可能出现，
如\cref{fig:locking:System Architecture and Lock Unfairness}所示。
如果CPU~0释放了一把其他CPU都想获取的锁，
由于CPU~0与CPU~1共享互连通道，CPU~1比CPU~2--7更容易抢到这把锁。
如果CPU~1持有锁的时间足够长，使得CPU~0在CPU~1释放时又在请求锁，
反之亦然，那么锁就可能只在CPU~0和CPU~1之间来回传递，
绕过了CPU~2--7。

\QuickQuiz{
	使用好的并行设计使锁竞争足够低以避免不公平性
	不是更好吗？
}\QuickQuizAnswer{
	在某种意义上是更好的，但有些情况下使用
	有时会导致高锁竞争的设计是合适的。
	...
	例如，设想一个受罕见错误条件影响的系统。
	在罕见错误条件持续期间，拥有一个性能和可扩展性
	较差的简单错误处理设计，可能比拥有一个只在那些
	罕见错误条件生效时才有用的复杂且难以调试的设计更好。

	话虽如此，通常值得投入一些精力来尝试设计一个
	既简单又在错误条件下高效的设计，例如通过分区问题。
}\QuickQuizEnd

\subsection{低效率}
\label{sec:locking:Inefficiency}

锁是通过原子指令和\IXpl{memory barrier}实现的，
并且常常涉及缓存未命中。
正如我们在\cref{chp:Hardware and its Habits}中所见，
这些指令代价相当昂贵，开销大约比简单指令高两个数量级。
这对加锁来说可能是一个严重的问题：
如果你用一把锁保护一条指令，开销就会增加一百倍。
即使假设可扩展性完美，也需要\emph{一百}个CPU
才能跟上一个执行不加锁版本的单个CPU。
...
如图所示，锯一块板会将板的一小部分（锯片的宽度）变成锯末。
当然，锁是对时间进行分区而不是锯木头，\footnote{
	也就是说，加锁是时间维度的同步。
	在时间和空间上都进行同步的机制在
	\cref{chp:Deferred Processing}中描述。}
但就像锯木头一样，使用锁对时间进行分区会因为锁开销
和（更糟糕的）锁竞争而浪费一些时间。
一个重要的区别是，如果有人把板锯成太小的碎片，
大部分板变成锯末的结果会立即显而易见。
相比之下，某次锁获取是否浪费了过多的时间并不总是显而易见的。
...
不过一旦持有了锁，特别是对于较大的\IXpl{critical section}来说，
CPU的缓存反而是高效的性能加速器。
获取锁可能代价高昂，但一旦持有，持有者可以不受干扰地
访问被该锁保护的数据。
...
锁的类型之多，说出来一定让你大吃一惊，远超本章所能详尽描述的。
以下各节讨论
...
一次只能有一个线程持有该锁。
持有者对受该锁保护的所有数据享有排他性的访问权。

当然，这一切的前提是该锁确实覆盖了所有应当受保护的数据。
虽然有一些工具可以帮助检查
（例如参见\cref{sec:formal:Axiomatic Approaches and Locking}），
但确保在需要时始终获取锁的最终责任还是落在开发者身上。

\QuickQuiz{
	让一个互斥锁获取后紧接着释放同一个锁，
	即空的临界区，有意义吗？
}\QuickQuizAnswer{
	基于锁的空临界区很少使用，但确实有其用途。
	要点是互斥锁的语义有两个组成部分：
	\begin{enumerate*}[(1)]
	\item 熟悉的数据保护语义和
	\item 消息语义，释放给定锁会通知等待获取同一锁的操作。
	\end{enumerate*}
	空的临界区使用消息组件而不使用数据保护组件。
...
这种实现和互斥锁一样，对于很短的临界区来说开销太大：
获取和释放锁的开销比一条简单指令的开销高两个数量级。
当然，如果临界区足够长，获取和释放锁的开销与之相比就可以忽略不计了。
可是因为一次只有一个线程能操纵锁，
随着CPU数目的增加，临界区的大小也需要增加才能平衡掉开销。
...
因为作用域只能嵌套，所以严格的RAII式加锁不允许重叠的临界区。
...
几乎可以肯定这会导致死锁，就像库函数直接调用了信号处理程序一样。
...
以及如何获取它们（传入指向锁获取和释放函数的指针），
同时也允许给定的库函数通过决定在哪里获取和释放lock
来控制自己的并发性。

特别是，该策略允许锁获取和释放函数根据需要阻塞信号，
...
这些规则可以通过使用类似于Linux内核的lockdep锁依赖
