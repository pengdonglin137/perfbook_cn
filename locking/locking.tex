...
在近来对并行编程的研究中，加锁总是扮演着反派角色。
\IX{加锁}背负着诸多指控，包括引发死锁、护送效应（convoy）、\IX{starvation}饥饿、
\IX{unfairness}不公平、\IXpl{data race}数据竞争，以及各种其他并发罪行。
有趣的是，在生产级共享内存并行软件中
真正承担重任的角色同样是加锁。
本章将着眼于加锁的这种二元身份——是英雄还是反派，
如
\cref{fig:locking:Locking: Villain or Slob?,%
fig:locking:Locking: Workhorse or Hero?}
所示。

这种"变身怪医"（Jekyll-and-Hyde）般的看法源于以下几个原因：

\begin{enumerate}
\item	很多因加锁产生的问题都有务实的设计解决方案，
	而且在大多数情况下效果良好，例如：
	\begin{enumerate}
	...
	\end{enumerate}
\item	加锁的某些问题只在竞争程度很高时才会出现，
	而一般只有不良的设计才会让竞争如此激烈。
\item	加锁的某些问题可以通过将其他同步机制
	与加锁配合使用来避免。
	这些其他机制包括
...
\item	直到不久之前，几乎所有的共享内存并行程序
	都是闭源的，因此多数研究者很难
	了解到业界的务实解决方案。
\item	加锁在某些软件上运行得很好，
	在某些软件上又运行得极差。
	那些在加锁运行良好的软件上做开发的程序员，
	对加锁的态度往往比另一些没那么幸运的程序员更加正面，
	\cref{sec:locking:Locking: Hero or Villain?}讨论了这个问题。
\item	所有美好的故事都需要一个反派，而加锁在研究文献中
	扮演替罪羊的角色已经有着悠久而光荣的历史了。
\end{enumerate}
...
对于共享内存并行程序的开发者来说，
最重要的考虑之一就是保持程序的运转。
因此，以下各节将介绍死锁、\IX{livelock}活锁、饥饿、
不公平和低效率。

\subsection{死锁（deadlock）}
\label{sec:locking:Deadlock}

当一组线程（thread）中的每个线程都持有至少一个lock，同时又在等待
该组中另一个成员释放它持有的另一个lock时，就会发生\IXB{Deadlock}。
即使在只包含单个线程的组中，当该线程试图获取它已经持有的
非递归lock时，这种情况也会发生。
因此，即使只有一个线程和一个lock，也可能发生deadlock！

如果缺乏外界干预，deadlock会一直持续。
除了持有lock的线程释放，没有线程可以获取到该lock，
但是持有lock的线程在等待获取该lock的线程释放其他lock之前，
又无法释放该lock。
...
我们可以用有向图来表示deadlock场景，节点代表lock和线程，
如\cref{fig:locking:Deadlock Cycle}所示。
从lock指向线程的箭头表示该线程持有该lock，
例如，线程~B持有Lock~2和Lock~4。
从线程指向lock的箭头表示该线程在等待该lock，
例如，线程~B在等待Lock~3。

Deadlock场景必然包含至少一个deadlock循环。
...
虽然有一些软件环境（如数据库系统）可以修复已有的deadlock，
但这种方法要么需要杀掉其中一个线程，要么强制从某个线程中窃取lock。
杀线程和强制窃取对于事务来说是可以的，
但是对内核和应用程序这种层次的加锁来说问题多多：
处理部分更新的数据极其复杂、危险且容易出错。

因此，内核和应用程序需要避免deadlock，而非从deadlock中恢复。
...
虽然没有任何一种deadlock避免策略可以适用于所有情况，
但有很多工具可供选择。

\subsubsection{锁层次结构}
\label{sec:locking:Locking Hierarchies}

Locking hierarchy为lock逐个编号，禁止不按顺序获取lock。
在\cref{fig:locking:Deadlock Cycle}中，
我们可以用数字为lock编号，这样如果线程已经持有编号相同或者更高的lock，
就不允许再获取编号相同或者更低的lock。
线程~B违反了这个层次结构，因为它在持有Lock~4时
又试图获取Lock~3，因此导致了deadlock的发生。

再次强调，按locking hierarchy使用lock时要为lock编号，严禁不按顺序获取lock。
...
然而，locking hierarchy本质上要求全局性，因此很难应用于库函数。
毕竟，如果调用某个库函数的应用程序还没有开始编写，
那么可怜的库函数实现者又怎么才能遵循这个还不存在的locking hierarchy呢？

一种特殊的情况——幸运的是这也是普遍情况——是库函数并不调用任何调用者的代码。
这时，如果库函数持有任何lock，它绝不会再去获取调用者的lock，
这样就不可能出现同时包含库和调用者的lock的deadlock循环。
...
传递给\co{qsort()}，而这个简单函数不获取任何lock。

现在，假如\co{qsort()}在持有Lock~C时调用\co{cmp()}，
这违背了之前提到的黄金规则"释放全部的lock"，如
\cref{fig:locking:Without qsort() Local Locking Hierarchy}所示，
那么deadlock将会发生。
为了让读者理解，假设一个线程调用\co{foo()}，
另一个线程同时调用了\co{bar()}。
第一个线程将获取Lock~A，第二个线程将获取Lock~B\@。
如果第一个线程调用\co{qsort()}时获取了Lock~C，那么这时它在调用
\co{cmp()}时将无法获取Lock~B。
但第一个线程获取了Lock~C，所以第二个线程调用\co{qsort()}时
将无法获取Lock~C，因此也无法释放Lock~B，导致deadlock。

相反，如果\co{qsort()}在调用比较函数之前——
对于\co{qsort()}来说属于未知代码——释放Lock~C，
就可以避免deadlock，
如\cref{fig:locking:Local Locking Hierarchy for qsort()}所示。

如果每个模块都在调用未知代码之前释放全部的lock，
那么每个模块自身都避免了deadlock，
这样整个系统也就避免发生deadlock了。
因此，这条规则极大地简化了deadlock分析，增强了代码的模块化。
...
不幸的是，有时保留状态可能不可行，
重新初始化也可能不可行，
这时我们无法通过在调用未知代码之前释放lock的方式来构建局部locking hierarchy。
可是我们可以代之以构建一个分层locking hierarchy，如
\cref{fig:locking:Layered Locking Hierarchy for qsort()}所示。
这里，\co{cmp()}函数在获取了Lock~A、B、C之后再获取一个新的Lock~D，
这就避免了deadlock。
这样我们把全局deadlock层次结构分成了三层：
...
请注意，让\co{cmp()}使用分层的Lock~D并不是简单的机械修改。
恰恰相反，这种改动通常需要在设计层面进行大量更改。
然而，为了避免deadlock而付出这些修改的努力
往往只是一个很小的代价。

还有一个例子，也是无法在调用未知代码之前释放全部lock的情况。
请设想一个链表的迭代器，如
\cref{lst:locking:Concurrent List Iterator}（\path{locked_list.c}）所示。
\co{list_start()}函数获取链表上的lock并且返回第一个元素（假如链表不为空），
\co{list_next()}要么返回指向下一个元素的指针，
要么在到达链表末尾时释放lock并返回\co{NULL}。
...
这个例子十分简单，并且隐藏了所有lock的细节。
\end{fcvref}

只要用户代码在处理每个列表元素时，不要再去获取已经被
其他对\co{list_start()}或\co{list_next()}的调用所持有的lock——
否则会导致deadlock——那么lock在用户代码里就可以继续保持隐藏。
我们在给列表迭代器加锁时通过分层locking hierarchy，就可以避免deadlock。

这种分层方法可以扩展到任意多的层次，
但是每增加一层，加锁的整体设计就复杂一分。
这种复杂性的增长对于某些类型的面向对象设计来说极不方便，
会导致一大堆对象将lock毫无纪律地传来传去。\footnote{
	这种情况的一个名称是"面向对象的意大利面条代码。"}
面向对象设计的习惯与避免deadlock的需求之间的这种冲突，
也是并行编程被某些人认为如此困难的一个重要原因。
...
虽然有一些例外，但一般来说设计一个包含指向lock的指针的API
意味着这个设计本身就存在问题。
将内部lock传递给其他软件组件违反了信息隐藏的美学，
而信息隐藏恰恰是一个关键的设计准则。
...
比如说有个函数要返回某个对象，而在对象成功返回之前必须持有
调用者提供的lock。
再比如POSIX的\apipx{pthread_cond_wait()}函数，
要传递一个指向\apipx{pthread_mutex_t}的指针
来防止错过唤醒而导致的挂起。
...
长话短说，如果你发现API需要将一个指向lock的指针作为参数或者返回值，
请慎重考虑一下是否要修改设计。
这有可能确实是正确的做法，但是经验告诉我们这种可能性很低。

\subsubsection{条件加锁}
\label{sec:locking:Conditional Locking}

但假如某个场景设计不出合理的locking hierarchy。
这在现实生活中是可能发生的，比如，在分层网络协议栈中，
数据包流是双向的。
当数据包从一层传往另一层时，有可能需要在两层中同时持有lock。
因为数据包可以从协议栈上层传往下层也可能相反，
这简直是deadlock的天然温床，
如\cref{lst:locking:Protocol Layering and Deadlock}所示。
\begin{fcvref}[ln:locking:Protocol Layering and Deadlock]
在这个例子中，当数据包在协议栈中从上往下发送时，
必须逆序获取下一层的lock。
而数据包在协议栈中从下往上发送时，是按顺序获取lock，
\clnref{acq}处的lock获取将导致deadlock。
\end{fcvref}
...
在本例中避免deadlock的办法是先强加一套locking hierarchy，
但在必要时又可以有条件地乱序获取lock，
...
与无条件获取第1层的lock不同，\clnref{trylock}
使用\apik{spin_trylock()}有条件地获取lock。
该原语在lock可用时立即获取并返回非零值，
在lock不可用时不获取lock而返回零。
...
条件加锁有一个重要的特例：在执行真正的处理工作之前，
已经拿到了所有必需的lock。
在这种情况下，处理不需要是幂等的（idempotent）：
如果发现在不释放lock的情况下拿不到某个lock，
那么释放所有持有的lock，重新获取。
只有在持有所有必需的lock以后才开始处理工作。
...
两阶段加锁~\cite{PhilipABernstein1987}在事务数据库系统中
已经存在了很长时间，它就应用了这个策略。
两阶段加锁事务的第一阶段，只获取lock但不释放lock。
一旦所有必需的lock全部获得，事务进入第二阶段，
只释放lock但不获取lock。
这种加锁方法使得数据库可以对执行的事务提供可串行化保证，
换句话说，保证事务看到和产生的数据在全局范围内顺序一致。
很多数据库系统都依靠中止事务的能力，不过两阶段加锁也可以简化成
在持有所有必需的lock之前避免修改共享数据。
虽然使用两阶段加锁仍然会出现livelock和deadlock，
但是在现有的大量数据库教科书中已经有了很多实用的解决办法。

\subsubsection{一次一个锁的设计}
\label{sec:locking:Single-Lock-at-a-Time Designs}

在某些情况下，可以避免嵌套加锁，从而避免deadlock。
比如，如果有一个可以完美分割的问题，每个分片拥有一个lock。
然后处理任何特定分片的线程只需获得对应这个分片的lock。
因为没有任何线程在同一时刻持有一个以上的lock，deadlock就不可能发生。

但是必须要有某种机制来保证在没有持有lock的情况下
所需的数据结构仍然存在。
一种这样的机制在
\cref{sec:locking:Lock-Based Existence Guarantees}中讨论，
然而，一个重要的限制条件是被处理的问题必须能够被分割。
\Cref{chp:Data Ownership}对数据所有权（data ownership）进行了更详细的讨论，
\cref{chp:Deferred Processing}中还介绍了其他几种方法。

\subsubsection{信号/中断处理程序}
\label{sec:locking:Signal/Interrupt Handlers}

涉及信号处理程序的deadlock通常可以很快解决：
在信号处理程序中调用\apipx{pthread_mutex_lock()}
是不合法的~\cite{OpenGroup1997pthreads}。
可是，精心构造一种\emph{可以}在信号处理程序中使用的
加锁原语是可能的（虽然通常来说不太明智）。
此外，几乎所有操作系统内核都允许在中断处理程序中获取lock，
中断处理程序可以说是内核对信号处理程序的模拟。

其中的诀窍是在任何可能在信号（或中断）处理程序中获取lock的时候
阻塞信号（或禁用中断）。
不仅如此，如果已经获取了这样的lock，那么在不阻塞信号的情况下
尝试去获取任何可能在信号处理程序之外被持有的lock，
这种操作都是非法的。
...
假如处理程序获取lock是为了处理多个信号，
那么无论是否获取了lock，甚至无论lock是否在信号处理程序之内被获取，
每个信号也都必须被阻塞。
...
不幸的是，在一些操作系统中阻塞和解除阻塞信号属于代价昂贵的操作，
这里包括Linux。所以出于性能上的考虑，
能在信号处理程序中持有的lock仅能在信号处理程序中获取，
应用程序代码和信号处理程序之间的通信通常使用无锁（lock-free）同步机制。

又或者除非处理致命异常，否则完全禁止使用信号处理程序。
...
对于基于共享内存的并行程序员来说，有大量避免deadlock的策略可用，
但如果遇到这些策略都不适用的时候，总还是可以用顺序代码来实现。
这也是为什么专家级程序员的工具箱里总是有好几样工具的原因之一：
加锁是一种威力巨大的处理并发的工具，但是别忘了总有些活儿适合用其他工具处理。
...
\QuickQuiz{
	...
	\item	使用事务内存（transactional memory）。
		这种方法有许多优点和缺点，
...
虽然条件加锁是一种有效避免deadlock的机制，但是有可能被滥用。
看看\cref{lst:locking:Abusing Conditional Locking}中
漂亮且对称的例子吧。
该例中的美遮掩了一个丑陋的livelock。
为了发现它，考虑以下事件序列：
...
Livelock可以被看成是starvation的一种极端形式，
此时不是一个线程，而是所有线程都在饿死。\footnote{
...
Livelock和starvation都属于事务内存软件实现中的严重问题，
所以现在引入了\emph{竞争管理器}这样的概念来封装这些问题。
以加锁为例，通常简单的指数退避就能解决livelock和starvation。
指数退避是指在每次重试之前增加按指数增长的延迟，
...
不过，为了获取更好的性能，退避应该有个上限，
如果使用排队加锁~\cite{Anderson90}甚至还可以在高竞争时取得更好的性能，
\cref{sec:locking:Other Exclusive-Locking Implementations}讨论了这个方法。
当然，最好的方法还是通过良好的并行设计使\IX{lock contention}程度变低。
...
\IXB{Unfairness}可被看成是starvation的一种不太严重的表现形式，
当某些线程争抢同一个lock时，其中一部分线程在绝大多数时间都可获取到lock，
另一部分线程则遭遇不公平对待。
这在带有共享缓存或\IXacr{numa}内存的机器中可能出现，
如\cref{fig:locking:System Architecture and Lock Unfairness}所示。
如果CPU~0释放了一个其他CPU都想获取的lock，
因为CPU~0与CPU~1共享内部连接，所以CPU~1相较于CPU~2--7更容易抢到lock。
反之亦然，如果一段时间后CPU~0又开始争抢该lock，
那么CPU~1释放lock时CPU~0也更容易获取，
导致lock绕过了CPU~2--7，只在CPU~0和CPU~1之间换手。
...
Lock是由原子指令和\IXpl{memory barrier}实现的，
并且常常涉及缓存未命中（cache miss）。
正如我们在\cref{chp:Hardware and its Habits}中所见，
这些指令代价都比较昂贵，粗略地说开销比简单指令高两个数量级。
这对加锁来说可能是一个严重的问题：
如果你用lock保护一条指令，你很可能在以百倍的速度带来开销。
对于相同的代码，即使假设可扩展性非常完美，
也需要\emph{一百}个CPU才能跟上一个执行不加锁版本的单个CPU。
...
粒度太粗会限制可扩展性，粒度太细会导致巨大的同步开销。

不过一旦持有了lock，特别是对于较大的\IXpl{critical section}来说，
CPU的缓存反而是高效的性能加速器。
获取lock可能代价高昂，但是一旦持有，持有者可以不受干扰地
访问被该lock保护的数据。
...
Lock有多少种类型，说出来一定让你大吃一惊，远超本节所能详尽描述的。
以下各节讨论
...
\IXhpl{Exclusive}{lock}正如其名：
一次只能有一个线程持有该lock。
持有者对受lock保护的所有数据享有排他性的访问权。

当然，这是在假设该lock保护了所有应当受保护的数据的前提下。
虽然有一些工具可以帮助检查
（例如参见\cref{sec:formal:Axiomatic Approaches and Locking}），
但确保始终在需要时获取lock的最终责任还是落在开发者身上。
...
一方面允许任意数量的读者同时持有lock，
另一方面允许最多一个写者持有lock。
理论上，reader-writer lock对读侧重的数据来说拥有极佳的可扩展性。
在实践中的可扩展性则取决于具体的实现。

经典的reader-writer lock实现使用一组只能以原子操作方式修改的计数器和标志。
这种实现和exclusive lock一样，对于很小的critical section来说开销太大：
获取和释放lock的开销比一条简单指令的开销高两个数量级。
当然，如果critical section足够长，获取和释放lock的开销与之相比就可以忽略不计了。
可是因为一次只有一个线程能操纵lock，
随着CPU数目的增加，critical section的大小也需要增加才能平衡掉开销。

另一个设计reader-writer lock的方法是使用每线程的exclusive lock
~\cite{WilsonCHsieh92a}，这种reader-writer lock对读者非常有利。
线程在读的时候只要获取本线程的lock即可，
而在写的时候需要获取所有线程的lock。
在没有写者的情况下，每个读者的开销只相当于一条原子指令和一个memory barrier开销之和，
并且不会有缓存未命中，这点对于加锁原语来说非常不错。
不过，写者的开销包括缓存未命中，再加上原子指令和
memory barrier开销之和——乘以线程数量。

简单地说，reader-writer lock在有些场景非常有用，
但各种实现方式都有各自的缺点。
reader-writer lock的正统用法是用于非常长的
\IXhpl{read-side}{critical section}，
critical section耗时几百微秒或者毫秒以上最好。
...
Reader-writer lock和exclusive lock允许的规则不大相同：
exclusive lock只允许一个持有者，而reader-writer lock
允许任意多个读持有者（但只能有一个写持有者）。
Lock可能的准入策略有很多种，VAX/VMS分布式lock管理器
（DLM）~\cite{Snaman87}就是其中一个例子，如
\cref{tab:locking:VAX/VMS Distributed Lock Manager Policy}所示。
空白格表示兼容，包含"X"的格表示不兼容。

VAX/VMS DLM使用六种模式。
为了更好地比较，exclusive lock使用两种模式（持锁和未持锁），
而reader-writer lock使用三种模式（未持锁、持读锁和持写锁）。

第一种模式是null，也就是未持锁。
这种模式与所有其他模式兼容，这也是我们所期待的：
如果线程没有持有任何lock，那么也不应该阻止其他获取了lock的线程执行。

第二种模式是concurrent read，与除exclusive之外的所有模式兼容。
concurrent-read模式可用于对数据结构进行粗略的累加统计，
同时允许并发的写操作。

第三种模式是concurrent write，与null、
concurrent read和concurrent write兼容。
concurrent-write模式可用于近似统计计数的更新，
同时允许并发的读操作和写操作。

第四种模式是protected read，与null、
concurrent read和protected read兼容。
protected-read模式可用于读取数据结构的准确结果，
同时允许并发的读操作，但不允许并发的写操作。

第五种模式是protected write，与null和concurrent read兼容。
protected-write模式可用于在可能会受到protected read干扰的情况下
对数据结构进行写操作，允许并发的读操作。

第六种也是最后一种模式是exclusive，只与null兼容。
exclusive模式可用于需要排他访问的场合。

有趣的是，exclusive lock和reader-writer lock
可以用VAX/VMS DLM来模拟。
Exclusive lock只使用null和exclusive模式，而
reader-writer lock只使用null、protected-read和
protected-write模式。
...
虽然VAX/VMS DLM策略已在分布式数据库中得到广泛应用，
但在共享内存的应用程序中却很少见。
其中一个可能的原因是分布式数据库中的通信开销
在一定程度上可以抵消VAX/VMS DLM更复杂策略所带来的开销。

然而，VAX/VMS DLM只是一个例子，用来说明加锁背后的概念可以有多灵活。
同时这个例子也是对现代DBMS所使用的加锁机制的简单介绍，
相对于VAX/VMS DLM的六种模式，有些数据库中使用的lock甚至可以有三十种以上的模式。

\subsection{作用域加锁}
\label{sec:locking:Scoped Locking}

到目前为止我们讨论的加锁原语都需要显式的获取和释放函数，
比如\co{spin_lock()}和\co{spin_unlock()}。
另一种方法则是使用面向对象的"资源获取即初始化"（\IXacrmfst{raii}）
模式~\cite{MargaretAEllis1990Cplusplus}。\footnote{
	在\url{https://www.stroustrup.com/bs_faq2.html\#finally}
	中有更清晰的表达。}
该设计模式常见于C++等支持auto变量的语言中，
当进入对象的作用域时调用\emph{构造函数}，
当退出对象的作用域时调用\emph{析构函数}。
同理，加锁可以让构造函数去获取lock，析构函数释放lock。

这个方法十分有用，事实上在1990年本书作者曾认为这是唯一有用的
加锁方法。\footnote{
	我后来在Sequent Computer Systems做并行工作
	很快就让我摒弃了这个错误的观念。}
RAII式加锁法有一个非常好的特性：你不需要精心思考如何在每个会退出对象作用域的
代码路径上释放lock，这个特性避免了一系列bug的出现。

但是，RAII式加锁也有其黑暗面。
RAII使得对获取和释放lock的封装极其困难，
比如在迭代器中。
在很多迭代器的实现中，你需要在
迭代器的``start''函数中获取lock，在``stop''函数中释放它。
相反RAII式加锁要求获取和释放lock都发生在相同的对象作用域中，
这使得对它们的封装变得困难甚至不可能。

因为作用域只能嵌套，所以严格的RAII式加锁不允许重叠的critical section。
这让lock的很多有用的用法变得不再可能，
比如用于协调对并发访问某事件的加锁树。
对于任意规模的并发访问，只需要其中一个成功，
其余请求最好是让他们尽可能快速且无痛地失败。
否则在大型系统上（几百个CPU）对lock的竞争
会成为大问题。
...
这里使用的是一种通用策略——\emph{锦标赛}，
任意给定CPU有条件地获取其
叶子\co{rcu_node}结构的\co{->fqslock}，如果成功，
尝试获取其父节点的，如成功再释放子节点的。
此外，CPU在每一层检查全局\co{gp_flags}变量，
如果这个变量表明其他CPU已经访问过这个事件，
该CPU被淘汰出锦标赛。
这种先获取后释放的序列一直持续到
\co{gp_flags}变量表明已经有人赢得了锦标赛、
某一层获取\co{->fqslock}失败、或者
拿到了根\co{rcu_node}结构的\co{->fqslock}为止。
...
此函数使用嵌套循环，外层循环重复地将lock的值
与1（意为``已锁定''）作原子交换。
如果旧值已经为1（换句话说，别人已经持有该lock），
那么内层循环（\clnrefrange{inner:b}{inner:e}）
持续自旋直到lock可用，到那时外层循环再次尝试获取lock。
\end{fcvref}
...
虽然这只是一个简单的test-and-set lock~\cite{Segall84}的例子，
但在生产环境中广泛采用一种非常类似的机制来实现纯spinlock。

\subsection{其他互斥锁实现}
\label{sec:locking:Other Exclusive-Locking Implementations}

基于原子指令的lock有很多种可能的实现，
Mellor-Crummey和Scott~\cite{MellorCrummey91a}综述了其中很多种。
这些实现代表着多维设计权衡中的不同
顶点~\cite{Guerraoui:2019:LPA:3319851.3301501,HugoGuirouxPhD,McKenney96a}。
例如，前一节介绍的基于原子交换的test-and-set lock
在低度竞争时性能良好，并且具有内存占用小的优点。
它避免了给不能使用它的线程提供lock，但作为结果可能会导致
在高度竞争时出现\IX{unfairness}甚至\IX{starvation}。

相比之下，ticket lock~\cite{MellorCrummey91a}曾在Linux内核中使用，
避免了高度竞争时的unfairness。
然而，由于其先进先出的准则，它可以将lock授予
给当前无法使用它的线程，例如线程由于被抢占、中断或其他方式而失去CPU。
...
只要是等待者在单个内存位置上自旋等待lock的各种实现，
包括test-and-set lock和ticket lock，
都在高度竞争时存在性能问题。
原因是释放lock的线程必须更新对应内存位置的值。
在低度竞争时，这不是问题：
相应的\IX{cache line}很可能仍然属于本地CPU并且仍然可以由持有lock的线程来更改。
相反，在高度竞争时，每个尝试获取lock的线程
将拥有cache line的只读副本，因此lock持有者
将需要使所有此类副本无效，然后才能更新内存位置来释放lock。
通常，CPU和线程越多，在高度竞争条件下
释放lock时所产生的开销就越大。

这种负可扩展性已经引发了许多种不同的排队lock
...
排队lock通过为每个线程分配一个队列元素，避免了高昂的缓存无效化开销。
这些队列元素链接在一起成为一个队列，控制着等待线程获取lock的顺序。
这里的关键点在于每个线程只在自己的队列元素上自旋，
使得lock持有者只需要使下一个线程CPU缓存中的第一个元素无效即可。
这种安排大大减少了在高度竞争时交换lock的开销。

最近的排队lock实现也将系统的架构纳入考虑之中，
优先在本地授予lock，同时也采取措施避免
饥饿（starvation）~\cite{McKenney02e,radovic03hierarchical,radovic02efficient,BenJackson02,McKenney02d}。
这些实现可以看成是传统上用于调度磁盘I/O时使用的电梯算法的模拟。
...
不幸的是，虽然相同的调度逻辑提高了排队lock在高度竞争时的\IX{效率}，
也增加了其在低度竞争时的开销。
因此，Beng-Hong Lim和Anant Agarwal将简单的test-and-set lock
与排队lock相结合，在低度竞争时使用test-and-set lock，
在高度竞争时切换到排队lock~\cite{BengHongLim94}，
因此得以在低度竞争时获得低开销，并在高度竞争时获得公平性和高吞吐量。
Browning等人采取了类似的方法，但避免了使用单独的标志位，
使得test-and-set快速路径使用与简单test-and-set lock
相同的指令序列~\cite{LukeBrowning2005SimpleLockNUMAAware}。
该方法已在生产环境中使用。

在高度竞争中出现的另一个问题是当lock的持有者受到延迟，
特别是当延迟的原因是抢占时，这可能导致\emph{优先级反转}——
低优先级的线程持有lock，但是被中等优先级且绑定在某CPU上的线程抢占，
这导致高优先级线程在尝试获取lock时阻塞。
结果是绑定在某CPU的中优先级进程阻止高优先级进程运行。
...
避免优先级反转的另一种方法是在持有lock时防止抢占。
由于在持有lock的同时防止抢占也提高了吞吐量，
因此大多数专有UNIX内核提供了某种形式的调度器感知
...
我们可以期望出现更多的专用加解锁原语，
例如参见Guerraoui等人~\cite{Guerraoui:2019:LPA:3319851.3301501,HugoGuirouxPhD}。
不过，你应该仔细考虑这个重要的安全提示：
只要可能，尽量使用标准同步原语。
标准同步原语与自己开发的原语相比，
最大的优点是标准原语通常\emph{不太容易出bug}。\footnote{
	是的，我至少做了自己应有份额的自制
	同步原语。
	然而，你会注意到我的头发比
	我开始做那种工作之前灰白得多。
...
像在现实生活中的情况一样，加锁可以是英雄也可以是反派，
既取决于如何使用它，也取决于要解决的问题。
以作者的经验，那些写应用程序的人很喜欢加锁，
那些编写并行库的同行不那么开心，
而那些并行化现有顺序库的人则非常不爽。
以下各节讨论了这些观点差异的原因。
...
比较函数参数就是一个典型的例子。
另一个复杂情况是与信号处理程序的交互。
如果库函数接收的信号调用了应用程序的信号处理程序，
几乎可以肯定这会导致deadlock，就像库函数直接调用了信号处理程序一样。
最后一种复杂情况发生在那些可以在\co{fork()}/\co{exec()}对之间
使用的库函数中，例如由于使用\co{system()}函数。
在这种情况下，如果你的库函数在\co{fork()}的时候持有lock，
那么子进程就在持有该lock的情况下出生。
因为会释放lock的线程在父进程运行而不是子进程，
如果子进程调用你的库函数，deadlock会随之而来。
...
随着到处可见的低成本多核系统的出现，
常见的任务往往是并行化现有的库，这些库的设计仅考虑了单线程使用的情况。
从并行编程的角度看，这种对并行性的全面忽视可能导致库函数API的严重缺陷。
比如：
...
假设你正在编写一个单线程哈希表实现。
可以很容易并且快速地得到哈希表中元素总数的精确计数，
同时也可以很容易并且快速地在每次添加和删除操作时返回此计数。
所以为什么在实际中不这样做呢？

一个原因是精确计数器在多核系统上要么执行有误，要么扩展性不佳，
如\cref{chp:Counting}中所示。
因此，并行化这个哈希表的实现将会出现错误或者扩展性不佳。

那么我们能做什么呢？
一种方法是返回近似计数，使用\cref{chp:Counting}中的一个算法。
另一种方法是完全不用元素计数。
...
虽然对于一个对并发不友好的API来说，人们很容易去指责加锁是罪魁祸首，
但这并没有用。
另一方面，人们除了同情当年写下这段代码的倒霉程序员以外，
也没什么更好的办法。
如果程序员能在1985年就预见未来对并行性的需求，那简直是稀罕和高瞻远瞩；
如果那时就能设计出一个对并行友好的API，那真的是运气和荣耀的罕见巧合了。

随着时间变化，代码必须随之改变。
也就是说，如果某个受欢迎的库拥有大量用户，
在这种情况下对API进行不兼容的更改将是相当愚蠢的。
添加一个对并行友好的API来补充现有的顺序API，
可能是在这种情况下的最佳行动方案。

然而，出于人类的本性，不幸的程序员们更可能抱怨是加锁带来问题，
而不是他或她自己糟糕的（虽然可以理解的）API设计选择。
...
描述了对回调的无规律使用将提高加锁的难度。
这些章节也描述了如何设计库函数来避免这些问题，
但是期望一个1990年代的程序员在没有并行编程的经验时就遵循了这样的设计，
是不是有点不切实际？
因此，尝试并行化拥有大量回调的现有单线程库的程序员，
很可能会相当憎恨加锁。
...
从20世纪80年代或90年代的某个时候开始，面向对象编程变得主流起来，
因此在生产环境中有大量面向对象的代码，大部分是单线程的。
虽然面向对象是一种很有价值的软件技术，
但是毫无节制地使用对象可以很容易写出面向对象的意大利面条代码。
在面向对象的意大利面条代码中，执行流基本上是随机地
从一个对象走到另一个对象，使得代码难以理解，
甚至无法加入加锁层级。

虽然许多人可能会认为不管在任何情况这样的代码都应该清理，
说起来容易做起来难。
如果你的任务是并行化这样的庞然大物，通过运用
\cref{sec:locking:Conditional Locking,sec:locking:Acquire Needed Locks First}
中描述的技巧，以及将在
\cref{chp:Data Ownership,chp:Deferred Processing}中讨论的内容，
你对人生（还有加锁）感到绝望的机会可以大大减少。
这种情况似乎就是激发事务内存灵感的用例，
所以也值得一试。
也就是说，应该根据\cref{chp:Hardware and its Habits}中讨论的硬件特性
来选择同步机制。
毕竟，如果同步机制的开销大于那些被保护的操作的开销几个数量级，
结果不会好看。
