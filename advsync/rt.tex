...
不过话说回来，由于这明显不仅仅是一个硬件问题——而实在是一个大硬件问题——
将其归咎于软件也许并不公平。\footnote{
	或者，考虑到现代的锤子，应该说是大型钢铁问题。}
这表明我们可以将硬实时软件定义为总是能满足其截止时间的软件，
前提是没有硬件故障。
不幸的是，故障并非总是可选项，
...
我们将着眼于两种主要类型的实时系统：事件驱动和轮询。
事件驱动的实时系统大部分时间处于空闲状态，
对实时事件的响应通过操作系统传递给应用。
可选地，系统也可以在后台运行非实时工作负载，而不是让其处于空闲。
轮询式实时系统具有一个CPU密集型的实时线程，
在一个紧凑循环中运行，每次迭代都轮询输入并更新输出。
...
虽然在Linux仅拥有最少实时功能的时期，将Linux内核与RTOS配对
不失为一种明智而有用的短期应对措施，
但这也激发了向Linux内核添加实时功能的动力。
朝着这一目标的进展如
...
\paragraph{定时器}对实时操作来说显然极其重要。
毕竟，如果你不能指定某些事情在特定时间完成，
又怎么能在那个时间之前做出响应？
...
在优先级继承方案中，试图获取锁的高优先级线程将其优先级
传递给持有锁的低优先级线程，直到锁被释放，
从而防止长时间的优先级反转。
...
\QuickQuiz{
	但如果也需要确定性的更新呢？
	这不会因为非确定性的 RCU grace period 而产生问题吗？
}\QuickQuizAnswer{
	使用 \co{synchronize_rcu()} 确实会有问题，因为它会阻塞
	等待 RCU grace period 过期，但还有其他方法来处理 grace period。

	一种方法是改用 \co{call_rcu()}，它只是将 RCU 回调排队
	然后继续执行。
	经过一个 grace period 后，该回调将被调用，
	从而将 RCU grace period 延迟从执行更新的线程中卸载。
	然而，一些 RCU 实现会使 \co{call_rcu()} 执行大量工作，
	包括调用任意数量的回调。
	这类实现需要不同的方法。

	截至 2020 年代初期，Linux 内核中的一种不同方法是使用
	\cref{sec:defer:RCU has a Family of Wait-to-Finish APIs}
	中描述的轮询 RCU grace period API。
	\apik{get_state_synchronize_rcu()} 和
	\apik{poll_state_synchronize_rcu()} API 都是非阻塞的，
	这允许非阻塞算法使用 RCU，
	至少在周期性地调用其他 RCU 函数（如
	\apik{start_poll_synchronize_rcu()}）以确保启动所需的
	RCU grace period 的情况下是如此。
	希望轮询 RCU grace period API 能出现在更多 RCU 实现中。
}\QuickQuizEnd
