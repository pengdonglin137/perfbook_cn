% future/future.tex
% mainfile: ../perfbook.tex
% SPDX-License-Identifier: CC-BY-SA-3.0

\QuickQuizChapter{chp:Conflicting Visions of the Future}{对未来的不同愿景}{qqzfuture}
%
\Epigraph{预测是非常困难的，尤其是关于未来的预测。}
	 {Niels Bohr}

本章描述了关于并行编程未来可能出现的一些相互矛盾的愿景。
目前尚不清楚这些愿景中哪些会成为现实，事实上，也不清楚其中是否有任何一个会实现。
然而它们仍然很重要，因为每种愿景都有忠实的拥护者。
如果有足够多的人强烈相信某件事，你就不得不在它的阴影下生活，
并考虑它对拥护者们的思想、言论和行动所产生的影响。
此外，这些愿景中的一个或多个很可能会成为现实。
当然，大多数不会。
能分辨出哪个是哪个，你就发财了~\cite{KeithRSpitz1977}！

因此，以下各节概述了transactional memory、hardware transactional memory、
回归测试中的形式化验证以及并行functional programming。
但首先，让我们看一个来自 21 世纪初的关于预测的警示故事。

\input{future/cpu.tex}
\IfTwoColumn{}{\FloatBarrier}
\input{future/tm.tex}
\IfTwoColumn{}{\FloatBarrier}
\input{future/htm.tex}
\input{future/formalregress.tex}
\input{future/fp.tex}
% @@@ Maybe add quantum computing back in, but heavily summarized.

\section{总结}
\label{sec:future:Summary}
%
\epigraph{生活只能在回顾中理解；
	   但必须向前过。}
	  {S{\o}ren Kierkegaard}

本章快速浏览了若干可能的未来方向，
包括多核、transactional memory、形式化验证作为回归测试，
以及并发 functional programming。
这些未来方向中的任何一个都可能成为现实，但更有可能的是，
如同以往一样，未来将比我们所能想象的更加奇异。

\QuickQuizAnswersChp{qqzfuture}
