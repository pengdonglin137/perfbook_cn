...
透过多年经验的棱镜，过去的岁月总是显得如此简单而纯真。
21世纪初对于\IXr{Moore's Law}即将无法继续带来传统的CPU时钟频率提升
这一迫在眉睫的失败，大多数人还浑然不知。
...
因此过了一段时间，这个社区才得出结论：
\IXaltr{Moore's-Law}{Moore's Law}带来的CPU核心时钟频率提升这顿``免费午餐''
确实已经彻底结束了。
永远不要忘记：
信念是一种情感，不一定是理性技术思考过程的结果！
...
\begin{quote}
	单处理器至上的一个不那么极端的变体是具有硬件多线程的单处理器，
	事实上，多线程CPU现在已经是许多台式和笔记本电脑系统的标配。
	最激进的多线程CPU共享所有级别的缓存层次结构，
	从而消除了CPU到CPU的\IXh{memory}{latency}，
	进而大大降低了传统同步机制的性能损失。
	然而，多线程CPU仍然会因竞争和内存屏障导致的
	流水线停顿而产生开销。
	此外，由于所有硬件线程共享所有级别的缓存，
	给定硬件线程可用的缓存只是等效单线程CPU上
	缓存的一部分，这可能会降低具有大缓存占用的应用程序的性能。
	还存在一些可能性，即可用缓存的限制将导致基于RCU的算法
	由于其宽限期引起的额外内存消耗而遭受性能损失。
	调查这种可能性是未来的工作。

	然而，为了避免这种性能下降，许多多线程CPU和多CPU芯片
	至少在某些缓存级别上按每个硬件线程进行分区。
	这增加了每个硬件线程可用的缓存量，
	但重新引入了从一个硬件线程传递到另一个硬件线程的
	缓存行的内存延迟。
\end{quote}

我们都知道这个故事是如何发展的：单个芯片上有多个多线程核心，
插在单个插槽中，对较少的活跃线程数有不同程度的优化。
...
\begin{quote}
	一切照旧情景假设内存延迟比率将大致保持在今天的水平。

	这一情景实际上代表了一种变化，因为要保持不变，
	互连性能必须开始跟上\IXaltr{Moore's-Law}{Moore's Law}
	带来的核心CPU性能提升。
	在这一情景中，由于流水线停顿、内存延迟和竞争引起的开销
	仍然显著，RCU保持着它今天所享有的高适用性。
\end{quote}

而变化一直是\IXr{Moore's Law}仍在提供的不断增加的集成度。
但从长远来看，会是什么呢？
每个芯片更多的CPU？
还是更多的I/O、缓存和内存？
...
一方面，这段话未能预见到RCU在具有显著更新密集度的工作负载中
可能遭受的缓存预热问题，部分原因是当时似乎不太可能
RCU真的会被用于此类工作负载。
