...
（TM），这一想法出现得更
晚~\cite{Herlihy93a}。不幸的是，尽管也有一些类似的提案被提
出~\cite{JMStone93}，支持此类事务的商用硬件并未立即出现。
...
隐含于TM的基本思想是原子地执行一段代码，使得其他线程（thread）看不到
任何中间状态。
TM的语义可以简单地通过将每个事务替换为反复获取、释放全局锁来实现，
尽管性能和可扩展性会极其糟糕。
TM实现（无论是硬件还是软件）中的大部分复杂性在于
高效地检测并发事务何时可以安全地并行运行。
由于这种检测是动态进行的，冲突的事务可能会被中止或``回滚''，
而且在某些实现中，这种失败模式对程序员是可见的。

随着事务大小的减小，事务回滚的可能性也随之降低，
因此TM对于小型基于内存的操作相当有吸引力，
例如用于栈、队列、哈希表（hash table）和搜索树的链表操作。
然而，对于大型事务，特别是包含I/O和进程创建等
...
事实是对于绝大多数计算机系统来说，与外部世界进行交互是第一级的需求。
因此，本节评析transactional memory在这些交互方面的能力，
重点关注I/O操作、时间延迟和持久存储。
...
它们被创建和销毁，它们的内存映射被修改，
它们会链接动态库，它们可以被调试。
以下各小节探讨transactional memory如何处理不断变化的执行环境。
...
要么认命TM只是并行程序员工具箱中的一个普通工具。
（公平地说，已经有一些TM支持者接受了这个现实。）
...
有一些理由让人相信transactional memory将比其他同步机制更能提高
\IX{productivity}（生产力），
但如果传统的调试技术不能应用于事务的话，
这种改进很可能被抵消。
特别是当新手程序员使用transactional memory编写大型事务时，更是如此。
...
如果某一天transactional memory证明它能为所有人完成所有事情，
那么它就不需要与任何其他同步机制交互。
...
一切就会完美运作。
但在实践中，根据TM系统的实现细节，可能会出现许多
不明显的复杂因素~\cite{Volos2008TRANSACT}。
这些复杂因素可以被解决，但代价是在事务外获取锁增加45\pct 的开销，
在事务内获取锁增加300\pct 的开销。
