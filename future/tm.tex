...
TM背后的基本思想是原子地执行一段代码，使得其他线程看不到中间状态。
...
例如，动态链接的函数可能会为锁引入\IX{deadlock}（死锁），
...
引用计数、hazard pointers和RCU都被大量使用，如
...
\item	与冲突TM更新并发运行的RCU读者从任何冲突的RCU加载中
	获取旧的（事务前的）值。
	这保留了RCU语义和性能，也防止了RCU更新\IX{starvation}（饥饿）。
