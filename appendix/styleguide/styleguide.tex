...
perfbook中关于等宽字体的首要策略是避免直接使用 \qco{\\texttt} or \qco{\\tt} macro.
...
\qco{<options>} to the \texttt{\textbackslash begin\{snippet\}} meta command
...
``\texttt{\textbackslash begin\{fcvlabel\}[<label base string>]}'' command as shown on
...
There can be multiple pairs of \texttt{\textbackslash begin\{snippet\}} and \texttt{\textbackslash end\{snippet\}}
...
pair of \texttt{\textbackslash begin[snippet]} and \texttt{\textbackslash end[snippet]} because of
...
要添加到API索引中的标识符的特殊情况如下：

\begin{itemize}
	\item	\verb|\apic{}| 用于C11 API成员。
	\item	\verb|\apig{}| 用于GCC API成员。
	\item	\verb|\apik{}| 用于Linux内核API成员。
	\item	\verb|\apikh{}| 用于Linux内核历史API成员，
  		例如\apikh{ACCESS_ONCE()}。
	\item	\verb|\apialtc{}| 用于C11 API成员，其中第一个参数
  		被打印，第二个参数被索引。
	\item	\verb|\apialtg{}| 用于GCC API成员，其中第一个参数
  		被打印，第二个参数被索引。
	\item	\verb|\apialtk{}| 用于Linux内核成员，其中第一个参数
  		被打印，第二个参数被索引。
	\item	\verb|\apipf{}| 用于perfbook API成员。
	\item	\verb|\apipx{}| 用于pthreads API成员。
	\item	\verb|\apiur{}| 用于用户空间RCU API成员。
\end{itemize}
...
many characters to escape.\footnote{
  The \texttt{\textbackslash verb} command is not almighty though.
  For example, you can't use it within a footnote.
  If you do so, you will see a fatal \LaTeX\ error.
  A workaround would be a macro named \texttt{\textbackslash VerbatimFootnotes}
...
Now that \texttt{\textbackslash clnrefrange}, \texttt{\textbackslash crefrange},
...
the issue.\footnote{To be exact, it is the \texttt{\textbackslash textellipsis} macro
  that is redefined.
  The behavior of \texttt{\textbackslash dots} macro in math mode is not affected.
  The ``amsmath'' package has another definition of \texttt{\textbackslash dots}.
...
by setting a negative value to the \texttt{\textbackslash abovetopsep} variable,
which controls the behavior of \texttt{\textbackslash toprule}.
...
\texttt{\textbackslash parbox} or \co{minipage}.
For example,
...
本节展示了一些使用booktabs和xcolors包的实验性表格。
