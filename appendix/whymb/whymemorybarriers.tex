...
因此，需要内存屏障来强制对同步原语等依赖于有序内存引用才能正确操作的事物进行排序。
...
这意味着发生了``缓存未命中''（更具体地说，是``预热''缓存未命中）。
...
且没有链接的硬件哈希表，如\cref{fig:app:whymb:CPU Cache Structure}所示。
...
\QuickQuiz{
	...
	Hennessy和Patterson的经典计算机体系结构入门著作~\cite{Hennessy2026}
	涵盖了许多这些问题。
}\QuickQuizEnd
...
这些存储缓冲区对于给定的CPU是本地的，或者在具有硬件多线程的系统上，
对于给定的核心是本地的。
