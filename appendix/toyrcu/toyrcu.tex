...
在这个实现中，\co{rcu_read_lock()}获取一个全局自旋锁（spinlock），\co{rcu_read_unlock()}释放它，\co{synchronize_rcu()}获取后立即释放它。

因为\co{synchronize_rcu()}在获取（并释放）锁之前不会返回，所以它不能在所有先前的RCU读端临界区（critical section）完成之前返回，从而忠实地实现了RCU语义。
...
\QuickQuizB{
	为什么
	\cref{lst:app:toyrcu:Lock-Based RCU Implementation}
	中RCU实现的死锁（deadlock）不会在任何其他RCU实现中也是死锁？
}\QuickQuizAnswerB{
...
\QuickQuizE{
	为什么不简单地在
	\cref{lst:app:toyrcu:Lock-Based RCU Implementation}
	的RCU实现中使用读写锁（reader-writer lock），以允许RCU读者并行执行？
}\QuickQuizAnswerE{
	事实上确实可以以这种方式使用读写锁。
	然而，教科书上的读写锁存在内存竞争问题，
...
这种每CPU一锁方法的修改版本，但改为每线程（thread）一锁，将在下一节中描述。
...
\QuickQuizE{
	...
	\co{rcu_assign_pointer()}需要内存屏障（memory barrier）。
...
这个实现使用了在\clnref{lock_unlock:grc}定义的全局引用计数器（reference counting）\co{rcu_refcnt}。
...
然而，这个实现仍然有一些严重的缺点。
首先，\co{rcu_read_lock()}和\co{rcu_read_unlock()}中的原子操作（atomic operation）仍然相当重，
...
\QuickQuiz{
	...
	虽然这确实会消除饥饿（starvation），但也意味着\co{rcu_read_lock()}会自旋或阻塞
	等待写者，而写者又在等待读者。
...
因此将单调递减。\footnote{
	...
	这个竞态条件（race condition）将由\co{synchronize_rcu()}的代码来处理。
	在此期间，我建议暂时搁置怀疑。}
...
\QuickQuiz{
	...
	它唯一的目的是让编译器的类型检查满意。
	如果Linux内核运行在8位CPU上，它还需要防止"存储撕裂（store tearing）"，
...
这些要求在几乎所有软件环境中都可以满足，
但通常会导致线程数量有固定的上限。
更复杂的实现可能会避免这种限制，例如使用可扩展的哈希表（hash table）。
这种实现可能会动态跟踪线程，
...
描述了一类宽限期为固定时间间隔的RCU实现。
\Cref{fig:defer:RCU Spatial/Temporal Synchronization}及其在
\cref{sec:defer:Quasi Reader-Writer Lock}中的讨论
表明可能存在一类对应的RCU实现，
其中宽限期不是固定的时间段，而是固定数量的地址空间，
2026年与\ppl{Hui}{Xie}的一次讨论使我关注到了这种可能性。

可以说，Linux内核地址消毒器
(KASAN)~\cite{AndreyRyabinin2015KASAN}包含了固定空间RCU\@。
KASAN将新释放的内存块保存在固定大小的隔离区中。
这样做是为了增加释放后使用访问发生在相应内存块尚未被重新分配的概率，
从而减少如果在任何仍然（错误地）引用它的悬挂指针的生命周期结束之前
内存被重新分配时可能发生的假阴性数量。
然而，这可以被视为一种固定空间RCU实现，
其中"读者"（持有指向新释放内存块的悬挂指针的任务）
被假定在相应内存块通过固定大小的隔离区并被重新分配之前完成。

就其本身而言，rcutorture做了类似的事情，它延迟释放
全局变量\co{rcu_torture_current}先前引用的
\co{RCU_TORTURE_PIPE_LEN}（即10个）内存块。
此外，总共有10倍的元素（即100个）。
与KASAN一样，目的是减少假阴性的发生，
使得一个极长的RCU读者与一系列极短的RCU宽限期相结合时，
仍然有大约99\%的概率让每个读者为那些过短的宽限期发出诊断。

尽管在这两个例子中，固定缓冲区的目的是检测错误，
但它们也可以用来创建一个极其有限且相当尖锐的RCU宽限期检测机制。
因此固定空间RCU是相当真实的，尽管可能只是非常罕见地值得推荐。
