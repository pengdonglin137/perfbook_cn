...
进程通过 \apipx{fork()} 原语创建，使用 \apipx{kill()} 原语销毁，
也可以使用 \apipx{exit()} 原语自我撤销。
执行 \co{fork()} 原语的进程被称为新创建进程的"父进程"。
父进程可以通过 \apipx{wait()} 原语等待子进程执行完毕。

请注意，本节中的示例都极其简单。
真实世界的应用使用这些原语时，还可能需要操作信号、文件描述符、
共享内存段以及许多其他资源。
此外，有些在特定子进程终止时需要执行特定操作的应用程序，
可能还需要关注子进程终止的原因。
这些情况下的判断逻辑显然会增加代码的复杂性。
...
如果 \co{fork()} 执行成功，它会返回两次，一次是在父进程中，
另一次是在子进程中。
\co{fork()} 的返回值可以让调用者区分这两种情况，如
\cref{lst:toolsoftrade:Using the fork() Primitive}
(\path{forkjoin.c})所示。
\Clnref{fork}执行 \co{fork()} 原语，用局部变量
\co{pid} 存储返回值。
\Clnref{if}检查 \co{pid} 是否为零，如果为零
则是子进程，继续执行\clnref{child}。
如前所述，子进程也可以调用 \co{exit()} 原语来终止。
如果小于零，则是父进程返回了错误，\clnref{else}检查
\co{fork()} 调用返回的错误码，在\clnrefrange{errora}{errorb}
打印错误信息并退出。
如果 \co{pid} 大于零，则 \co{fork()} 成功执行，父进程执行\clnref{parent}，
此时 \co{pid} 包含子进程的进程 ID 号。
\end{fcvref}
...
父进程还可以通过 \apipx{wait()} 原语来等待子进程执行完毕。
但是，使用该原语要比在 shell 脚本中使用它复杂一些，
因为每次调用 \co{wait()} 只能等待一个子进程。
我们可以将 \co{wait()} 封装成一个类似于
\cref{lst:toolsoftrade:Using the wait() Primitive}
(\path{api-pthreads.h})所示的 \apipx{waitall()} 函数，
这个 \co{waitall()} 函数的语义与 shell 脚本中的 \co{wait} 一样。
\clnrefrange{loopa}{loopb}的循环每执行一次，就等待一个子进程
执行完毕一次。\Clnref{wait}调用 \co{wait()} 原语开始阻塞直到一个子进程退出，
它返回子进程的进程 ID 号。
如果该进程号为 $-1$，那么说明 \co{wait()} 无法等待子进程执行完毕。
接着\clnref{ECHILD}检查错误码 errno 是否为 \co{ECHILD}，
这代表没有其他子进程了，所以\clnref{break}退出循环。
不然的话，\clnref{perror,exit}打印错误信息并退出。
\end{fcvref}
...
也就是说，共享内存式的并行化可要比 fork-join 式的并行化复杂得多。
...
在一个已有的进程中创建 thread，需要调用
\apipx{pthread_create()} 原语，例如如
\cref{lst:toolsoftrade:Threads Created Via pthread-create() Share Memory}
(\path{pcreate.c})的\clnref{create:a,create:b}所示。
它的第一个参数是指向 \co{pthread_t} 类型的指针，用于存储待创建 thread 的 ID，
第二个 \co{NULL} 参数是一个可选的指向 \co{pthread_attr_t} 结构的指针，
第三个参数是新 thread 将要调用的函数（在本例中为 \co{mythread()}），
最后一个 \co{NULL} 参数是传递给 \co{mythread()} 的参数。
\end{fcvref}
...
在本例中，\co{mythread()} 直接就返回了，但它也可以选择调用
\apipx{pthread_exit()} 结束。
...
\begin{fcvref}[ln:toolsoftrade:pcreate:mythread]
\apipx{pthread_join()} 原语（如\clnref{join}所示）是对
fork-join 中 \apipx{wait()} 的模仿，
它一直阻塞到 \co{tid} 变量指定的 thread 返回。thread 返回有两种方式，
要么调用 \apipx{pthread_exit()} 返回，
要么通过 thread 的调用函数返回。
thread 的返回值存放在 \co{pthread_join()} 的第二个指针类型参数中。
thread 的返回值要么是传给 \co{pthread_exit()} 的返回值，
要么是 thread 调用函数返回的值，这取决于该 thread 如何退出。
\end{fcvref}

\cref{lst:toolsoftrade:Threads Created Via pthread-create() Share Memory}
所示程序显示的输出如下，说明了一个事实：thread 之间共享内存。
...
请读者注意，该程序小心地构造出一个场景，确保一次只有一个
thread 向变量 \co{x} 存储值。
任何一个 thread 为某变量赋值而另一 thread 读取该变量值的场景，
都会产生一种被称为\emph{\IX{data race}}的情况。
因为出现 data race 时 C 语言并不保证结果的合理性，
所以我们需要一些手段来安全地并发访问数据，
比如下一节将会提到的 locking 原语。
...
POSIX 标准允许程序员使用"POSIX locking"来避免 data race。
POSIX locking 包含多种原语，其中最基础的是
\apipx{pthread_mutex_lock()} 和 \apipx{pthread_mutex_unlock()}。
这些原语操作类型为 \apipx{pthread_mutex_t} 的 lock。
该 lock 的静态声明和初始化由
\apipx{PTHREAD_MUTEX_INITIALIZER} 完成，
或者由 \apipx{pthread_mutex_init()} 原语来动态分配并初始化。
本节的示例代码将采用前者。

\co{pthread_mutex_lock()} 原语"获取"指定的 lock，
\co{pthread_mutex_unlock()} "释放"指定的 lock。
因为这些是互斥的加锁/解锁原语，
所以一次只能有一个 thread 在特定时刻"持有"一把特定的 lock。
比如，如果一对 thread 尝试同时获取同一把 lock，那么
其中一个 thread 会先"获准"持有该 lock，
另一个 thread 只能等待第一个 thread 释放该 lock。
...
(\path{lock.c})所使用的代码展示了这种互斥加锁/解锁特性。
\Clnref{lock_a}定义并初始化了一个名为 \co{lock_a} 的 POSIX lock，
\clnref{lock_b}又定义了一个类似的 lock \co{lock_b}。
\Clnref{x}定义并初始化了一个共享变量~\co{x}。
\end{fcvref}

\begin{fcvref}[ln:toolsoftrade:lock:reader_writer:reader]
\Clnrefrange{b}{e}定义了函数 \co{lock_reader()}，
在持有 \co{arg} 指定的 lock 后反复读取共享变量 \co{x} 的值。
\Clnref{cast}将 \co{arg} 转换成一个指向 \co{pthread_mutex_t} 的指针，
该指针随后被传给 \co{pthread_mutex_lock()} 和 \co{pthread_mutex_unlock()}
作为参数。
\end{fcvref}
...
\co{pthread_mutex_t} lock，检查错误值，如果有错误则程序退出。
\Clnrefrange{loop:b}{loop:e}反复检查 \co{x} 的值，
如果值发生改变就将新值打印出来。
\Clnref{sleep}休眠一毫秒，让这个示例代码能在单处理器机器上运行良好。
\Clnrefrange{rel:b}{rel:e}释放 \co{pthread_mutex_t} lock，
接着再一次检查错误值，如果有错误则程序退出。
最后，\clnref{return}返回 \co{NULL}，满足
\co{pthread_create()} 所要求的返回值类型。
\end{fcvref}
...
\cref{lst:toolsoftrade:Demonstration of Exclusive Locks}的
\Clnrefrange{b}{e}是 \co{lock_writer()} 函数，
在持有指定的 \co{pthread_mutex_t} 后周期性地更新共享变量 \co{x} 的值。
和 \co{lock_reader()} 一样，\clnref{cast}将 \co{arg}
转换成指向 \co{pthread_mutex_t} 的指针，
\clnrefrange{acq:b}{acq:e}获取指定的 lock，
\clnrefrange{rel:b}{rel:e}释放这把 lock。
当持有这把 lock 时，\clnrefrange{loop:b}{loop:e}增加共享变量
\co{x} 的值，每次增加后都休眠五毫秒。
最后，\clnrefrange{rel:b}{rel:e}释放这把 lock。
\end{fcvref}
...
展示了一段执行 \co{lock_reader()} 和
\co{lock_writer()} 的代码片段，两个 thread 都使用同一把 lock，即 \co{lock_a}。
\Clnrefrange{cr:reader:b}{cr:reader:e}创建一个执行
\co{lock_reader()} 的 thread，
\clnrefrange{cr:writer:b}{cr:writer:e}创建一个执行
\co{lock_writer()} 的 thread。
\Clnrefrange{wait:b}{wait:e}等待两个 thread 返回。
该代码片段的输出如下：
\end{fcvref}
...
因为两个 thread 都使用同一把 lock，\co{lock_reader()} thread
无法看到 \co{lock_writer()} 在持有 lock 时产生的变量 \co{x} 的
任何中间值。
...
由于两个 thread 使用不同的 lock，它们并不能互斥，因此可以同时执行。
\co{lock_reader()} 这下可以看见
\co{lock_writer()} 存储的共享变量 \co{x} 的中间状态了。
...
虽然 POSIX 互斥 locking 还有很多内容，但本节介绍的这些原语提供了
一个良好的起点，而且在许多情况下实际上已经足够了。
下一节将简要介绍 POSIX reader-writer locking。
...
正如读者所预期的那样，\IXhpl{reader-writer}{lock}是专门为读多写少的场景设计的。
在这种场景中，reader-writer lock 可以提供比\IXh{exclusive}{lock}
大得多的可扩展性，因为 exclusive lock 从定义上已经限制了
任意时刻只能有一个 thread 持有 lock，
而 reader-writer lock 允许任意多数目的 reader 并发持有 lock。
不过我们需要知道 reader-writer lock 到底能增加多少可扩展性。
...
用于同步测试的开始和结束。\co{goflag} 的初始值为 \co{GOFLAG_INIT}，
当所有 reader thread 都启动后设置为 \co{GOFLAG_RUN}，
最后设置为 \co{GOFLAG_STOP} 来终止测试程序运行。
\end{fcvref}

\begin{fcvref}[ln:toolsoftrade:rwlockscale:reader:reader]
\Clnrefrange{b}{e}定义了 \co{reader()}，也就是 reader thread。
\Clnref{atmc_inc}原子地增加 \co{nreadersrunning} 的值，
用来表示该 thread 现在正在运行，
\clnrefrange{wait:b}{wait:e}等待测试开始。
\apik{READ_ONCE()} 原语强迫编译器在循环的每次迭代中都去取 \co{goflag}
的值——否则编译器可能会假定 \co{goflag} 的值没有改变。
\end{fcvref}
...
\clnrefrange{hold:b}{hold:e}在一段指定的时间间隔内持有 lock，
\clnrefrange{rel:b}{rel:e}释放 lock，
\clnrefrange{think:b}{think:e}在重新获取 lock 前等待
一段指定的时间间隔。
\Clnref{count}统计此次 lock 获取。

\Clnref{mov_cnt}将 lock 获取次数的统计值放入该 thread 在
\co{readcounts[]} 数组中对应的元素，\clnref{return}返回，结束该 thread。
\end{fcvref}
...
正如图中所示，reader-writer locking 的可扩展性显然说不上理想，
\IXpl{critical section}较小时尤其如此。
为什么读锁的获取这么慢呢？这应该是由于所有想获取读锁的 thread
都要更新 \co{pthread_rwlock_t} 的数据结构。
因此，一旦全部 448 个 thread 同时尝试以读模式获取 reader-writer lock，
那么这些 thread 必须一个一个更新底层的 \co{pthread_rwlock_t}。
最幸运的 thread 可能几乎立刻就获取读锁了，
最倒霉的 thread 则必须等待其他 447 个 thread 更新后才能获取读锁。
增加 CPU 数量只能让这种情况变得更糟。
还要注意 y 轴是对数刻度。
...
尽管存在这些不足，在很多情况下 reader-writer locking 仍然十分有用，
比如当 reader 必须执行高延迟的文件或网络 I/O 时。
还有一些替代方案，其中一些将在
...
\Cref{fig:toolsoftrade:Reader-Writer Lock Scalability vs. Microseconds in Critical Section}
表明 reader-writer locking 在 critical section 最小时开销最为严重，
考虑到这一点，那么最好能有其他手段来保护极其短小的 critical section。
一种这样的手段使用 \IX{atomic} 操作。
\begin{fcvref}[ln:toolsoftrade:rwlockscale:reader:reader]
我们已经见过一个 atomic 操作了，即
\cref{lst:toolsoftrade:Measuring Reader-Writer Lock Scalability}
\clnref{atmc_inc}上的 \apig{__sync_fetch_and_add()} 原语。
该原语将它的第二个参数的值原子地加到其第一个参数所引用的值上，
并返回原值（在本例中被忽略了）。
如果一对 thread 并发地对同一个变量执行 \co{__sync_fetch_and_add()}，
该变量的值将包含两次加法的结果。
\end{fcvref}

\GNUC\ 编译器提供了许多附加的 atomic 操作，
包括 \apig{__sync_fetch_and_sub()}、
\apig{__sync_fetch_and_or()}、
\apig{__sync_fetch_and_and()}、
\apig{__sync_fetch_and_xor()} 和
\apig{__sync_fetch_and_nand()} 原语，这些操作都返回参数的原值。
如果你需要变量的新值，可以改用
...
经典的\IXacrml{cas}操作由一对原语
\apig{__sync_bool_compare_and_swap()} 和
\apig{__sync_val_compare_and_swap()} 提供。
当变量的原值与指定的值相等时，
这两个原语都会原子地将新值写给指定变量。
第一个原语在操作成功时返回 1，
或者在变量原值不等于指定值时操作失败并返回 0。
第二个原语返回变量的原值，如果等于指定的旧值则表示操作成功。
任何对单一变量进行操作的 atomic 操作都可以用 \acrml{cas}
来实现，从这种意义上说这两个 \acrml{cas} 操作都是通用的，
尽管前面的操作在适用的场景中通常更高效。
\acrml{cas}操作也能作为其他 atomic 操作的基础，
不过这些 atomic 操作通常存在复杂性、可扩展性和性能等
诸方面的问题~\cite{MauriceHerlihy90a}。
...
它限制编译器和 CPU 对指令乱序执行的优化，如
\cref{chp:Advanced Synchronization: Memory Ordering}中所讨论的。
在某些情况下，只需限制编译器对指令的优化就足够了，CPU 的优化可以保留，
此时可以使用 \apik{barrier()} 原语。
\begin{fcvref}[ln:toolsoftrade:lock:reader_writer:reader]
在某些情况下，只需确保编译器不会优化掉某个特定的内存读取就行了，
...
不幸的是，远在各标准委员会着手处理之前，thread 操作、locking 原语和
atomic 操作就已经被广泛使用了。
因此，这些操作有很多种变体。
用汇编语言实现这些操作也十分常见，
不仅因为历史原因，还因为可以在某些特定场合获得更好的性能。
例如，\GCC 的 \co{__sync_} 系列原语都提供完整的内存顺序执行语义，
这让许多开发者有动力实现自己的函数，来满足许多不需要
内存顺序执行语义的场景。
...
直到 2011 年，C 标准才为共享变量的并发读写访问定义了语义。
然而，并发 C 代码至少在二十五年前就已经在编写了~\cite{Beck85,Inman85}。
这引出了一个问题：在那个遥远的 C11 之前的时代，前辈们是怎么做的。
简短的回答是"他们冒着危险生活"。
...
至少如果他们使用的是 2021 年的编译器的话，确实会很危险。
在（比方说）1990 年代早期，编译器做的优化还比较少，
部分原因是编译器开发者较少，部分原因是那个时代的内存相对较小。
尽管如此，问题确实出现了，如
...
\item[代码重排序（Code reordering）] 是一种常见的编译技术，用于
合并公共子表达式、减少寄存器压力，以及
...
根据经验法则，应该在能胜任的工具中选择最简单的一个。
如果可以，尽量使用串行编程。
如果达不到要求，那么使用 shell 脚本来实现并行化。
如果 shell 脚本的 \co{fork()}/\co{exec()} 开销
（在 Intel Core Duo 笔记本上最简单的 C 程序大约需要 480 微秒）
太大，那么使用 C 语言的 \apipx{fork()} 和 \apipx{wait()} 原语。
如果这些原语的开销（最小的子进程也需要大约 80 微秒）仍然太大，
那么你可能需要使用 POSIX threading 原语，
选择合适的 locking 和/或 atomic 操作原语。
如果 POSIX threading 原语的开销（一般低于微秒级）
仍然太大，那么就需要
\cref{chp:Deferred Processing}中引入的原语了。
当然，实际的开销并不只取决于你的硬件，
更重要的是取决于如何使用这些原语。
此外，请一定记住，在考虑共享内存多 thread 执行方案时
不要忘了进程间通信和消息传递这个选项，
特别是当你的代码充分利用了
