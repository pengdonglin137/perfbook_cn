...
\QuickQuizChapter{chp:Data Ownership}{数据所有权（data ownership）}{qqzowned}
%
\Epigraph{那是我的，我告诉你。
	  我自己的。
	  我的宝贝。
...
避免locking带来的同步开销的最简单方法之一，是在各个thread之间
（或者对于内核来说，各个CPU之间）分配数据，
以便让每一份数据仅由一个thread访问和修改。
...
Data ownership的方法有很多。
\Cref{sec:owned:Multiple Processes}介绍了data ownership的极端情况，
即每个thread拥有自己的私有地址空间。
\Cref{sec:owned:Partial Data Ownership and pthreads}介绍了
另一方向的极端，即数据完全共享，但不同的thread拥有对数据的不同访问权限。
\Cref{sec:owned:Function Shipping}描述了function shipping，
这是一种允许其他thread间接访问特定thread所拥有数据的方法。
\Cref{sec:owned:Designated Thread}描述了如何为指定thread
分配特定功能及其相关数据的所有权。
\Cref{sec:owned:Privatization}讨论了如何通过将共享数据的算法
转换为使用data ownership来提升性能。
最后，\cref{sec:owned:Other Uses of Data Ownership}列举了
几个将data ownership视为头等公民的软件环境。
...
此示例并行运行两个\co{compute_it}程序实例，
因为进程之间不共享内存。
因此，进程中的数据都由该进程拥有，
上面例子中几乎全部数据都归一个进程所有。
这种方法几乎完全消除了同步开销。
这种极度简单和最佳性能的组合显然是相当有吸引力的。
...
\Cref{sec:owned:Partial Data Ownership and pthreads}将讨论在
shared-memory并行程序中使用data ownership。
...
但做了一点改变。
不允许thread修改其他thread拥有的数据，
但允许thread读取这些数据。
总之，shared memory的使用允许更细粒度的所有权和访问权限概念。

例如，考虑
\cref{lst:count:Per-Thread Statistical Counters}（位于
\cpageref{lst:count:Per-Thread Statistical Counters}）
中所示的per-thread统计计数器（statistical counter）实现。
其中\co{inc_count()}只更新相应thread实例的
\co{counter}，
...
在这个算法中，每个thread的cache完全归该thread所有。
...
上一节描述了一种data ownership的弱形式，其中thread需要更改
其他thread的数据。
这可以认为是将数据带给需要它的函数。
另一种方法是将函数发送给数据。
...
\co{flush_local_count_sig()}函数是一个信号处理函数，
这里作为被转移的函数。
\co{flush_local_count()}中的\co{pthread_kill()}函数
发送信号——送出函数——然后等待被转移的函数执行完毕。
这个被转移的函数因为要与并发执行的\co{add_count()}
或\co{sub_count()}函数交互，增加了一些复杂度（见
...
前面几节描述了允许每个thread保留自己的数据副本或部分数据副本的方法。
相比之下，本节描述了一种分解功能的方法，
其中特定的指定thread拥有完成其工作所需数据的权限。
...
这个\co{eventual()} thread周期性地将per-thread计数值
拉入全局计数器，从而如其名称所示，
最终将全局计数器收敛于实际值。
\end{fcvref}
...
对于shared-memory并行程序，一种提升性能和可扩展性的方法是
将共享数据转换为由特定thread拥有的私有数据。

一个很好的例子见于\cref{sec:SMPdesign:Dining Philosophers Problem}
中一道Quick Quiz的答案，
该答案使用私有化解决哲学家就餐问题，
这种方法具有比标准教科书解法更好的性能和可扩展性。
原来的问题是五位哲学家围坐在桌旁，
每对相邻哲学家之间有一把叉子，至多允许两位哲学家同时进餐。

我们可以通过提供额外的五把叉子来简单地私有化这个问题，
这样每位哲学家都有自己的私人叉子。
这允许所有五位哲学家同时进餐，
也大大减少了传播某些类型疾病的机会。
...
中所示的简单限值计数器（limit counter）。
在这个算法的例子中，thread可以读取彼此的数据，
但只允许更新自己的数据。
我们快速回顾一遍算法，唯一的跨thread访问只在
\co{read_count()}的求和循环中。
如果去除这个循环，这里将变成更高效的纯data ownership模式，
但代价是\co{read_count()}的结果不太准确。
...
总之，在并行程序员的工具箱中，私有化是一个强大的工具，
但必须小心使用。
就像其他每一个同步原语一样，它有可能带来复杂性，
同时降低性能和可扩展性。
...
当数据可以被分割，使得很少或没有需要跨thread访问或更新的地方时，
data ownership效果最好。
幸运的是，这种情况很常见，并且在各种并行编程环境中都存在。
...
Data ownership也许是最不起眼的同步机制。
当使用得当时，它能提供无与伦比的简洁性、性能和可扩展性。
也许正是它的简洁性使它没有得到应有的尊重。
希望对data ownership的微妙之处和威力的认识，
能够带来更多的尊重，更不用说那些由于复杂性的降低
而带来的性能和可扩展性的提升。
