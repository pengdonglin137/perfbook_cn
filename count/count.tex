...
\EQuickQuiz{
	...
	提示：
	更新计数器的操作必须极快，但由于计数器大约每五百万次更新
	才被读取一次，读取计数器的操作可以相当慢。
	此外，读取的值通常不需要非常精确——毕竟，既然计数器
...
这里，我们在\clnref{counter}处有一个计数器，在\clnref{inc}处
...
当计数器频繁被读取却几乎不递增时，这种方法快得令人惊叹。
在小型系统上，性能也非常出色。

不过让人扫兴的是，这种方法可能丢失计数。
在我的六核x86笔记本电脑上，一次短暂运行调用了\co{inc_count()}
285,824,000次，但计数器的最终值仅为35,385,525。
...
\QuickQuizB{
	...
	虽然\co{++}运算符\emph{可能}是原子的，但除非将其应用于
	...
	volatile类型转换，它们告诉编译器该位置很可能是一个
	MMIO设备寄存器。
	因为MMIO寄存器不被缓存，编译器假设递增操作是原子的
	将是不明智的。
}\QuickQuizEndB
%
\QuickQuizE{
...
它也比非原子递增慢了二十多倍。\footnote{
	有趣的是，非原子递增计数器的速度反而比原子递增更快。
	当然，如果你的唯一目标是让计数器快速增长，
	更简单的方法是直接给计数器赋一个大值。
...
对应一个线程（假设数组已按cache line对齐并且填充过了，以避免伪共享）。
...
\Clnref{define}定义了一个包含每线程计数器集合的数组，
...
\QuickQuiz{
	...
	获取更多信息。
	一种有问题的优化是对\co{read_count()}函数的连续调用
...
并使用\apipf{per_thread()}原语获取指定线程的计数器。
...
换句话说，per-CPU计数器的更新不会像单个全局计数器的
原子递增那样受到慢光和大原子的困扰。\footnote{
...
\QuickQuiz{
	...
	从$N$个计数器中的每个读取均匀分布在时间段$\Delta$内的情况。
	在$N$次读取之间将有$N+1$个持续时间为$\frac{\Delta}{N+1}$
	的间隔。
	递增速率$r$预计均匀分布在$N$个计数器上，
	每个计数器每单位时间$\frac{r}{N}$次递增。
	从最后一个线程的计数器读取后的延迟引起的误差
	...
	到目前为止，我们一直在考虑一个只增不减的计数器。
	如果计数器值以每单位时间$r$次的速率变化，
	但可以是任一方向，我们应该期望误差会减少。
	然而，最坏情况不变，因为虽然计数器\emph{可以}
	...
	递减被递增抵消，因此计数器在负方向移动
	$-1+2\left(1-f\right)$，
	简化为$1-2f$，因此计数器在两种情况下
	平均每次操作移动$1-2f$。
	因此，计数器的长期移动由$\left( 1-2f \right) r$给出。
...
（\path{count_end.c}）所示，
可以使用它来实现一个统计计数器，这个计数器不仅能扩展，
而且相对于简单的非原子递增来说，对递增方几乎不产生性能损失。
...
\co{counter}是每线程计数器变量，
\co{counterp[]}数组允许线程访问彼此的计数器，
...
这种方法让更新方的性能几乎和非原子计数一样，并且也能线性扩展。
另一方面，并发的读取方争用一个全局锁，因此性能不佳，扩展能力也很差。
但是，对于统计计数器来说这不是问题，
因为统计计数器总是在增加计数，很少读取计数。
另外，由于给定线程的per-thread变量在该线程退出时消失，
本方法比基于数组的方案要复杂一些。

\QuickQuiz{
	好吧，但Linux内核在读取每CPU计数器的
	聚合值时不需要获取锁。
	那为什么用户空间代码需要这样做？
}\QuickQuizAnswer{
...
\end{listing}
	...
	此代码的分析留作读者的练习，
	但请注意它需要对\path{counttorture.h}计数器评估方案
	进行调整。
...
我们通过维护一个全局计数器来利用最终一致性。
但是因为写者只操纵它自己的per-thread计数器，
所以单独有一个线程负责将per-thread计数器的计数值传递给全局计数器，
读者只是简单地访问全局计数器的值。
如果写者正在更新计数，读者读出的值将不是最新的，
不过一旦写者更新完毕，全局计数器最终会回归正确的值——
这就是为什么这种方法被称为最终一致性。
...
展示了跟踪计数器值的每线程变量和全局变量，
\clnref{stopflag}展示了用于协调终止的\co{stopflag}
（用于我们希望以准确计数器值终止程序的情况）。
...
\QuickQuizB{
	...
	\co{inc_count()}不需要使用原子指令？
	毕竟，我们现在有多个线程访问每线程计数器！
}\QuickQuizAnswerB{
	因为两个线程中的一个只进行读取，并且由于
	变量是对齐的且是机器字大小的，非原子指令即可满足。
	话虽如此，使用了\co{READ_ONCE()}宏来防止
	可能阻止计数器更新对\co{eventual()}可见的
	编译器优化。\footnote{
...
\QuickQuizM{
	...
	\co{eventual()}函数中的单个全局线程不会像全局锁一样
	成为严重的瓶颈吗？
}\QuickQuizAnswerM{
	在这种情况下不会。
	相反会发生的是，随着线程数量的增加，
	\co{read_count()}返回的计数器值估计将变得更加不准确。
}\QuickQuizEndM
%
\QuickQuizM{
	\cref{lst:count:Array-Based Per-Thread Eventually Consistent Counters}中
...
\QuickQuizM{
	...
	当然，可以做出其他权衡。
	例如，可以创建单个线程来处理所有最终一致性计数器，
	这将把开销限制在单个CPU上，
	但随着计数器数量的增加，更新到读取的延迟会增加。
	或者，该单个线程可以跟踪计数器的更新速率，
	更频繁地访问频繁更新的计数器。
	此外，处理计数器的线程数量可以设置为
...
实现限值计数器的一种可能方法是将10,000的限制值平均划分
...
简而言之，在很多重要的场景下我们都不能将计数器完全分区。
在\cref{sec:count:Statistical Counters}中，考虑到对计数器的分区才是
...
per-thread \co{counter}完美地分区了计数器值，
而\co{global_count}保存完整的全局数值。

对于限值计数器，我们也可以使用这种方法的变体，
部分地分区计数器。
...
虽然这个办法仍然需要全局锁，但该锁只需要每五次递增操作
获取一次，这就大大降低了该锁的\IXalt{竞争}{lock contention}程度。
...
包含聚合计数器的上限，
\clnref{globalcount}上的\co{globalcount}变量是全局计数器。
