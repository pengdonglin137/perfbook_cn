...
计数也许是计算机能做的最简单、最自然的事情。
然而，在大型共享内存多处理器上高效且可扩展地计数可能颇具挑战性。
此外，计数这一底层概念的简洁性使我们能够在不受复杂数据结构或
精巧同步原语干扰的情况下，探索并发的基本问题。
因此，计数为并行编程提供了一个绝佳的入门途径。

本章涵盖了若干特殊情况，这些情况下存在简单、快速且可扩展的
计数算法。
...
如果你进行大量读取而几乎不做递增操作，这种方法还具有极快速度的
额外优势，而且在小型系统上性能非常出色。
...
虽然近似计算在计算领域确实占有重要地位，但丢失87\pct\ 的计数
未免过于夸张了。
...
atomic递增的性能随CPU和线程数量的增加而变慢也不足为奇，
...
从另一个角度来看全局atomic递增，请考虑
\cref{fig:count:Data Flow For Global Atomic Increment}。
为了让每个CPU有机会递增给定的全局变量，
包含该变量的\IX{cache line}必须在所有CPU之间流转，
如红色箭头所示。
由于光速有限和原子大小非零，这种流转将花费大量时间，
这些物理定律在\cref{sec:cpu:Hardware Free Lunch?}中已经讨论过。
这种缓慢的流转反过来导致了
\cref{fig:count:Atomic Increment Scalability on x86}中
所见的糟糕性能，
或者如\cref{fig:count:Waiting to Count}所更生动描绘的那样。
以下章节讨论高性能计数，它避免了此类流转所固有的延迟。
...
本节涵盖statistical counters的常见特殊情况，其中计数值
被极其频繁地更新，而很少甚至从不被读取。
这些将被用来解决\QuickQuizRef{\QcountQstatcnt}中提出的
网络数据包计数问题。

\subsection{设计}

Statistical counting通常通过为每个线程（或在内核中运行时为每个CPU）
提供一个counter来处理，这样每个线程更新自己的counter，
正如\cref{sec:toolsoftrade:Per-CPU Variables}
（\cpageref{sec:toolsoftrade:Per-CPU Variables}）中所预示的那样。
counter的聚合值通过简单地对所有线程的counter求和来读取，
利用加法的交换律和结合律。
...
提供每线程变量的一种方法是分配一个数组，
每个线程一个元素（最好是cache对齐和填充的，以避免false sharing）。
...
\Clnref{define}定义了一个包含每线程counter集合的数组，
...
编译器优化。
举一个例子，编译器有权将即将被存储的位置
用作临时存储，从而在执行所需存储之前写入
实际上就是垃圾的内容。
这当然可能会让试图读取计数值的任何东西感到困惑。
...
这种方法随着调用\co{inc_count()}的更新线程数量的增加而线性扩展。
如\cref{fig:count:Data Flow For Per-Thread Increment}中每个CPU上的
绿色箭头所示，原因是每个CPU可以快速递增
其线程的变量，而不需要任何昂贵的跨系统通信。
换句话说，每CPU counter的更新不会像单个全局counter的
...
然而，许多实现提供了更廉价的每线程数据机制，
且不受任意数组大小限制。
这是下一节的主题。

\subsection{基于每线程变量的实现}
...
（\path{count_end.c}）所示，
可以使用它来实现一个不仅扩展性好且避免了任意线程数量限制，
而且与简单的非atomic递增相比，对递增方几乎
不产生性能损失的statistical counter。
...
\co{counter}是每线程counter变量，
\co{counterp[]}数组允许线程访问彼此的counter，
\co{finalcount}在各个线程退出时累积总数，
\co{final_mutex}用于协调累积counter总值的线程和正在退出的线程。
\end{fcvref}
...
\QuickQuiz{
	...
	Linux内核可以忽略的挑战。
	当用户级线程退出时，其每线程变量全部消失，
	这使每线程变量的访问变得复杂，
	特别是在用户级RCU出现之前
	（参见\cref{sec:defer:Read-Copy Update (RCU)}）。
	相比之下，在Linux内核中，当CPU离线时，
	该CPU的每CPU变量仍然保持映射和可访问。

	类似地，当创建新的用户级线程时，
	其每线程变量突然出现。
	相比之下，在Linux内核中，所有每CPU变量
...
更新方使用的\co{inc_count()}函数相当简单，
如\clnrefrange{b}{e}所示。
\end{fcvref}

\begin{fcvref}[ln:count:count_end:whole:read]
读取方使用的\co{read_count()}函数稍微复杂一些。
\Clnref{acquire}获取一个锁以排斥正在退出的线程，
\clnref{release}释放它。
\Clnref{sum:init}将sum初始化为已退出线程累积的计数值，
\clnrefrange{loop:b}{loop:e}对当前运行线程正在累积的
计数值求和。
...
该函数简单地设置此线程在\co{counterp[]}数组中的元素，
使其指向自己的每线程\co{counter}变量。
\end{fcvref}
...
\Clnref{add}将此线程的\co{counter}加到全局\co{finalcount}上，
然后\clnref{NULL}将其\co{counterp[]}数组条目设为\co{NULL}。
随后的\co{read_count()}调用将在全局\co{finalcount}中看到
退出线程的计数，并在遍历\co{counterp[]}数组时跳过该退出线程，
从而获得正确的总数。
\end{fcvref}

这种方法使更新方获得与非atomic加法几乎完全相同的性能，
并且也是线性扩展的。
另一方面，并发读取争用单个全局锁，
因此性能差且扩展性极差。
然而，对于statistical counters来说这不是问题，
因为递增频繁而读取几乎从不发生。
当然，由于给定线程的每线程变量在该线程退出时消失，
这种方法比基于数组的方案复杂得多。

\QuickQuiz{
	好吧，但Linux内核在读取每CPU counter的
	聚合值时不需要获取锁。
	那为什么用户空间代码需要这样做？
}\QuickQuizAnswer{
	记住，Linux内核的每CPU变量始终是可访问的，
	即使相应的CPU离线——即使相应的CPU从未存在且永远不会存在。

\begin{listing}
\input{CodeSamples/count/count_tstat@whole.fcv}
\caption{带无锁求和的Per-Thread Statistical Counters}
...
基于数组和基于\co{_Thread_local}的方法都提供了出色的
更新端性能和可扩展性。
然而，这些优势导致大量线程时读取端的开销很大。
下一节展示了一种在保持更新端可扩展性的同时
降低读取端开销的方法。

\subsection{最终一致性实现}
\label{sec:count:Eventually Consistent Implementation}

在保持更新端可扩展性的同时大幅提高读取端性能的一种方法
是放松一致性要求。
前一节中的计数算法保证返回的值介于
\co{read_count()}执行开始附近理想counter的值和
执行结束附近的值之间。
\emph{最终一致性}~\cite{WernerVogels:2009:EventuallyConsistent}
提供了更弱的保证：
在没有\co{inc_count()}调用的情况下，
\co{read_count()}调用将最终返回准确的计数值。

我们通过维护一个全局counter来利用最终一致性。
但是，更新方仅操作其每线程counter。
一个单独的线程负责将每线程counter的计数值
传输到全局counter。
读取方只需访问全局counter的值。
如果更新方处于活动状态，读取方使用的值将过时，
然而，一旦更新停止，全局counter最终将收敛到
真实值——因此这种方法属于最终一致性。
...
这种方法提供了极快的counter读取，同时仍然支持
线性的counter更新可扩展性。
然而，这种出色的读取端性能和更新端可扩展性
是以运行\co{eventual()}的额外线程为代价的。
\end{fcvref}
...
这三种实现表明，尽管运行在并行机器上，
仍然可以为statistical counters获得接近单处理器的性能。
...
\QuickQuizE{
	...
	更新方会递增其每线程变量，但当它达到某个预定限制时，
	将其atomic地加到全局变量上，然后将每线程变量清零。
...
近似结构分配限制问题所述，
假设你需要维护已分配结构数量的计数，
以便在使用中的结构数量超过限制（本例中为10,000）时
拒绝任何分配。
进一步假设这些结构是短生命周期的，此限制很少被超过，
且此限制是近似的——即偶尔超出某个有界数量
或偶尔未达到某个有界数量都是可以接受的。
如果你需要限制是精确的，参见\cref{sec:count:Exact Limit Counters}。

\subsection{设计}

limit counter的一种可能设计是将10,000的限制除以线程数量，
给每个线程一个固定的结构池。
例如，给定100个线程，每个线程将管理自己的100个结构的池。
这种方法简单，在某些情况下效果很好，但它无法处理
常见的情况：一个结构由一个线程分配而由另一个线程
释放~\cite{McKenney93}。
一方面，如果给定线程为其释放的任何结构记账，
那么做大部分分配的线程会耗尽结构，
而做大部分释放的线程则拥有大量无法使用的额度。
另一方面，如果释放的结构记在分配它们的CPU名下，
则需要CPU操作彼此的counter，这将需要昂贵的
\IX{atomic}指令或其他线程间通信手段。\footnote{
	话虽如此，如果每个结构总是由分配它的
	同一CPU（或线程）释放，那么这种简单的分区方法
	效果极好。}

简而言之，对于许多重要的工作负载，我们无法完全分区counter。
鉴于分区counter正是
\cref{sec:count:Statistical Counters}中讨论的三种方案
带来出色更新端性能的原因，这可能令人悲观。
然而，\cref{sec:count:Eventually Consistent Implementation}中
提出的最终一致性算法提供了一个有趣的提示。
回忆一下，该算法保持两套账目：
用于更新方的每线程\co{counter}变量和用于读取方的
\co{global_count}变量，
由一个\co{eventual()}线程周期性地更新\co{global_count}
使其与每线程\co{counter}的值最终一致。
每线程\co{counter}完美分区了counter值，
而\co{global_count}保持完整值。

对于limit counters，我们可以使用这个主题的变体，
即\emph{部分分区}counter。
例如，考虑四个线程，每个线程不仅有一个
每线程\co{counter}，还有一个每线程最大值
（称之为\co{countermax}）。

但如果给定线程需要递增其\co{counter}，
而\co{counter}已等于其\co{countermax}，该怎么办？
这里的技巧是将该线程\co{counter}值的一半
移到\co{globalcount}，然后递增\co{counter}。
例如，如果给定线程的\co{counter}和\co{countermax}
变量都等于10，我们执行以下操作：

\begin{enumerate}
\item	获取全局锁。
\item	向\co{globalcount}加5。
\item	为了平衡这次加法，从此线程的\co{counter}中减去5。
\item	释放全局锁。
\item	递增此线程的\co{counter}，得到值6。
\end{enumerate}

虽然此过程仍然需要全局锁，但该锁只需要每五次递增操作
获取一次，大大降低了该锁的\IXalt{竞争}{lock contention}程度。
我们可以通过增加\co{countermax}的值将此竞争降低到任意水平。
然而，增加\co{countermax}值的相应代价是
\co{globalcount}的准确性降低。
要看到这一点，请注意在四CPU系统上，如果\co{countermax}
等于10，\co{globalcount}的误差最多为40次。
相比之下，如果\co{countermax}增加到100，\co{globalcount}
的误差可能高达400次。

这引出了一个问题：我们究竟多关心\co{globalcount}
偏离counter聚合值的程度，其中聚合值是
\co{globalcount}和每个线程的\co{counter}变量之和。
这个问题的答案取决于聚合值距counter限制
（称之为\co{globalcountmax}）还有多远。
这两个值之间的差距越大，\co{countermax}
就可以越大而不冒超过\co{globalcountmax}限制的风险。
这意味着给定线程的\co{countermax}变量值可以
基于这个差距来设置。
当远离限制时，\co{countermax} 每线程变量设置为较大值
以优化性能和可扩展性，
而接近限制时，这些变量设置为较小值
以最小化对\co{globalcountmax}限制检查的误差。

这种设计是\emph{parallel快速路径}的一个例子，
这是一种重要的设计模式，其中常见情况不执行昂贵指令
且线程之间没有交互，但偶尔也使用更保守设计的
（且开销更高的）全局算法。
...
展示了此实现使用的每线程和全局变量。
每线程\co{counter}和\co{countermax}变量分别是
...
\co{counterp[]}数组的每个元素引用对应线程的\co{counter}变量，
最后，\co{gblcnt_mutex} spinlock保护所有全局变量，
换句话说，除非线程已获取\co{gblcnt_mutex}，
否则不允许访问或修改任何全局变量。
...
该函数清除线程本地变量，并根据需要调整全局变量，
从而简化全局处理。
（但不要只听\emph{我}说，请自己尝试编写！）
...
\Clnref{checklocal}检查每线程counter是否能容纳此次减法，
如果能，\clnref{sub}执行减法，
...
当聚合值接近零时，这种类型的counter相当快，
由于\co{add_count()}和\co{sub_count()}快速路径中的
比较和分支会有一些开销。
然而，使用每线程\co{countermax}储备意味着
即使counter的聚合值远未接近\co{globalcountmax}，
\co{add_count()}也可能失败。
类似地，即使counter的聚合值远未接近零，
\co{sub_count()}也可能失败。

在许多情况下，这是不可接受的。
即使\co{globalcountmax}打算作为近似限制，
通常也有一个可容忍的近似程度限制。
限制近似程度的一种方法是对每线程\co{countermax}实例
的值施加上限。
这个任务在下一节中进行。
...
它们对每线程\co{countermax}变量强制执行
\co{MAX_COUNTERMAX}限制。
\end{fcvref}

\subsection{近似限值计数器讨论}

这些更改大大减少了前一版本中看到的限制不准确性，
但带来了另一个问题：
任何给定的\co{MAX_COUNTERMAX}值都将导致取决于工作负载的
一部分访问偏离快速路径。
随着线程数量的增加，非快速路径执行将成为
性能和可扩展性问题。
然而，我们将暂缓讨论此问题，转而关注
具有精确限制的counter。
...
精确结构分配限制问题，
我们需要一个能精确判断其限制何时被超过的limit counter。
实现这样一个limit counter的一种方法是
让已保留计数的线程交出这些计数。
一种做法是使用\IX{atomic}指令。
...
不幸的是，如果一个线程要安全地从另一个线程移除计数，
两个线程都需要atomic地操作该线程的
\co{counter}和\co{countermax}变量。
通常的做法是将这两个变量合并为单个变量，
例如，给定一个32位变量，使用高16位表示\co{counter}，
低16位表示\co{countermax}。
...
\clnref{release}释放它。
\Clnref{initsum}将局部变量\co{sum}初始化为\co{globalcount}的值，
\clnrefrange{loop:b}{loop:e}的循环将每线程counter
...
是否允许任何每线程计数，如果不允许，\clnref{return:n}返回。
...
这是第一个真正允许counter一直运行到其任一限制的实现，
但代价是在快速路径中添加了atomic操作，
这在某些系统上会显著减慢快速路径。
虽然某些工作负载可能容忍这种减速，
但值得寻找具有更好写入端性能的算法。
一种这样的算法使用signal handler从其他线程窃取计数。
因为signal handler在被信号中断的线程的上下文中运行，
atomic操作不是必需的，如下一节所示。

\QuickQuiz{
	但signal handler可以在运行时被迁移到其他CPU上。
	这种可能性不需要atomic指令和内存屏障来
...
即使每线程状态现在仅由相应线程操作，
仍然需要与signal handler进行同步。
此同步由\cref{fig:count:Signal-Theft State Machine}
所示的状态机提供。
...
发现本地线程的计数与全局计数的组合无法容纳请求时，
相应的慢速路径将每个线程的\co{theft}状态设置为REQ
（除非该线程没有计数，此时直接转换到READY）。
只有持有\co{gblcnt_mutex} lock的慢速路径才允许
从IDLE状态转换，如绿色所示。\footnote{
	对于持有黑白版本本书的读者，
	IDLE和READY是绿色，REQ是红色，ACK是蓝色。}
然后慢速路径向每个线程发送信号，
相应的signal handler检查对应线程的\co{theft}和
\co{counting}变量。
如果\co{theft}状态不是REQ，则signal handler
不允许改变状态，因此简单地返回。
否则，如果\co{counting}变量已设置（表示当前线程的
快速路径正在进行中），signal handler将\co{theft}状态
设置为ACK，否则设为READY。

如果\co{theft}状态为ACK，
只有快速路径才允许改变\co{theft}状态，如蓝色所示。
当快速路径完成时，它将\co{theft}状态设置为READY。

一旦慢速路径看到线程的\co{theft}状态为READY，
慢速路径就被允许窃取该线程的计数。
然后慢速路径将该线程的\co{theft}状态设置为IDLE。
...
\QuickQuizB{
	...
	因为除非持有\co{gblcnt_mutex} 锁，
	否则不允许其他线程更改其\co{countermax}变量的值。
	但调用者已获取此锁，因此其他线程不可能持有它，
...
Signal-theft实现在我的六核x86笔记本电脑上
比atomic实现快八倍以上。
它总是更好的选择吗？

Signal-theft实现在Pentium-4系统上将是极其可取的，
鉴于其缓慢的atomic指令，但基于老式80386的
Sequent Symmetry系统使用atomic实现较短的路径长度
会做得更好。
然而，这种增加的更新端性能是以更高的读取端开销为代价的：
那些POSIX信号不是免费的。
如果极致性能至关重要，你需要在应用程序将要部署的
系统上对两者进行测量。
...
可移除I/O设备访问计数问题所述，
访问次数是无关紧要的，除了在某人实际尝试
移除设备的罕见情况下。

这个问题的一个简单解决方案是给counter添加一个大的``偏置''
（例如十亿），以确保值距零足够远，
使counter能够高效运行。
当有人想要移除设备时，从counter值中减去此偏置。
计数最后几次访问将相当低效，
但重要的是之前的许多访问都以全速被计数了。
...
虽然偏置counter可以非常有用，但它只是
\cpageref{chp:Counting}上提出的可移除I/O设备
访问计数问题的部分解决方案。
当试图移除设备时，我们不仅必须知道当前I/O访问的
精确数量，还需要防止任何未来的访问开始。
一种实现方法是在更新counter时读获取读写锁，
在检查counter时写获取同一读写锁。
执行I/O的代码可能如下：
...
\QuickQuiz{
	这太荒谬了！
	我们在\emph{读}获取读写锁来\emph{更新}counter？
	你在搞什么？
}\QuickQuizAnswer{
	奇怪，也许，但确实如此！
	几乎足以让你觉得``读写锁''这个名字起得不好，不是吗？
}\QuickQuizEnd
...
\Clnref{acq}写获取锁，\clnref{rel}释放它。
...
本章介绍了传统计数原语的可靠性、性能和可扩展性问题。
C语言的\co{++}运算符不保证在多线程代码中可靠运行，
而对单个变量的atomic操作既不具备良好的性能也无法很好地扩展。
因此，本章介绍了多种在某些特殊情况下
性能和扩展性极好的计数算法。
...
性能和扩展性极好的计数算法。
但我们的并行计数必须局限于特殊情况吗？
拥有一个在所有情况下都高效运行的通用算法不是更好吗？
下一节将探讨这些问题。

\subsection{并行计数的专门化}
\label{sec:count:Parallel Counting Specializations}

这些算法仅在各自的特殊情况下才能良好工作，
这一事实可能被视为并行编程整体的一个主要问题。
毕竟，C语言的\co{++}运算符在单线程代码中工作得很好，
而且不仅在特殊情况下，在一般情况下也是如此，对吧？

这种推理确实包含一粒真理，但本质上是误导的。
问题不在于并行性本身，而在于可扩展性。
要理解这一点，首先考虑C语言的\co{++}运算符。
事实上它\emph{并不}在一般情况下工作，
只在有限范围的数字内工作。
如果你需要处理1,000位十进制数，
C语言的\co{++}运算符对你不起作用。
...
B树？
基数树？
还是允许存储和查询数据的众多其他数据结构和环境之一？
这取决于你需要做什么、需要多快完成、以及数据集有多大
——即使在串行系统上也是如此。

类似地，如果你需要计数，你的解决方案将取决于
你需要处理多大的数字、多少个CPU需要并发操作
同一个数字、该数字如何使用，以及你需要什么级别的
性能和可扩展性。
...
哥伦比亚河河口，这种设计也不适合
承载混凝土运输车的桥梁。
简而言之，正如桥梁设计必须随跨度和载荷的增加而改变，
软件设计也必须随CPU数量的增加而改变。
话虽如此，自动化这个过程会很好，
使软件适应硬件配置和工作负载的变化。
事实上，已经有一些关于这种自动化的研究
~\cite{Appavoo03a,Soules03a}，
Linux内核确实进行了一些启动时的重新配置，
包括有限的二进制重写。
随着主流系统上CPU数量的持续增加，
这种适应将变得越来越重要。

简而言之，如\cref{chp:Hardware and its Habits}所讨论的，
物理定律对并行软件的约束与对桥梁等
机械制品的约束同样确定。
这些约束迫使专门化，尽管在软件的情况下，
有可能自动选择适合当前硬件和工作负载的专门化方案。
...
本章的开头段落承诺我们对计数的研究将为并行编程
提供绝佳的入门。
本节明确建立了本章经验教训与后续章节内容之间的联系。

本章的示例表明，一个重要的可扩展性和性能工具是\emph{分区}。
Counter可以是完全分区的，
如\cref{sec:count:Statistical Counters}中讨论的
statistical counters，
也可以是部分分区的，
如\cref{sec:count:Approximate Limit Counters,%
sec:count:Exact Limit Counters}中讨论的limit counters。
分区将在\cref{chp:Partitioning and Synchronization Design}中
更深入地讨论，
特别是部分并行化将在\cref{sec:SMPdesign:Parallel Fastpath}中讨论，
在那里它被称为\emph{parallel快速路径}。
...
部分分区的计数算法使用锁来保护全局数据，
锁是\cref{chp:Locking}的主题。
相比之下，已分区的数据往往完全由相应线程控制，
因此根本不需要任何同步。
这种\emph{data ownership}将在
\cref{sec:SMPdesign:Data Ownership}中介绍，
并在\cref{chp:Data Ownership}中更详细地讨论。
...
讨论的最终一致性statistical counter表明，
延迟活动（在此情况下是更新全局counter）
可以提供显著的性能和可扩展性优势。
这种方法允许常见情况下的代码使用
比其他方式便宜得多的同步操作。
\Cref{chp:Deferred Processing}将研究延迟可以改善性能、
可扩展性甚至实时响应的多种其他方式。
...
\begin{enumerate}
\item	分区促进性能和可扩展性。
\item	部分分区——即仅应用于常见代码路径的分区——
	效果几乎一样好。
\item	部分分区可以应用于代码（如
	\cref{sec:count:Statistical Counters}的statistical
	counters的分区更新和非分区读取），也可以跨时间应用
...
简而言之，正如本章开头所指出的，
计数的简洁性使我们能够在不受复杂同步原语或
精巧数据结构干扰的情况下探索许多基本的并发问题。
这些同步原语和数据结构将在后续章节中介绍。
