...
\Clnref{acquire}获取一个lock以排斥正在退出的线程，
\clnref{release}释放它。
...
使其指向自己的per-thread \co{counter}变量。
\end{fcvref}
...
因为递增频繁而读取几乎从不发生。
当然，由于给定线程的per-thread变量在该线程退出时消失，
这种方法比基于数组的方案复杂得多。
...
但是，更新方仅操作其per-thread counter。
一个单独的线程负责将per-thread counter的计数值
传输到全局counter。
读取方只需访问全局counter的值。
...
回忆一下，该算法保持两套账目：
用于更新方的per-thread \co{counter}变量和用于读取方的
\co{global_count}变量，
由一个\co{eventual()}线程周期性地更新\co{global_count}
使其与per-thread \co{counter}的值最终一致。
per-thread \co{counter}完美分区了counter值，
而\co{global_count}保持完整值。

对于limit counters，我们可以使用这个主题的变体，
即\emph{部分分区}counter。
例如，考虑四个线程，每个线程不仅有一个
per-thread \co{counter}，还有一个per-thread最大值
（称之为\co{countermax}）。
...
虽然此过程仍然需要全局lock，但该lock只需要每五次递增操作
获取一次，大大降低了该lock的\IXalt{竞争}{lock contention}程度。
...
基于这个差距来设置。
当远离限制时，\co{countermax} per-thread变量设置为较大值
以优化性能和可扩展性，
而接近限制时，这些变量设置为较小值
以最小化对\co{globalcountmax}限制检查的误差。

这种设计是\emph{parallel fastpath}的一个例子，
...
比较和分支会有一些开销。
然而，使用per-thread \co{countermax}储备意味着
...
通常也有一个可容忍的近似程度限制。
限制近似程度的一种方法是对per-thread \co{countermax}实例
的值施加上限。
这个任务在下一节中进行。
...
精确数量，还需要防止任何未来的访问开始。
一种实现方法是在更新counter时读获取读写lock，
在检查counter时写获取同一读写lock。
执行I/O的代码可能如下：
...
部分分区的计数算法使用lock来保护全局数据，
lock是\cref{chp:Locking}的主题。
