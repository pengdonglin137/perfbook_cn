...
\QuickQuiz{
	...
	可以追溯到2005年。
	从那以后，C和C++内存模型（memory model）~\cite{PeteBecker2011N3242}
	已经被形式化
...
本节面向希望跳过\IXalthmr{Linux-kernel memory model (\acr{lkmm})}
{Linux kernel}{memory model}全部细节的读者，
让他们可以更快地回到并发编程实践中来。
...
但首先，需要理解的是，有用的排序并非系统中某个
单独组件的属性，而是整个系统的属性。
例如，如果编译器不保持排序，
硬件保持排序也无济于事。
同理，即使硬件和编译器都提供了保持排序的方法，
如果开发者不使用这些方法，同样无济于事。
...
接下来，需要理解的是，使用内存作为通信媒介时，
从一个线程到另一个线程的通信具有时序性和非时序性，
这将在以下章节中详细讨论：
\cref{sec:memorder:Multicopy Atomicity}.
关键点在于，虽然load和store在概念上很简单，
但在真实的多核硬件上，它们的效果需要相当长的时间
才能对所有其他线程可见。
虽然听起来不可思议，但给定store的值可能不会被
挂钟时间上更晚的load返回，
也可能被挂钟时间上更早的store覆盖。

简单且直观的情况发生在一个线程加载另一个线程
所存储的值时。
这种直接的因果情况表现出时序行为，因此
...
更弱的排序。
以下经验法则捕捉了这些差异。
然而，需要注意的是，这些规则都不涉及互斥。
这些规则具有if-then的性质，如下所示：
...
完整屏障包括\co{smp_mb()}、成功的全强度非\co{void}
...
也作为完整屏障起作用，但代价远比\co{smp_mb()}高。
力量伴随着代价，尽管完整屏障通常对性能的损害
大于对可扩展性的损害。
这条经验法则的极端逻辑终点是
...
\begin{enumerate}
\item	内存排序操作仅在至少两个线程共享至少两个变量时才是必需的。
...
可能比未调优的软件运行得更慢。
解决这个问题的一个方案是，一旦CPU开始退化就将系统退出服务。
...
另一个选择是回顾\cref{chp:Hardware and its Habits}中的教训，
特别是对于许多重要的工作负载，主存跟不上现代CPU，
现代CPU可以在从内存获取单个变量所需的时间内
执行数百条指令。
对于这样的工作负载，CPU的详细内部结构无关紧要，
...
负责通过使用\emph{\IXBpl{memory barrier}}（内存屏障）
（例如Linux内核中的\co{smp_mb()}）来维持排序的假象。
这些内存屏障可以是显式指令，如在\ARM、\Power{}、Itanium和Alpha上，
...
\co{smp_mb()} Linux内核\IXh{full}{memory barrier}（完整内存屏障），
...
% No
这些屏障在我的x86笔记本电脑上100,000,000次试验中
阻止了反直觉结果的发生。
有趣的是，这些屏障带来的额外开销导致
...
load-load 控制依赖需要一个完整的\IXh{read}{memory barrier}（读内存屏障），
而不仅仅是数据依赖屏障。
考虑以下代码片段：
...
给定 CPU 的内存屏障指令仅与内存子系统（store buffer、cache 和互连）交互，
而不直接与其他 CPU 交互。
这意味着为了两个 CPU 彼此同步，每个 CPU 都必须执行
内存屏障指令，如
\cref{fig:memorder:Symmetric Memory Barriers Interactions}
中 CPU~0 和~1 所示。
考虑到内存屏障配对及其推广 cycle 的需求，这并不令人惊讶。

\QuickQuiz{
	\cref{fig:memorder:Symmetric Memory Barriers Interactions}
	显示内存屏障与 store buffer、cache、互连和内存交互。
	但轻量级内存排序操作（load-acquire、
	...
	轻量级内存排序操作之所以可以只与 store buffer 交互，
...
\emph{非对称内存屏障} 背后的技巧是创建一个
\emph{重量级}内存屏障，当给定 CPU 执行它时，
...
描绘，它显示 CPU~0 执行内存屏障指令但也向 CPU~1 发送 IPI。
当 CPU~1 执行 IPI 处理程序时，它也执行内存屏障指令，
从而提供所需的内存屏障配对，但不需要 CPU~1 自行
执行任何内存屏障指令。
CPU~0 的内存屏障结合其 IPI（因此结合 CPU~1 的非自愿
内存屏障）构成了重量级内存屏障。

然而，CPU~1 不能完全被动，因为编译器可以重排内存引用操作，
这可能会破坏排序。
因此 CPU~1 的代码必须包含 Linux 内核的 \co{barrier()}
编译器指令，这被称为\emph{轻量级}内存屏障。

重量级非对称内存屏障可以与对称内存屏障
（或任何其他提供足够排序的操作）配对，
也可以与重量级或轻量级非对称内存屏障配对。
换句话说，重量级非对称内存屏障可以与对称
内存屏障能配对的所有东西配对，它还可以与
轻量级非对称内存屏障配对。
然而，轻量级非对称内存屏障 \emph{只能}与
重量级内存屏障配对。
% @@@ Double-check this with litmus tests.

许多系统提供对重量级非对称内存屏障的用户空间访问。
...
\QuickQuiz{
	RCU 不是早在 2000 年之前就作为非对称内存屏障的
	一个例子吗？重量级屏障由 \co{synchronize_rcu()} 提供，
	轻量级屏障则是 \co{rcu_read_lock()} 和
	\co{rcu_read_unlock()} 之间你想要的任何地方的隐式屏障？
}\QuickQuizAnswer{
	算是吧？

	如果你持这种观点，你可以将非对称内存屏障
	视为针对特定 RCU 用例的优化。
	...
	非对称内存屏障~\cite{McKenney01f}。
	还有未经证实的传言称 DEC 的 Digital UNIX 在 1990 年代
	就使用了非对称内存屏障。

	但我们可能需要面对这样一个事实：非对称内存屏障的
	发明已经消失在时间的沙尘中。
}\QuickQuizEnd
...
\begin{fcvref}[ln:memorder:synchronize-rcu]
	...
	一些研究工作曾使用硬件事务内存（transactional memory）来允许编译器更积极地
