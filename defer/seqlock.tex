...
\QuickQuiz{
	为什么这个 顺序锁 讨论不在\cref{chp:Locking}中，
	你知道的，关于\emph{锁}的那一章？
}\QuickQuizAnswer{
	顺序锁机制实际上是两种独立同步机制的组合，
	即序列计数和锁。
	...
	所以这一节标题为``顺序锁''以便人们仅从标题就能
	理解它讲的是什么，
	它出现在``延迟处理''中是因为 (1) 强调了``顺序锁''
	的``sequence count''方面，(2) 因为``顺序锁''远不仅仅是一个锁。
}\QuickQuizEnd
...
结果是，受顺序锁保护的数据可以有任意多个并发读者，
但同一时间只能有一个写者。
顺序锁在 Linux 内核中用于保护时间保持所用的校准量。
...
\Clnref{struct:sl}添加了 顺序锁，由 \co{route_lookup()} 使用
\end{fcvref}
...
顺序锁的读侧和写侧临界区都可以被视为事务，
因此顺序锁可以被视为事务内存的一种受限形式，
这将在\cref{sec:future:Transactional Memory}中讨论。
顺序锁的局限性是：
\begin{enumerate*}[(1)]
\item 顺序锁限制更新，
\item 顺序锁不允许遍历可能被更新者释放的对象的指针。
\end{enumerate*}
这些局限性当然可以被事务内存克服，
但也可以通过将其他同步原语与顺序锁结合来克服。
...
另一方面，在没有写者的情况下，顺序锁读者相当快且线性可扩展。
人们自然想要两全其美：
快速的读者且没有读侧失败的可能性，更不用说饥饿了。
此外，克服顺序锁在指针方面的局限性也会很好。
