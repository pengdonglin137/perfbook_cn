...
从最基本的角度来说，RCU就是一种等待事物结束的方式。
当然，有很多其他的方式可以用来等待事物结束，比如引用计数、
读写锁、事件等等。
RCU的最伟大之处在于它可以等待（比如说）20,000种不同的事物，
而无需显式地去跟踪它们中的每一个，也无需去担心对性能的影响、
对可扩展性的限制、复杂的死锁场景，还有内存泄漏带来的危害等等
使用显式跟踪手段会出现的问题。

在RCU的例子中，被等待的事物称为
\emph{\IXBhmr{RCU read-side}{critical section}}。
如\cref{tab:defer:Core RCU API}中所述，
RCU read-side critical section从\co{rcu_read_lock()}原语开始，
到对应的\co{rcu_read_unlock()}原语结束。
RCU read-side critical section可以嵌套，也可以包含几乎任何代码，
只要这其中的代码不包含quiescent state。
...
如果你遵守这些约定，就可以使用RCU去等待\emph{任何}已有的
RCU read-side critical section完成。
RCU通过间接地确定这些事物何时完成来实现这一壮举，\co{synchronize_rcu()}
正是用这种间接方式来执行实际的
...
\cref{sec:together:Correlated Data Elements}中描述的顺序锁的
使用。
