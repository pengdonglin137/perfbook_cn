...
然后指出了\IXpl{hazard pointer}、顺序锁和RCU的
一些公开可见的生产用途\@。
...
无论是在对象内部还是外部。
引用计数可以在结构中每个对象内使用一个整数，
不需要额外存储。
风险指针需要配置对象外部的指针，并且必须有足够的指针
供每个CPU或线程在任何给定时间跟踪所有被引用的对象。
鉴于大多数基于风险指针的遍历只需要少量风险指针，
这在实践中通常不是问题。
当然，顺序锁不提供指针遍历保护，
这就是它们通常用于静态数据的原因。
...
顺序锁当然必须在其critical section与任何更新并发
...
该被移除对象可以被释放之间的最小延迟。
引用计数是唯一能够在最后一个读者完成后立即释放的
技术。
这一优势当然是引用计数大缺点的另一面——
它的读者慢且不可扩展。
理论上，风险指针也可以立即回收，但在实践中
生产级风险指针实现使用批处理来将扫描风险指针的
开销分摊到多次更新上，这种批处理会产生额外延迟。
RCU必须等待所有已有读者完成，无论这些读者是否与
新移除的对象有任何关系，然后，与风险指针一样，
生产级RCU实现由于批处理而产生额外延迟。

当然，不同的行在不同情况下会有不同的重要性级别。
例如，如果你当前的代码在使用风险指针时遇到读侧
可扩展性问题，那么风险指针可能需要重试引用获取
...
那么顺序锁无法保护指针这一点就无关紧要了。

最后，有一些关于根据延迟动态采样在风险指针和RCU之间
动态切换的工作~\cite{Balmau:2016:FRM:2935764.2935790}。
这将风险指针和RCU之间的选择推迟到运行时，
并将决策责任委托给软件。
...
你必须使用引用计数、风险指针或RCU\@。
顺序锁不提供存在性保证，而是提供更新检测，
重试任何遇到更新的read-side critical section。

当然，如"更新与读者并发推进"行所示，
这种更新检测意味着顺序锁不允许更新者和读者
并发地向前推进。
毕竟，阻止这种并发前进正是使用顺序锁的全部意义！
这种情况指出了一种方式：将顺序锁与引用计数、
风险指针或RCU结合使用，以同时提供存在性保证和更新检测。
事实上，Linux内核在路径名查找期间就以这种方式结合了RCU和
顺序锁。

"读者间争用"、"读者每个critical section开销"和
"读者每个对象遍历开销"行给出了这些技术读侧开销的大致感觉。
引用计数的开销可能相当大，读者之间存在争用，
而且每遍历一个对象都需要一个完全有序的读-修改-写原子操作。
风险指针为每个遍历的数据元素产生一次\IX{memory barrier}的开销，
顺序锁为每次尝试执行critical section产生一对
memory barrier的开销。
...
顺序锁的条目是"不安全"，因为顺序锁检测更新
而不是获取引用。
引用计数和风险指针都要求在给定获取失败时
从头重新开始遍历。
...
本节指出了风险指针、顺序锁和RCU的一些
公开可见的生产用途。
引用计数被省略，不是因为它不重要，而是因为它不仅
...
2010年，Keith Bostic向WiredTiger添加了风险指针的一个
变体~\cite{KeithBostic2010WiredTigerhazptr}。
2015年发布的MongoDB 3.0包含了WiredTiger，因此也包含了风险指针。

2011年，Samy Al Bahra向Concurrency Kit库添加了
风险指针~\cite{SamyAlBahra2011ckhp}。

2014年，Maxim Khizhinsky向libcds添加了
风险指针~\cite{MaximKhizhinsky2014libcdsHazptr}。

2015年，David Gwynne向OpenBSD引入了shared reference pointer，
一种风险指针的形式~\cite{DavidGwynne2015srp}。
...
风险指针~\cite{MagedMichael2018FollyHazptr}，
并在该库中被大量使用。
...
顺序锁~\cite{JonathanCorbet2003seqlock}，
这是从x86实现\co{gettimeofday()}系统调用时使用的一种
临时技术泛化而来。

2011年，Samy Al Bahra向Concurrency Kit库添加了
顺序锁~\cite{SamyAlBahra2011ckseqlock}。

2013年，Paolo Bonzini向QEMU模拟器添加了简单的
顺序锁~\cite{PaoloBonzini2013QEMUseqlock}。

2016年，Alexis Menard在Chromium中抽象了一个
顺序锁实现~\cite{AlexisMenard2016ChromiumSeqLock}。

2018年，一个简单的顺序锁实现被添加到
\co{jemalloc()}中~\cite{DavidGoldblatt2018seqlock}。
eigen库也有一个由类似于顺序锁的机制管理的
特殊用途队列。
