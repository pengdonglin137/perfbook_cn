...
\caption{受引用计数保护的 Pre-BSD 路由表，对数刻度}
...
多个更新线程（thread）反复调用 \co{route_add()} 和 \co{route_del()} 将很快
...
那一瞬间没有保护！
这是 Gamsa 等人~\cite{Gamsa99}注意到的锁问题的引用计数对应物。
