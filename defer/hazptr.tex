...
它尝试将 \co{p} 引用的指针存储到 \co{hp} 引用的 风险指针 中。
如果成功，它返回存储指针的值。
...
\QuickQuiz{
	鉴于关于风险指针的论文使用每个指针的低位来标记已删除的元素，
	\co{HAZPTR_POISON} 是怎么回事？
}\QuickQuizAnswer{
	风险指针的已发布实现使用非阻塞同步技术来进行插入和删除。
...
\co{hp_clear()} 函数更加直接，
使用 \co{smp_mb()} 来强制调用者对 风险指针 保护的对象的使用
与将 风险指针 设置为 \co{NULL} 之间的完全排序。
\end{fcvref}
...
每个线程固定的最大 风险指针 数（\co{K}），
这允许使用固定大小的风险指针数组。
因为任何线程都可能需要扫描风险指针，每个线程维护自己的数组，
由每线程变量 \co{gplist} 引用。
...
所有 风险指针，将非 NULL 指针累积到 \co{plist} 数组中，
并在 \co{psize} 中计数。
\clnref{mb2}上的 memory barrier 确保对风险指针的读取
发生在任何对象被释放之前。
...
如果\clnref{chkhazp:b,chkhazp:e}确定有风险指针
...
\clnref{hh}显示了用于排队待 风险指针 释放的对象的 \co{->hh} 字段，
\clnref{re_freed}显示了用于检测释放后使用错误的 \co{->re_freed} 字段，
\clnref{tryrecord}调用 \co{hp_try_record()} 尝试获取一个 风险指针。
...
受风险指针保护的链表，它经历以下事件序列：
\end{fcvref}

\begin{enumerate}
\item	线程~0 存储一个指向元素~B 的风险指针
	（大概是从元素~A 遍历到元素~B）。
\item	线程~1 从列表中移除元素~B，
	这将元素~B 到元素~C 的指针设置为特殊的
	\co{HAZPTR_POISON} 值以标记删除。
	因为线程~0 有一个指向元素~B 的风险指针，
	它还不能被释放。
\item	线程~1 从列表中移除元素~C。
	因为没有风险指针引用元素~C，
	它被立即释放。
\item	线程~0 尝试获取一个指向已移除元素~B 的后继的风险指针，
...
因此，风险指针读者在遇到并发删除时通常必须从头重新启动完整遍历。
...
\QuickQuiz{
	...
	则可以使用 \co{hp_record()} 来反复尝试记录 风险指针，
	即使在面对并发删除时也是如此。
...
因为使用风险指针的算法可能在其遍历链式数据结构的
任何步骤被重新启动，这些算法通常必须注意在获取所需更新的
所有风险指针之前避免对数据结构进行任何更改。
...
风险指针使 CPU 缓存能够进行资源复制，
这反过来允许弱化并行访问控制机制，
从而提升性能和可扩展性。

重新启动风险指针遍历的另一个优势是减少了最小内存占用：
任何当前没有被某个风险指针引用的对象都可以被立即释放。
...
\caption{风险指针 Pre-BSD 路由表添加\slash 删除}
...
\QuickQuizM{
	...
	有对数刻度 y 轴。
	其次，该论文使用轻量加载的哈希表（hash table），而
	\cref{fig:defer:Pre-BSD Routing Table Protected by Hazard Pointers}
	使用 10 元素的简单链表，这意味着风险指针
	...
	这可能将风险指针的性能提升到超出图中所示的水平。

	一如既往，结果因人而异。
	鉴于性能差异，很明显风险指针在以下情况下
	给你最佳性能：非常大的数据结构（其中 memory-barrier 开销
	至少部分与缓存未命中（cache miss）惩罚重叠）或像哈希表这样的数据结构，
	其中查找操作只需要最少数量的风险指针。
}\QuickQuizEndM
%
\QuickQuizE{
	...
	精确确定引用给定数据元素的风险指针数量何时降为零。
}\QuickQuizEndE
}

2023 年 6 月 17 日，ISO C++ 标准委员会投票将风险指针
纳入 C++26~\cite{MagedMichael2023C++26Hazptr3}。
Daniel Anderson 基于风险指针制作了一个 C++ 原子
shared-pointer 实现的原型~\cite{DanielAnderson2023HazptrSharedPtr}。

风险指针就是本章开头在\cpageref{sec:defer:Mysteries hazard pointers}
提到的并发引用计数器。
下一节试图通过使用顺序锁来改进风险指针，
顺序锁既避免了读侧写操作，也避免了每对象的 memory barriers。
