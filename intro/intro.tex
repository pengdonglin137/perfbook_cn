...
因此，如果你想论证并行编程将一直保持目前许多人所认为的那么困难，
那么举证责任在你，请记住在许多领域中有好几个世纪的反例。
...
事实上，并行系统长期以来一直在学生和爱好者的预算范围之内。
因此，我们可以期待围绕并行系统的发明和创新水平大幅提高，
而且随着时间推移，日益增长的熟悉度将使曾经费用高昂的
并行编程领域变得更加友好和普及。

第三，在20\textsuperscript{世纪}，大型高度并行的软件系统几乎总是被严格保护的专有秘密。
令人高兴的是，21\textsuperscript{世纪}已经涌现出许多开源（因而公开可用的）
...
但这些项目已经向其他社区输送了一批理解开发生产级并行代码所需工程学科的开发人员。
本书的一个主要目的就是展示这种工程学科。
...
因此，考虑并行编程的替代方案是有意义的。
然而，如果不了解并行编程的目标，就不可能合理地考虑并行编程的替代方案。
这一主题将在下一节中讨论。
...
性能是大多数并行编程工作背后的首要目标。
毕竟，如果性能不是问题，何不帮自己一个忙：
直接编写顺序代码，然后开心地收工呢？
顺序代码很可能更容易编写，你也很可能完成得更快。
...
话虽如此，性能的焦点已经从硬件转向了并行软件。
这一焦点变化是因为，尽管 \IXr{Moore's Law}
持续带来晶体管密度的增加，但它已经不再提供传统的单线程性能提升。
这可以从
...
话虽如此，仅仅因为你有多个 CPU 并不一定就是使用它们全部的理由，
特别是考虑到多 CPU 系统近年来的价格下降。
需要理解的关键点是，并行编程本质上是一种性能优化，
因此它是众多潜在优化方法之一。
如果你的程序以目前的写法已经足够快了，那就没有理由去优化，
无论是通过并行化还是通过应用众多潜在的顺序优化方法。\footnote{
	当然，如果你是一个主要兴趣在于编写并行软件的业余爱好者，
	那么并行化你感兴趣的任何软件都是足够充分的理由。}
同样道理，如果你想将并行性作为顺序程序的一种优化手段，
那么你需要将并行算法与最佳顺序算法进行比较。
这可能需要一些注意，因为太多的出版物在分析并行算法的性能时
忽略了顺序情况。
...
近几十年来，\IX{Productivity} 变得越来越重要。
为了理解这一点，请考虑早期计算机的价格高达数千万美元，
而当时工程师的薪水每年只有几千美元。
如果安排一个十人工程师团队专门服务于这样一台机器能将其性能
提高哪怕只有10\pct，那么他们的薪水就会被多倍回报。
...
硬件成本快速下降的一个不可避免的后果是，
软件生产力变得越来越重要。
仅仅高效地使用硬件已经不够了：现在还必须极其高效地利用软件开发人员。
对于顺序硬件来说，这早已如此，
但并行硬件只是在最近才成为低成本的商品。
...
证明并行软件高开发成本合理性的一种方法是
追求最大化的 \IX{generality}。
在其他条件相同的情况下，一个更通用的软件制品的成本
可以分摊到比一个不那么通用的制品更多的用户身上。
...
该图展示了生产力在系统栈上层如何变得越来越重要，
而性能和通用性在系统栈下层如何变得越来越重要。
在下层产生的巨大开发成本必须分摊到同样巨大数量的用户上
（因此通用性很重要），而且在底层损失的性能
无法在更高层轻松恢复。
在栈的上层，对于一个给定的特定应用可能只有很少的用户，
在这种情况下生产力问题至关重要。
这解释了栈上层趋向``臃肿软件''的倾向：
额外的硬件通常比额外的开发人员更便宜。
本书面向在栈底层工作的开发人员，在那里性能和通用性最为重要。
...
需要注意的是，生产力和通用性之间的权衡在许多领域已经存在了几个世纪。
举一个例子，钉枪在打钉子方面比锤子更高效，
但与钉枪不同，锤子除了打钉子外还可以用于许多其他事情。
...
运行顺序应用程序的多个实例可以让你在不实际进行并行编程的情况下
进行并行编程。
...
能够提供单线程编程环境的并行软件环境已不再缺乏，包括关系
数据库~\cite{Date82}、Web 应用服务器和 map-reduce 环境。
例如，一种常见的设计为每个用户提供一个单独的进程，
每个进程从用户查询生成 SQL。
这些针对每个用户的 SQL 在一个公共关系数据库上运行，
该数据库自动并发地运行用户的查询。
每个用户的程序只负责用户界面，
关系数据库则全权负责围绕并行性和持久性的困难问题。
...
并行化可以是一种强大的优化技术，但它不是唯一的这种技术，
也并非适合所有情况。
当然，并行化你的程序越容易，并行化作为一种优化就越有吸引力。
并行化有着相当困难的名声，
...
重要的是要注意，并行编程的困难既是人因问题，
也是并行编程问题的一组技术属性。
我们确实需要人类能够告诉并行系统该做什么，
也就是编程。
但并行编程涉及双向通信：程序的性能和可扩展性是
机器向人类的通信。
简而言之，人类编写程序告诉计算机该做什么，
而计算机通过产生的性能和可扩展性来评价这个程序。
因此，诉诸抽象或数学分析通常只能起到非常有限的作用。
...
并行程序可能需要执行非平凡的同步操作
才能安全地处理此类全局事件。
更一般地说，每个分区都需要某种形式的通信：
毕竟，如果一个给定的 thread 完全不进行通信，
它就不会产生任何效果，因此也不需要被执行。
然而，由于通信会产生开销，草率的划分选择
可能导致严重的性能下降。
...
这可能使程序难以理解和调试，从而降低生产力。
在其他条件相同的情况下，具有更规则结构的较小状态空间
更容易理解，但这与其说是技术或数学陈述，
不如说是人因陈述。
好的并行设计可能具有极大的状态空间，
但由于其规则的结构而易于理解；
而糟糕的设计尽管状态空间相对较小却可能令人费解。
...
本节概述了并行编程的困难、目标和替代方案。
在此概述之后，讨论了什么使并行编程变得困难，
以及处理并行编程困难的高级方法。
那些仍然坚持认为并行编程不可能难以完成的人
应该回顾一些较早的并行编程
